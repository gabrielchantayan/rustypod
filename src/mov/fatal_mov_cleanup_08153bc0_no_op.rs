//! `fatal_mov_cleanup_08153bc0_no_op` — retailOS `FUN_08153bc0` @ **0x08153bc0**
//! (4 bytes exactly; true extent `0x08153bc0..0x08153bc4`; the separately
//! linked next function begins at `0x08153bc4`).
//!
...
//! (`0x081c25b8`, `0x081c2dbc`, `0x081c34d4`, and `0x081c425c`) and no
...
pub unsafe extern "C" fn fatal_mov_cleanup_08153bc0_no_op(_object: *mut u8) -> *mut u8 {
    core::arch::naked_asm!("bx lr");
}

/// Host-callable equivalent of the empty MOV cleanup target.
#[cfg(not(all(target_os = "none", target_arch = "arm")))]
#[inline(never)]
#[cfg_attr(target_os = "none", no_mangle)]
pub unsafe extern "C" fn fatal_mov_cleanup_08153bc0_no_op(object: *mut u8) -> *mut u8 {
    object
}

#[cfg(test)]
mod tests {
    use super::fatal_mov_cleanup_08153bc0_no_op;

    #[test]
    fn preserves_object_words_without_accessing_memory() {
        for object_word in [0usize, 1, 0x0800_0001, 0x0815_3bc0, usize::MAX] {
            let object = object_word as *mut u8;
            assert_eq!(unsafe { fatal_mov_cleanup_08153bc0_no_op(object) }, object, "object={object_word:#x}");
        }
    }
    ...
    fn does_not_mutate_a_valid_cleanup_object() {
        ...
        let before = object;
        let returned = unsafe { fatal_mov_cleanup_08153bc0_no_op(object.as_mut_ptr()) };
