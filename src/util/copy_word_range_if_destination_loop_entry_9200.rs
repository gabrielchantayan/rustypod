...
/// copy_word_range_if_destination_loop_entry_9200 — retailOS
/// `thunk_FUN_083e9218` @ **0x083e9200** (**4 bytes exactly**,
/// `0x083e9200..0x083e9204`; the next real function boundary is the transfer
/// body at `0x083e9204`). Raw firmware is `b 0x083e9218`, entering that body's
...
/// advanced destination. Two verified inbound plain `bl` calls (0x083e4c10 and
/// 0x083e4c4c), zero predicated `bl` calls, and no outbound calls.
///
/// Deliberate deviations: Rust implements the branch entry's complete behavior
/// directly. A unique no-code assembly barrier retains this byte-identical body
/// as an independently hookable target. `wrapping_add` models cursor address
/// arithmetic without Rust pointer-arithmetic UB; volatile access retains
/// ordered loads and stores and prevents libc-copy substitution.
///
/// # Safety
///
...
#[inline(never)]
pub unsafe extern "C" fn copy_word_range_if_destination_loop_entry_9200(
    mut source: *const u32,
    end: *const u32,
    mut destination: *mut u32,
) -> *mut u32 {
    core::arch::asm!("/* copy_word_range_if_destination_loop_entry_9200 */", options(nostack, preserves_flags));
    while source != end {
...
mod tests {
    ...
    use super::copy_word_range_if_destination_loop_entry_9200;
    ...
    fn copies_empty_and_nonempty_ranges_and_returns_cursor() {
        ...
        let empty_result = unsafe {
            copy_word_range_if_destination_loop_entry_9200(
                source.as_ptr(),
                source.as_ptr(),
