//! `fatal_operation_cleanup_no_op` — retailOS `FUN_0815532c` @ **0x0815532c**
//! (4 bytes exactly; true extent `0x0815532c..0x08155330`; the separately
//! linked next function begins at `0x08155330`).
//!
...
//! (`0x081c26c8`, `0x081c2ec8`, `0x081c35f4`, and `0x081c43e0`) and no
//! predicated direct `bl` calls. Each caller invokes it with an opaque cleanup
...
/// Returns the cleanup object unchanged without accessing it.
///
/// # Safety
///
...
pub unsafe extern "C" fn fatal_operation_cleanup_no_op(_object: *mut u8) -> *mut u8 {
...
#[inline(never)]
#[cfg_attr(target_os = "none", no_mangle)]
pub unsafe extern "C" fn fatal_operation_cleanup_no_op(object: *mut u8) -> *mut u8 {
    object
}

#[cfg(test)]
mod tests {
    use super::fatal_operation_cleanup_no_op;
    ...
    fn preserves_object_words_without_accessing_memory() {
        for object_word in [0usize, 1, 0x0800_0001, 0x08a2_55e4, usize::MAX] {
            ...
            assert_eq!(unsafe { fatal_operation_cleanup_no_op(object) }, object, "object={object_word:#x}");
        }
    }
    ...
    fn does_not_mutate_a_valid_cleanup_object() {
        ...
        let before = object;
        let returned = unsafe { fatal_operation_cleanup_no_op(object.as_mut_ptr()) };
