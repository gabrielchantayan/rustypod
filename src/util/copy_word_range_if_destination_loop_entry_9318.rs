//! Loop-entry conditional forward word-range copy helper.

/// copy_word_range_if_destination_loop_entry_9318 — original:
/// `thunk_FUN_083e9330` @ **0x083e9318** (**4 bytes exactly**,
/// `0x083e9318..0x083e931c`; the next real function boundary is the transfer
/// body at `0x083e931c`).
///
/// Raw firmware is `b 0x083e9330`; that target is the loop test in the
/// 36-byte `0x083e931c..0x083e9340` conditional forward word-copy routine.
/// Two direct inbound plain `bl` calls target this entry (0x083e5640 and
/// 0x083e567c); there are no predicated `bl` calls. It advances `source` and
...
///
/// Deliberate deviations: the Rust export implements the loop-entry behavior
...
#[inline(never)]
#[cfg_attr(target_os = "none", link_section = ".text.copy_word_range_if_destination_loop_entry_9318")]
pub unsafe extern "C" fn copy_word_range_if_destination_loop_entry_9318(
...
mod tests {
    ...
    use super::copy_word_range_if_destination_loop_entry_9318;
    ...
    fn copies_range_and_returns_advanced_destination() {
        ...
        let result = unsafe {
            copy_word_range_if_destination_loop_entry_9318(
                source.as_ptr(),
    ...
    #[test]
    fn empty_range_preserves_destination_cursor() {
        let source = [0x11223344];
        let mut destination = [0xa5a5a5a5];
        ...
        assert_eq!(destination, [0xa5a5a5a5]);
