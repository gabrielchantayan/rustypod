...
/// `xor_8d_in_place` — original: `FUN_0833dd18` @ 0x0833dd18 (48 bytes;
/// 5 verified direct `bl` call sites, all unconditional).
///
/// Starting at `bytes`, XORs each byte in the signed half-open range `0..len`
/// with `0x8d`, loading and storing each element before advancing to the next.
...
#[inline(never)]
pub unsafe extern "C" fn xor_8d_in_place(bytes: *mut u8, len: i32) {
    for offset in 0..len {
        let byte = bytes.add(offset as usize);
        byte.write(byte.read() ^ 0x8d);
    }
}
...
mod tests {
    ...
    use super::xor_8d_in_place;

    fn reference_xor_8d_in_place(bytes: &mut [u8], len: i32) {
        for offset in 0..len {
            bytes[offset as usize] ^= 0x8d;
        }
    }

    #[test]
    fn xors_only_the_requested_range_with_8d() {
        let initial = [0x00, 0x72, 0x8d, 0xff, 0x80, 0x55, 0xaa, 0x7e];
        let mut actual = [0xa5; 12];
        let mut expected = actual;
        actual[2..10].copy_from_slice(&initial);
        expected[2..10].copy_from_slice(&initial);

        reference_xor_8d_in_place(&mut expected[2..10], 5);
        unsafe { xor_8d_in_place(actual[2..10].as_mut_ptr(), 5) };
    ...
    fn non_positive_lengths_leave_the_buffer_untouched() {
        for len in [-7, -1, 0] {
            ...
            unsafe { xor_8d_in_place(actual.as_mut_ptr(), len) };
