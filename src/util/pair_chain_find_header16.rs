//! Finds a node in a singly linked chain by a key in its 16-byte header.

/// `pair_chain_find_header16` — original: `FUN_083d6d80` @ `0x083d6d80` (72 bytes).
///
/// Raw A32 establishes the exact extent `0x083d6d80..0x083d6dc7`; the next
/// independently entered function begins with `push {r4,r5,r6,lr}` at
/// `0x083d6dc8`. Whole-image decoding finds two inbound plain `bl` sites
/// (`0x083d22a4` and `0x083d2b98`) and no predicated inbound `bl` sites. The
/// body has one plain `bl` to `FUN_083d7b88` and no predicated calls. It starts
/// at `*head_link`, compares the two words 16 bytes before each node against
/// `key`, and follows the node's target-width `+0x00` link until a match or
/// NULL.
///
...
///
/// # Safety
/// `head_link`, `key`, every nonzero node, and each node's preceding four words
/// must be valid aligned target-width addresses. `comparison_context` is not
/// dereferenced.
#[cfg_attr(target_os = "none", no_mangle)]
#[inline(never)]
pub unsafe extern "C" fn pair_chain_find_header16(
    ...
    while node != 0 {
        let candidate = (node as usize - 16) as *const u32;
...
mod tests {
    ...
    use super::pair_chain_find_header16;
    ...
    static FIXTURE: LazyLock<Option<usize>> = LazyLock::new(|| {
        try_map_u32_slab(hints::PAIR_CHAIN_FIND_HEADER16, FIXTURE_BYTES).map(|p| p as usize)
    });
    ...
    fn finds_head_middle_and_tail_by_complete_pair() {
        ...
        let Some(base) = *FIXTURE else {
            assert!(note_missing_u32_fixture("util/pair_chain_find_header16"));
            return;
        };
        unsafe {
            ...
            *head_link = head;
            ...
            assert_eq!(pair_chain_find_header16(0, head_link as usize as u32, key as usize as u32), middle);
            *key = 1;
            *key.add(1) = 2;
            assert_eq!(pair_chain_find_header16(0, head_link as usize as u32, key as usize as u32), head);
            *key = 7;
            *key.add(1) = 9;
            assert_eq!(pair_chain_find_header16(0, head_link as usize as u32, key as usize as u32), tail);
        }
    }
    ...
    fn returns_null_for_empty_chain_and_partial_pair_match() {
        ...
        unsafe {
            ...
            *key.add(1) = 5;
            let head_link = word(base, 0x60);
            *head_link = 0;
            assert_eq!(pair_chain_find_header16(0, head_link as usize as u32, key as usize as u32), 0);
