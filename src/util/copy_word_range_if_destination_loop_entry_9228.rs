...
/// copy_word_range_if_destination_loop_entry_9228 — retailOS
/// `thunk_FUN_083e9240` @ **0x083e9228** (**4 bytes exactly**,
/// `0x083e9228..0x083e922c`; the next real function boundary is the transfer
/// body at `0x083e922c`). Raw firmware is `b 0x083e9240`, entering that body's
...
/// advanced destination. Two verified inbound plain `bl` calls (0x083e4d44,
/// 0x083e4d80), zero predicated `bl` calls, and no outbound calls.
///
...
#[inline(never)]
pub unsafe extern "C" fn copy_word_range_if_destination_loop_entry_9228(
    mut source: *const u32,
    end: *const u32,
    mut destination: *mut u32,
) -> *mut u32 {
    core::arch::asm!("/* copy_word_range_if_destination_loop_entry_9228 */", options(nostack, preserves_flags));
    while source != end {
...
mod tests {
    ...
    use super::copy_word_range_if_destination_loop_entry_9228;
    ...
    fn copies_empty_and_nonempty_ranges_and_returns_cursor() {
        ...
        let empty_result = unsafe {
            copy_word_range_if_destination_loop_entry_9228(
                source.as_ptr(),
                source.as_ptr(),
