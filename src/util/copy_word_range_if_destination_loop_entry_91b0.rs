...
/// copy_word_range_if_destination_loop_entry_91b0 — retailOS
/// `thunk_FUN_083e91c8` @ **0x083e91b0** (**4 bytes exactly**,
/// `0x083e91b0..0x083e91b4`). Raw firmware is `b 0x083e91c8`, entering the
/// loop test of the separately entered 32-byte transfer body at
/// `0x083e91b4..0x083e91d4`; the next real function boundary is `0x083e91d8`.
...
/// destination. Two verified inbound plain `bl` calls, zero predicated `bl`
/// calls, and no outbound calls.
///
...
#[inline(never)]
pub unsafe extern "C" fn copy_word_range_if_destination_loop_entry_91b0(
    mut source: *const u32,
    end: *const u32,
    mut destination: *mut u32,
) -> *mut u32 {
    core::arch::asm!("/* copy_word_range_if_destination_loop_entry_91b0 */", options(nostack, preserves_flags));
    while source != end {
...
mod tests {
    ...
    use super::copy_word_range_if_destination_loop_entry_91b0;
    ...
    fn copies_empty_and_nonempty_ranges_and_returns_cursor() {
        ...
        let empty_result = unsafe {
            copy_word_range_if_destination_loop_entry_91b0(
                source.as_ptr(),
                source.as_ptr(),
