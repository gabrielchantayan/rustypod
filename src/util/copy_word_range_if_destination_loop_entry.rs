...
/// copy_word_range_if_destination_loop_entry — original:
/// `thunk_FUN_083e9358` @ **0x083e9340** (**4 bytes exactly**,
/// `0x083e9340..0x083e9344`; the next real function boundary is the transfer
/// body at `0x083e9344`).
///
/// Raw firmware is `b 0x083e9358`; that target is the loop test in the
/// 36-byte `0x083e9344..0x083e9368` conditional forward word-copy routine.
/// Two direct inbound plain `bl` calls target this entry (0x083e5784 and
/// 0x083e57c0); there are no predicated `bl` calls. It advances `source` and
...
#[inline(never)]
pub unsafe extern "C" fn copy_word_range_if_destination_loop_entry(
...
mod tests {
    ...
    use super::copy_word_range_if_destination_loop_entry;
    ...
    fn copies_range_and_returns_advanced_destination() {
        ...
        let result = unsafe {
            copy_word_range_if_destination_loop_entry(
                source.as_ptr(),
