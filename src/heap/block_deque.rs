...
//!   port in heap/client_erase.rs — + commit @ 0x081fc884 — ops slot
//!   defaulting to the real port in heap/client_commit.rs, its 0/1
//!   verdict discarded as in the original; the client handle is
//!   re-read before every call, faithfully), then zeroes the fill
//!   counters (+0x44/+0x48). The
...
//!   (0x081fbe4c, 0x081fc3f4, 0x081fc298) default to failure stubs
//!   matching the no-manager state; the erase
//!   @ 0x081fc080 defaults to the real port in heap/client_erase.rs
//!   (whose own unported callees default to no-ops, so the wired
//!   behavior is unchanged: a drained deque, nothing handed back) and
//!   the commit @ 0x081fc884 defaults to the real port in
//!   heap/client_commit.rs (whose unported body stub reports the
//!   no-manager refusal 0, discarded here).
...
pub struct PoolBaseOps {
    ...
    /// Block hand-back pair @ 0x081fc080 `(client, deque)` and
    /// 0x081fc884 `(client)` — defaults: the real ports
    /// (heap/client_erase.rs, heap/client_commit.rs). The commit's 0/1
    /// verdict is discarded by `pool_base_release_blocks`, as in the
    /// original.
    pub client_erase: unsafe extern "C" fn(client: *mut u8, deque: *mut BlockDeque),
    pub client_erase_commit: unsafe extern "C" fn(client: *mut u8) -> i32,
...
/// report failure. (The erase default is the real port in
/// heap/client_erase.rs, whose own unported callees no-op, and the
/// commit default is the real port in heap/client_commit.rs, whose
/// unported body stub reports the no-manager refusal 0 — both
/// behavior-identical for the no-manager state.)
...
pub(crate) const DEFAULT_POOL_BASE_OPS: PoolBaseOps = PoolBaseOps {
    ...
    client_erase_commit: crate::heap::client_commit::client_commit,
...
///
/// `deque_seg_capacity_alias_9e94` — original: `FUN_083d9e94` @
/// 0x083d9e94 (8 bytes, `0x083d9e94..0x083d9e9b`).
///
...
/// linked function, `deque_iter_assign_alias_9e9c`, begins at 0x083d9e9c.
/// Whole-image ARM B/BL decoding verifies two unconditional plain `bl` call
/// sites (0x083e9f48 and 0x083e9fd0), zero predicated forms, and zero body
...
#[cfg_attr(target_os = "none", no_mangle)]
#[cfg_attr(target_os = "none", unsafe(link_section = ".text.deque_seg_capacity_alias_9e94"))]
#[inline(never)]
pub unsafe extern "C" fn deque_seg_capacity_alias_9e94() -> usize {
    0x20
}
...
/// deque_construct_elem4_alias_ee30 — original: `FUN_083dee30` @
/// 0x083dee30 (64 bytes; 2 plain `bl` call sites at 0x081559e4 and
/// 0x081559f0; no predicated forms, binary-verified).
///
/// Default-constructs a 0x28-byte 4-byte-element deque head. The raw body
/// makes an all-NULL iterator in a four-word stack temporary through
/// [`crate::cxx::templates::deque_iter_init_elem4_alias_a26c`] @ 0x083da26c,
/// copies it to `end` then `begin`, and clears `count` and `map`.
///
/// Deliberate deviation: NULL inputs make the iterator deterministically
/// [`DequeIter::NULL`], so the port writes it directly and omits the
/// redundant stack temporary and callee. The original restores and returns
/// `dq`; so does this port.
///
/// # Safety
///
/// `dq` must be valid for writes through the 0x28-byte [`DequeHead`] prefix.
#[cfg_attr(target_os = "none", no_mangle)]
#[cfg_attr(target_os = "none", link_section = ".text.deque_construct_elem4_alias_ee30")]
#[inline(never)]
pub unsafe extern "C" fn deque_construct_elem4_alias_ee30(
    dq: *mut DequeHead,
) -> *mut DequeHead {
    (*dq).end = DequeIter::NULL;
    (*dq).begin = (*dq).end;
...
/// linked helper begins at 0x083df194). Raw ARM decoding finds two incoming
/// unconditional plain `bl` calls (0x08261b98 and 0x08261ba4), no predicated
/// forms, and one outgoing plain `bl` to
/// [`crate::cxx::templates::deque_iter_init_elem4_alias_a304`] @ 0x083da304.
///
/// Default-constructs a 0x28-byte deque head. Stock initializes a stack
/// iterator with NULL `cur` and slot, copies it to `end` then `begin`, and
/// clears `count` and `map`; its register restore returns the original `dq`.
///
/// Deliberate deviation: NULL inputs make the iterator unconditionally
/// [`DequeIter::NULL`], so this port writes it directly and omits the
/// redundant stack temporary and callee.
///
/// # Safety
///
/// `dq` must be valid for writes through the 0x28-byte [`DequeHead`] prefix.
#[cfg_attr(target_os = "none", no_mangle)]
#[cfg_attr(target_os = "none", link_section = ".text.deque_construct_elem4_alias_f154")]
#[inline(never)]
pub unsafe extern "C" fn deque_construct_elem4_alias_f154(
    dq: *mut DequeHead,
) -> *mut DequeHead {
    (*dq).end = DequeIter::NULL;
    (*dq).begin = (*dq).end;
...
mod tests {
    ...
    unsafe extern "C" fn mock_erase_commit(client: *mut u8) -> i32 {
    ...
    #[test]
    fn seg_capacity_alias_9e94_returns_its_raw_constant() {
        unsafe {
            assert_eq!(deque_seg_capacity_alias_9e94(), 0x20);
        }
    }
    ...
    #[test]
    fn construct_elem4_alias_f154_zeroes_a_garbage_head_and_returns_it() {
        let garbage = DequeIter {
            cur: 0x11 as *mut u8,
            seg_base: 0x22 as *mut u8,
            seg_end: 0x33 as *mut u8,
        ...
        unsafe {
            assert!(deque_construct_elem4_alias_f154(&mut dq) == &mut dq);
        }
        for it in [&dq.begin, &dq.end] {
            assert!(it.cur.is_null());
            assert!(it.seg_base.is_null());
    ...
    #[test]
    fn construct_elem4_alias_ee30_zeroes_a_garbage_head_and_returns_it() {
        let garbage = DequeIter {
            cur: 0x11 as *mut u8,
            seg_base: 0x22 as *mut u8,
            seg_end: 0x33 as *mut u8,
        ...
        unsafe {
            assert!(deque_construct_elem4_alias_ee30(&mut dq) == &mut dq);
        }
        for it in [&dq.begin, &dq.end] {
            assert!(it.cur.is_null());
            assert!(it.seg_base.is_null());
    ...
    #[test]
    fn construct_elem4_alias_ee30_preserves_following_map_capacity() {
        #[repr(C)]
        struct HeadWithTail {
            head: DequeHead,
            tail: u32,
        ...
        unsafe {
            deque_construct_elem4_alias_ee30(&mut obj.head);
        }
        assert_eq!(obj.tail, 0xcafe_babe);
    }
    ...
    #[test]
    fn construct_elem4_alias_f154_preserves_following_map_capacity() {
        #[repr(C)]
        struct HeadWithTail {
            head: DequeHead,
            tail: u32,
        ...
        unsafe {
            deque_construct_elem4_alias_f154(&mut obj.head);
        }
        assert_eq!(obj.tail, 0xcafe_babe);
    }
