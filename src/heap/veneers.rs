...
///
/// Raw `osos.dec` words `e3a00001 ea07dc13` establish the exact extent
/// 0x080b3d7c..0x080b3d83: `mov r0,#1; b 0x082aadd4`. The following
/// `push {r4,lr}` begins [`allocate_two_word_with_cleared_last_leading`] at
/// 0x080b3d84. This argument-less entry overwrites r0 with the one-byte size
/// and tail-branches to the tag-2 [`operator_new`], returning its allocation
/// unchanged, including NULL.
///
/// Deliberate deviation: Rust calls the existing allocator seam rather than
/// tail-branching. A distinct target-only section keeps this separately linked
/// `bl` target from folding into identical allocator veneers.
#[inline(never)]
#[cfg_attr(target_os = "none", link_section = ".text.allocate_one_byte_leading")]
#[cfg_attr(target_os = "none", no_mangle)]
pub unsafe extern "C" fn allocate_one_byte_leading() -> *mut u8 {
    operator_new(1)
}
...
/// inbound `bl` calls, none predicated).
///
/// Raw `osos.dec` words `e92d4010 e1a04000 e3a00001 eb07dc09 e5c04000
/// e8bd8010` establish the true extent 0x080b3d9c..0x080b3db3: `push
/// {r4,lr}; mov r4,r0; mov r0,#1; bl 0x082aadd4; strb r4,[r0]; pop
/// {r4,pc}`. The following `mov r0,#1` at 0x080b3db4 begins the next real
/// function. Raw A32 decoding finds the four inbound plain `bl` at
/// 0x081c2580, 0x081c2d84, 0x081c34a0, and 0x081c4228; no predicated `bl`
/// targets this entry. Allocates one tag-2 byte through [`operator_new`],
/// stores `value` unconditionally, then returns the allocation.
///
...
#[inline(never)]
#[cfg_attr(target_os = "none", link_section = ".text.allocate_byte_with_value_leading")]
#[cfg_attr(target_os = "none", no_mangle)]
pub unsafe extern "C" fn allocate_byte_with_value_leading(value: u8) -> *mut u8 {
...
/// allocate_byte_with_value_initial — original: `FUN_080b3dbc` @ 0x080b3dbc
/// (24 bytes; one plain direct `bl` in its body; four plain inbound `bl`
/// calls, none predicated).
///
/// Raw `osos.dec` words `e92d4010 e1a04000 e3a00001 eb07dc01 e5c04000
/// e8bd8010` establish the true extent 0x080b3dbc..0x080b3dd3: `push
/// {r4,lr}; mov r4,r0; mov r0,#1; bl 0x082aadd4; strb r4,[r0]; pop
/// {r4,pc}`. The following `push {r4,lr}` at 0x080b3dd4 begins
/// [`allocate_three_word_with_cleared_last_initial`]. Raw A32 decoding finds
/// the four inbound plain `bl` at 0x081c2764, 0x081c2f94, 0x081c369c, and
/// 0x081c4498; no predicated `bl` targets this entry. Allocates one tag-2
...
/// independently linked call target distinct from identical byte allocators.
#[inline(never)]
#[cfg_attr(target_os = "none", link_section = ".text.allocate_byte_with_value_initial")]
#[cfg_attr(target_os = "none", no_mangle)]
pub unsafe extern "C" fn allocate_byte_with_value_initial(value: u8) -> *mut u8 {
    let allocation = operator_new(1);
    allocation.write(value);
    allocation
}
...
/// allocate_one_byte_alternate — original: `FUN_080b3e84` @ 0x080b3e84
/// (8 bytes; 4 unconditional `bl` call sites, none predicated).
///
/// Raw ARM establishes the exact extent `0x080b3e84..0x080b3e8b`:
/// `mov r0,#1; b 0x082aadd4`. The next word, `push {r4,lr}` at
/// 0x080b3e8c, begins the separate
/// [`allocate_three_word_with_cleared_last`] entry. This alternate entry
/// overwrites its otherwise dead argument register with a one-byte size and
/// tail-branches to tag-2 [`operator_new`], returning its allocation
/// unchanged, including NULL.
///
/// Deliberate deviation: Rust makes the tail branch an ordinary call through
/// the existing allocator seam. A distinct target-only section preserves this
/// separately linked retailOS `bl` target despite its identical sibling at
/// 0x080b3ea4.
#[inline(never)]
#[cfg_attr(target_os = "none", link_section = ".text.allocate_one_byte_alternate")]
#[cfg_attr(target_os = "none", no_mangle)]
pub unsafe extern "C" fn allocate_one_byte_alternate() -> *mut u8 {
    operator_new(1)
}
...
pub(crate) mod tests {
    ...
    fn allocate_byte_with_value_initial_allocates_one_tag2_byte_and_stores_all_values() {
        ...
        unsafe {
            ...
            for (calls, value) in [0u8, 1, 0xff].into_iter().enumerate() {
                storage[0] = !value;
                let allocation = allocate_byte_with_value_initial(value);
                assert_eq!(allocation, storage.as_mut_ptr());
    ...
    #[test]
    fn allocate_byte_with_value_leading_allocates_one_tag2_byte_and_stores_all_values() {
        let _lock = mock_heap();
        let mut storage = [0u8; 1];
        unsafe {
            set_alloc_ret(storage.as_mut_ptr());
            for (calls, value) in [0u8, 1, 0xff].into_iter().enumerate() {
                storage[0] = !value;
                let allocation = allocate_byte_with_value_leading(value);
                assert_eq!(allocation, storage.as_mut_ptr());
                assert_eq!(storage[0], value);
                assert_eq!(ALLOC_CALLS, calls + 1);
                assert_eq!(LAST_ALLOC_SIZE, 1);
    ...
    #[test]
    fn allocate_one_byte_leading_forwards_fixed_size_and_null_unchanged() {
        let _lock = mock_heap();
        unsafe {
            set_alloc_ret(core::ptr::null_mut());
            assert!(allocate_one_byte_leading().is_null());
            assert_eq!(ALLOC_CALLS, 1);
            assert_eq!(LAST_ALLOC_SIZE, 1);
            assert_eq!(LAST_ALLOC_TAG, 2);

            set_alloc_ret(BLOCK_A as *mut u8);
            assert_eq!(allocate_one_byte_leading(), BLOCK_A as *mut u8);
            assert_eq!(ALLOC_CALLS, 2);
            assert_eq!(LAST_ALLOC_SIZE, 1);
            assert_eq!(LAST_ALLOC_TAG, 2);
        }
    ...
    #[test]
    fn allocate_one_byte_alternate_forwards_fixed_size_and_null_unchanged() {
        let _lock = mock_heap();
        unsafe {
            set_alloc_ret(core::ptr::null_mut());
            assert!(allocate_one_byte_alternate().is_null());
            assert_eq!(ALLOC_CALLS, 1);
            assert_eq!(LAST_ALLOC_SIZE, 1);
            assert_eq!(LAST_ALLOC_TAG, 2);

            set_alloc_ret(BLOCK_A as *mut u8);
            assert_eq!(allocate_one_byte_alternate(), BLOCK_A as *mut u8);
            assert_eq!(ALLOC_CALLS, 2);
            assert_eq!(LAST_ALLOC_SIZE, 1);
            assert_eq!(LAST_ALLOC_TAG, 2);
        }
