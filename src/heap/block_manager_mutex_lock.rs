//! `block_manager_mutex_lock` — retailOS `FUN_0818a1bc` at `0x0818a1bc`.
//!
//! Raw ARM words `e2800f52 ea035ff8` establish the complete 8-byte body
//! `add r0, r0, #0x148; b 0x082621a8`. The next real function begins at
//! 0x0818a1c4. Three verified inbound plain `bl` call sites and no predicated
//! `bl` call sites invoke this helper. It derives the block manager's C++
//! mutex at +0x148, then tail-branches to the `posix_mutex_lock` veneer.
//!
...
/// Locks the C++ mutex embedded at +0x148 in `block_manager`.
///
/// `block_manager` must point to the block manager object, as required by the
/// original unchecked address calculation.
#[cfg_attr(target_os = "none", no_mangle)]
#[cfg_attr(target_os = "none", link_section = ".text.block_manager_mutex_lock")]
#[inline(never)]
pub unsafe extern "C" fn block_manager_mutex_lock(block_manager: *mut u8) -> u32 {
    let lock = core::ptr::read_volatile(core::ptr::addr_of!(REGION_MUTEX_OPS.lock));
    lock(block_manager.add(BLOCK_MANAGER_MUTEX_OFFSET))
}
...
mod tests {
    ...
    #[test]
    fn locks_the_block_manager_mutex_at_the_target_offset() {
        let _reset = install();
        let mut block_manager = [0_u8; BLOCK_MANAGER_MUTEX_OFFSET + 1];
        unsafe {
            assert_eq!(block_manager_mutex_lock(block_manager.as_mut_ptr()), 0x5a);
            assert_eq!(LOCK_ARG, block_manager.as_mut_ptr().add(BLOCK_MANAGER_MUTEX_OFFSET) as usize);
        }
    }
}
