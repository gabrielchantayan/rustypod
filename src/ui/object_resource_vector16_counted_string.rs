//! Reads a counted string from a kind-resolved UI object's 16-byte resource vector.
...
const RESOURCE_VECTOR_OFFSET: usize = 0xec8;
...
/// object_resource_vector16_counted_string — original: `FUN_08053768` @
/// `0x08053768` (144 bytes; next real function starts at `0x080537f8`; **3
/// plain `bl` callers and no predicated-BL callers**).
///
...
/// index outside the object's `+0xec8` 16-byte-entry vector with `-50`, then
/// reads that entry's first word through the active item's inline pool at `+0xcc`.
///
...
pub unsafe extern "C" fn object_resource_vector16_counted_string(
    object: *const u8,
    index: i32,
    counted: *mut u16,
) -> i32 {
    ...
    let entries = object.add(RESOURCE_VECTOR_OFFSET).cast::<VectorBounds>();
...
mod tests {
    ...
    fn rejects_negative_and_out_of_range_indices_before_the_active_item_lookup() {
        ...
        let Some(object) = try_map_u32_slab(hints::OBJECT_RESOURCE_VECTOR16_COUNTED_STRING, 0x2000) else {
            assert!(note_missing_u32_fixture("ui/object_resource_vector16_counted_string"));
            return;
        };
        unsafe {
            object.write(1);
            ...
            (object.add(RESOURCE_VECTOR_OFFSET + 4) as *mut u32).write(table.add(16) as usize as u32);
            let mut counted = 0xffffu16;
            assert_eq!(object_resource_vector16_counted_string(object, -1, &mut counted), PARAM_ERR);
            assert_eq!(counted, 0);
            counted = 0xffff;
            assert_eq!(object_resource_vector16_counted_string(object, 1, &mut counted), PARAM_ERR);
            assert_eq!(counted, 0);
    ...
    fn reads_zero_id_and_restores_the_inline_pool_counter() {
        let _lock = TEST_LOCK.lock();
        let Some(object) = try_map_u32_slab(hints::OBJECT_RESOURCE_VECTOR16_COUNTED_STRING_VALID, 0x2000) else {
            assert!(note_missing_u32_fixture("ui/object_resource_vector16_counted_string"));
            return;
        };
        unsafe {
            ...
            let mut counted = 0xffffu16;
            assert_eq!(object_resource_vector16_counted_string(object, 0, &mut counted), 0);
            assert_eq!(counted, 0);
