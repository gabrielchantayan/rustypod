//! Whether an element can accept a vertical adjustment.
//!
//! `ui_element_vertical_adjustment_available` — original:
//! `FUN_082a251c` @ **0x082a251c**, 64 bytes
//! (`0x082a251c..0x082a255c`; the next distinct function starts at
//! `0x082a255c`). Raw decoding finds four direct, unconditional `bl` call
//! sites and no predicated `bl` call sites.
//!
//! # Algorithm
//!
//! The element has a vertical adjustment available when either its leading
...
const LEADING_TARGET_OFFSET: usize = 0x5c;
const LEADING_FLAGS_OFFSET: usize = 0x60;
const TRAILING_TARGET_OFFSET: usize = 0x74;
const TRAILING_FLAGS_OFFSET: usize = 0x78;

/// Returns whether either vertical adjustment target is absent and enabled.
#[cfg_attr(target_os = "none", no_mangle)]
#[inline(never)]
pub unsafe extern "C" fn ui_element_vertical_adjustment_available(element: *const u8) -> i32 {
    let trailing_target = ptr::read_volatile(element.add(TRAILING_TARGET_OFFSET).cast::<u32>());
    if trailing_target == 0
        && (ptr::read_volatile(element.add(TRAILING_FLAGS_OFFSET).cast::<u32>()) & ADJUSTMENT_AVAILABLE) != 0
    {
        return 1;
    }

    let leading_target = ptr::read_volatile(element.add(LEADING_TARGET_OFFSET).cast::<u32>());
    if leading_target == 0
        && (ptr::read_volatile(element.add(LEADING_FLAGS_OFFSET).cast::<u32>()) & ADJUSTMENT_AVAILABLE) != 0
    {
        return 1;
    }
...
mod tests {
    ...
    fn requires_an_absent_target_with_the_available_flag() {
        for (leading_target, leading_flags, trailing_target, trailing_flags, expected) in [
            ...
            assert_eq!(unsafe { ui_element_vertical_adjustment_available((&mut element as *mut ElementFixture).cast()) }, expected);
        }
    }
}
