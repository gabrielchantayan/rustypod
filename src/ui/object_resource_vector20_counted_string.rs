//! Reads a counted string through a UI object's 20-byte resource vector.
#[cfg(not(target_os = "none"))]
use crate::cxx::templates::VectorBounds;
use crate::cxx::templates::vector_size_elem20;
use crate::util::string_pool::{string_pool_read_counted, StringPool, PARAM_ERR};

use super::object_state::object_backend_for_kind;

const RESOURCE_VECTOR_OFFSET: usize = 0xe64;
const RESOURCE_ENTRY_SIZE: usize = 20;
const BACKEND_POOL_POINTER_OFFSET: usize = 0xf60;
const STRING_POOL_OFFSET: usize = 0x118;

/// object_resource_vector20_counted_string — original: `FUN_08052a0c` @
/// `0x08052a0c` (116 bytes; next real function begins at `0x08052a80`; four
...
/// rejects a negative index or one outside the object `+0xe64` vector of
/// 20-byte records with `-50`, then tail-branches to `string_pool_read_counted`
/// for the selected record's first word. Its two internal direct calls are plain
/// `bl` instructions to `object_backend_for_kind` and `vector_size_elem20`; the
...
pub unsafe extern "C" fn object_resource_vector20_counted_string(
    object: *const u8,
    index: i32,
    counted: *mut u16,
) -> i32 {
    ...
    if index >= vector_size_elem20(vector) {
        return PARAM_ERR;
    }
...
mod tests {
    ...
    unsafe fn fixture() -> Option<*mut u8> {
        try_map_u32_slab(hints::OBJECT_RESOURCE_VECTOR20_COUNTED_STRING, OBJECT_BYTES)
    }

    #[test]
    fn clears_output_before_rejecting_negative_and_out_of_range_indices() {
        let Some(object) = (unsafe { fixture() }) else {
            assert!(note_missing_u32_fixture("ui/object_resource_vector20_counted_string"));
            return;
        };
        unsafe {
            object.write(1);
            ...
            assert_eq!(object_resource_vector20_counted_string(object, -1, &mut counted), PARAM_ERR);
            assert_eq!(counted, 0);
            counted = 0xffff;
            assert_eq!(object_resource_vector20_counted_string(object, 1, &mut counted), PARAM_ERR);
            assert_eq!(counted, 0);
    ...
    #[test]
    fn reads_the_second_twenty_byte_record_through_the_backend_pool() {
        let Some(object) = (unsafe { fixture() }) else {
            assert!(note_missing_u32_fixture("ui/object_resource_vector20_counted_string"));
            return;
        };
        unsafe {
            ...
            let mut counted = 0xffffu16;
            assert_eq!(object_resource_vector20_counted_string(object, 1, &mut counted), 0);
            assert_eq!(counted, 0);
