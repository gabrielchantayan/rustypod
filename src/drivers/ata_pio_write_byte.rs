//! ATA PIO byte-write primitive.
//!
//! Port: [`ata_pio_write_byte`] — original: `FUN_080c4ba8` @ `0x080c4ba8`
//! (28 bytes, `0x080c4ba8..0x080c4bc4`; **21 `bl` call sites, no predicated
//! forms**). The next function begins at `0x080c4bc8`; the intervening word
//! at `0x080c4bc4` is this function's `0x38700000` literal pool. Decoding
...
//! stores the low byte of `value` to the caller-selected ATA task-file
//! register, and returns zero. It deliberately has no NULL or timeout path;
//! callers must supply a live PIO register and the controller must progress.
//!
...
/// ata_pio_write_byte — original: `FUN_080c4ba8` @ `0x080c4ba8` (28 bytes;
/// **21 `bl` call sites, all unconditional**).
///
/// Waits for `ATA_PIO_READY` bit 1, then writes `value` to `destination` and
/// returns the firmware's zero status. The byte store deliberately does not
/// validate the destination or impose a timeout.
///
/// # Safety
///
/// `destination` must be a valid, writable ATA task-file byte register.
#[inline(never)]
#[cfg_attr(target_os = "none", no_mangle)]
pub unsafe extern "C" fn ata_pio_write_byte(destination: *mut u8, value: u8) -> i32 {
...
mod tests {
    ...
    use super::{ata_pio_write_byte, HOST_ATA_PIO_READY};
    use core::sync::atomic::Ordering;
    use std::sync::mpsc;
    use std::time::Duration;

    #[test]
    fn waits_for_write_ready_bit_before_storing_byte() {
        ...
        let worker = std::thread::spawn(move || {
            ...
            let status = unsafe { ata_pio_write_byte(&mut destination, 0xa5) };
