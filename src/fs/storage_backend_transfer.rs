//! Storage-backend virtual-transfer dispatch.
//!
//! `storage_backend_transfer` — retailOS `FUN_08149e10` at load address
//! **0x08149e10**, 40 raw bytes (ten ARM words; the next independently linked
//! function begins at 0x08149e38). There are five inbound direct calls, all
//! plain unconditional `bl`; there are no predicated direct calls. Its body
//! makes one unconditional indirect `blx` through vtable slot `+0x0c`.
//!
//! The dispatcher holds its backend pointer at `+0x04`. The backend's first
//! word is its vtable, whose fourth word is invoked with the backend as `r0`
...
type StorageBackendTransfer = unsafe extern "C" fn(*mut u8, u32, u32, *mut u8, u32) -> i32;

#[cfg(not(target_os = "none"))]
unsafe extern "C" fn missing_storage_backend_transfer(
    _backend: *mut u8,
    _block: u32,
    _count: u32,
    _buffer: *mut u8,
...
#[cfg(not(target_os = "none"))]
static mut HOST_STORAGE_BACKEND_TRANSFER: StorageBackendTransfer = missing_storage_backend_transfer;

/// Dispatches a storage request through the backend vtable's slot three.
///
/// # Safety
///
/// On target, `dispatcher + 4`, its backend, and the backend vtable's fourth
...
#[cfg_attr(target_os = "none", link_section = ".text.storage_backend_transfer")]
#[inline(never)]
pub unsafe extern "C" fn storage_backend_transfer(
    dispatcher: *mut u8,
    block: u32,
    count: u32,
    buffer: *mut u8,
    ...
    #[cfg(target_os = "none")]
    {
        ...
        let method: StorageBackendTransfer = core::mem::transmute(
            ptr::read_volatile(vtable.add(3)) as usize,
        );
    ...
    #[cfg(not(target_os = "none"))]
    {
        let method = ptr::read_volatile(ptr::addr_of!(HOST_STORAGE_BACKEND_TRANSFER));
...
mod tests {
    ...
    unsafe fn install_recorder(status: i32) {
        HOST_STORAGE_BACKEND_TRANSFER = record_transfer;
        STATUS = status;
        CALLS.lock().clear();
    }

    unsafe fn reset_recorder() {
        HOST_STORAGE_BACKEND_TRANSFER = missing_storage_backend_transfer;
        CALLS.lock().clear();
    }
