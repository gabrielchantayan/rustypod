...
//! installed through the ported `mode_selected_position_set` at `0x0822ba50`
//! with mode argument 1. Independently, a changed cached position at `state+0x8b8`
...
//! `0x081cc618`.
//!
//! Deliberate deviation: the unported notification continuation is a fixed-address
//! call on ARM and a replaceable host seam for behavior tests.
...
unsafe fn set_backend_position(state: *mut u8, position: u32) {
    let _ = mode_selected_position_set(state, 1, position);
}
...
mod tests {
    ...
    fn clamps_signed_bounds_and_updates_both_observable_paths() {
        ...
        unsafe {
            ...
            assert_eq!(SETTER_STATE, state.0.as_mut_ptr().add(BACKEND_OFFSET + 0x38));
