...
//! - The first three constructors, [`element_array0_construct`],
//!   [`element_array1_construct`], and [`element_array2_construct`], are
//!   ported and wired as arrays 0 through 2's defaults. The other six
//!   constructors remain behind documented zeroing stubs. Thus arrays 0
//!   through 2 are empty, ten-slot containers after default construction;
//!   arrays 3 through 8 still report zero slots. The tracker-registration
//!   helper each constructor calls is a four-byte `mov pc, lr` stub in
//!   retailOS, so no registration seam is required.
...
/// Wired defaults: arrays 0 through 2 use their retailOS constructors; the
...
mod tests {
    ...
    static mut ALLOC_RESULTS: [*mut u8; 7] = [ptr::null_mut(); 7];
    ...
    #[test]
    fn wired_defaults_construct_arrays0_through2_but_leave_all_slots_empty() {
        let guard = mock();
        unsafe {
            // The table block is allocation 0; arrays 0 through 2 each
