...
//! | 0x081ded14 | [`lazy_singleton_0x28`] | 0x28 | 0x089cc7c8 | 0x081df284 | 26 |
...
//!
//! This getter is also reached through a long-branch veneer, ported
//! alongside it:
//!
...
//! the twenty-eight constructors is — they are large C++ constructors — so
...
//!   wires an embedded timer at +0x44 armed to 2000 ms, which its method
//!   cluster (0x0812f754..0x08130134, the getter's own neighbours)
//!   drives. Class NOT named.
//! - The 0xBC object is the family's **event-listener controller** —
//!   the only member whose constructor subscribes the object to the
//!   process-wide event hub: it builds two embedded map heads (a
//!   descriptor node at +0x14 through the unported FUN_083c00dc and a
//!   byte-keyed-map header node at +0x58 through the ported
//!   byte_key_tree_allocate_node @ 0x083b7f40, both self-pointing
//!   circular sentinels), two mutexes (the ported cxx_mutex_construct
//!   @ 0x08261e28 at +0x20 and +0xa0), two embedded PathObjects
//!   (+0x64/+0x6c, the ported path_object_default_construct @
//!   0x082792cc) and two embedded StringObjects (+0x7c/+0x94, the
//!   ported string_default_construct @ 0x08277440), sets byte +0x3c =
//!   2, plants primary vtable **0x089a7604** at +0 over the shared
//!   interface base 0x089a75c8 (the same first store settings_ctor
//!   makes) and secondary vtable **0x089a76e0** at +0x40, then hands
//!   `this + 0x40` to the event hub's kind-1 listener list through
//!   event_hub_instance_get @ 0x082579a0 and the subscribe push_back
//!   @ 0x08257aec (see app/event_hub.rs). The 22 call sites dispatch
//!   the object virtually (slots +0x20..+0x6c): fourteen inside the
//!   registry-class-0x8780 method cluster 0x081a3xxx-0x081a5xxx, four
//!   in the command handler FUN_081c9228 (command ids
//!   0x10001-0x10005 select slots +0x20/+0x28/+0x30/+0x34), one in the
...
/// Allocation size of the unidentified 0x28 singleton (`mov r0, #0x28`).
pub const SINGLETON_0X28_SIZE: usize = 0x28;

/// Allocation size of the unidentified 0x4c singleton (`mov r0, #0x4c`).
pub const SINGLETON_0X4C_SIZE: usize = 0x4c;
///
/// Allocation size of the unidentified 0xd0 singleton (`mov r0, #0xd0`).
pub const SINGLETON_0XD0_SIZE: usize = 0xd0;
///
/// Allocation size of the unidentified 0x80 singleton (`mov r0, #0x80`).
pub const SINGLETON_0X80_SIZE: usize = 0x80;
...
/// Allocation size of the unidentified 0xbc event-listener singleton
/// (`mov r0, #0xbc`).
...
/// Allocation size of the registry-class-0x8e80 singleton (`mov r0, #0x48`).
pub const CLASS_8E80_SIZE: usize = 0x48;
...
/// Indirect dispatch table for the twenty-six unported constructors (see the
...
pub struct SingletonCtors {
    ...
    pub singleton_0x28: Constructor,
    /// The 0x4c object's ctor @ 0x08258eb0.
    pub singleton_0x4c: Constructor,
    /// The 0xd0 object's ctor @ 0x0825af98.
    pub singleton_0xd0: Constructor,
    /// The 0x80 object's ctor @ 0x082590e0.
    pub singleton_0x80: Constructor,
    ...
    /// Registry-class-0x8e80 ctor @ 0x0820416c.
    pub class_8e80: Constructor,
...
zeroing_ctor!(zeroing_singleton_0x4c_ctor, SINGLETON_0X4C_SIZE);
zeroing_ctor!(zeroing_singleton_0xd0_ctor, SINGLETON_0XD0_SIZE);
zeroing_ctor!(zeroing_singleton_0x80_ctor, SINGLETON_0X80_SIZE);
...
zeroing_ctor!(zeroing_class_8e80_ctor, CLASS_8E80_SIZE);
...
pub(crate) const DEFAULT_SINGLETON_CTORS: SingletonCtors = SingletonCtors {
    ...
    singleton_0x4c: zeroing_singleton_0x4c_ctor,
    singleton_0xd0: zeroing_singleton_0xd0_ctor,
    ...
    class_8e80: zeroing_class_8e80_ctor,
...
/// The unidentified 0x28 singleton (original: the word @ 0x089cc7c8,
/// the pool literal @ 0x081ded40).
pub static mut SINGLETON_0X28: *mut u8 = core::ptr::null_mut();

/// The unidentified 0x4c singleton (original: the word @ 0x089cc950,
/// reached through the pool literal @ 0x08259590).
pub static mut SINGLETON_0X4C: *mut u8 = core::ptr::null_mut();
/// The unidentified 0xd0 singleton (original cache word @ 0x089d03ac, pool
/// literal @ 0x08257f30).
pub static mut SINGLETON_0XD0: *mut u8 = core::ptr::null_mut();
/// The unidentified 0x80 singleton (original cache word @ 0x089d00b8,
/// pool literal @ 0x08258bcc).
pub static mut SINGLETON_0X80: *mut u8 = core::ptr::null_mut();
...
/// The unidentified 0xbc event-listener singleton (original: the word
...
/// The registry-class-0x8e80 singleton (original cache word @ 0x089cc388,
/// addressed through the pool word @ 0x08204118).
pub static mut CLASS_8E80_INSTANCE: *mut u8 = core::ptr::null_mut();
...
/// extent; **26 direct `bl` call sites, all unconditional — 0 predicated
/// `bl` forms**, verified by decoding every B/BL word in `osos.dec`).
///
/// Algorithm: load the cache word @ 0x089cc7c8; on NULL, allocate exactly
/// 0x28 bytes with `operator_new`, call constructor `FUN_081df284`, store
/// its return, then reload and return the cache. Raw bytes show the next
/// function begins at 0x081ded44, after this function's literal pool;
/// Ghidra's 44-byte extent drops that word.
///
/// The object owns a queued-work subobject at +0xc and callers enqueue
/// tagged 12-byte work records before starting its worker, but neither its
/// constructor nor its vtable yields a class name. The symbol therefore
/// names only the verified lazy-singleton shape and allocation extent.
/// Deviation: the unported constructor uses the `singleton_0x28` dispatch
/// slot, with the family zeroing default and crate cache rather than the
/// runtime-initialized 0x089cc7c8 word. It is not hook-ready until
/// `FUN_081df284` is ported.
#[inline(never)]
#[cfg_attr(target_os = "none", no_mangle)]
pub unsafe extern "C" fn lazy_singleton_0x28() -> *mut u8 {
    let cache = core::ptr::addr_of_mut!(SINGLETON_0X28);
    lazy_singleton(cache, SINGLETON_0X28_SIZE, || unsafe { ctor!(singleton_0x28) })
}
...
/// lazy_singleton_0x4c — original: `FUN_08259564` @ **0x08259564**
/// (**44 code bytes** plus its pool word @ 0x08259590 = **48 bytes true
/// extent**; **7 direct `bl` call sites, all unconditional — 0 predicated
/// forms and 0 plain `b`**, verified by decoding every ARM B/BL word in
/// `work/firmware/osos.dec`).
///
/// Algorithm: load the cache word @ 0x089cc950; if NULL, allocate exactly
/// 0x4c bytes through `operator_new`, run constructor `FUN_08258eb0`, store
/// its return, reload the cache, and return it. Raw bytes put the next
/// separately linked function at 0x08259594, after the literal-pool word
/// Ghidra omits.

///
/// The constructor and callers recover neither a class name nor a registry
/// id, so the symbol records only the verified lazy-singleton shape and
/// allocation extent. Deliberate deviations: the unported constructor uses
/// the [`SINGLETON_CTORS`] `singleton_0x4c` dispatch slot with the family's
/// zeroing default, and the cache is crate static [`SINGLETON_0X4C`] rather
/// than the runtime-initialized firmware word. It is not hook-ready until
/// `FUN_08258eb0` is ported.
#[inline(never)]
#[cfg_attr(target_os = "none", no_mangle)]
pub unsafe extern "C" fn lazy_singleton_0x4c() -> *mut u8 {
    let cache = core::ptr::addr_of_mut!(SINGLETON_0X4C);
    lazy_singleton(cache, SINGLETON_0X4C_SIZE, || unsafe { ctor!(singleton_0x4c) })
}
/// lazy_singleton_0xd0 — original: `FUN_08257f04` @ **0x08257f04**
/// (**44 code bytes** plus its pool word @ 0x08257f30 = **48 bytes true
/// extent**; **7 direct `bl` call sites, all unconditional — 0 predicated
/// forms and 0 plain `b`**, verified by decoding every ARM B/BL word in
/// `work/firmware/osos.dec`).
///
/// ```text
/// 08257f04  push {r4, lr}
/// 08257f08  ldr  r4, [pc, #32]      @ = 0x089d03ac (pool @ 0x08257f30)
/// 08257f0c  ldr  r0, [r4]
/// 08257f10  cmp  r0, #0
/// 08257f14  bne  0x08257f28
/// 08257f18  mov  r0, #0xd0
/// 08257f1c  bl   0x082aadd4         @ operator new
/// 08257f20  bl   0x0825af98         @ constructor
/// 08257f24  str  r0, [r4]
/// 08257f28  ldr  r0, [r4]           @ reload the cache before returning
/// 08257f2c  pop  {r4, pc}
/// 08257f30  .word 0x089d03ac
/// ```
///
/// Algorithm: load the cache word @ 0x089d03ac; on NULL allocate exactly
/// 0xd0 bytes through `operator_new`, construct it with `FUN_0825af98`, store
/// the constructor return, reload the slot, and return it. The next separately
/// linked function begins at 0x08257f34. Six calls occur in the
/// `0x0818e698` component-message handler, whose returned object is dispatched
/// through virtual slots +0x30, +0x4c, and +0x28; the seventh is the
/// boot-time non-NULL checker @ 0x08258070. Neither that evidence nor the
/// constructor recovers a class name or registry id, so the symbol records
/// only the verified singleton shape and allocation extent.
///
/// Deliberate deviations: the unported constructor uses the
/// [`SINGLETON_CTORS`] `singleton_0xd0` dispatch slot with the family's
/// zeroing default, and [`SINGLETON_0XD0`] replaces the runtime-initialized
/// firmware word. It is not hook-ready until `FUN_0825af98` is ported.
#[inline(never)]
#[cfg_attr(target_os = "none", no_mangle)]
pub unsafe extern "C" fn lazy_singleton_0xd0() -> *mut u8 {
    let cache = core::ptr::addr_of_mut!(SINGLETON_0XD0);
    lazy_singleton(cache, SINGLETON_0XD0_SIZE, || unsafe { ctor!(singleton_0xd0) })
}


/// lazy_singleton_0x80 — original: `FUN_08258ba0` @ **0x08258ba0**
/// (**44 code bytes** plus its pool word @ 0x08258bcc = **48 bytes true
/// extent**; **7 direct `bl` call sites, all unconditional — 0 predicated
/// forms and 0 plain `b`**, verified by decoding every ARM B/BL word in
/// `work/firmware/osos.dec`).
///
/// Algorithm: load the cache word @ 0x089d00b8; if NULL, allocate exactly
/// 0x80 bytes through `operator_new`, run constructor `FUN_082590e0`, store
/// its return, reload the cache, and return it. Raw bytes put the next
/// separately linked function at 0x08258bd0, after the pool word Ghidra
/// omits. Its callers use the returned opaque object through vtable slots
/// +0x20 and +0x24, but neither they nor the constructor provide a class
/// name, so the symbol records the verified lazy-singleton shape and
/// allocation extent.
///
/// Deliberate deviations: the unported constructor uses the
/// [`SINGLETON_CTORS`] `singleton_0x80` dispatch slot with the family's
/// zeroing default, and the cache is crate static [`SINGLETON_0X80`] rather
/// than the runtime-initialized firmware word. It is not hook-ready until
/// `FUN_082590e0` is ported.
#[inline(never)]
#[cfg_attr(target_os = "none", no_mangle)]
pub unsafe extern "C" fn lazy_singleton_0x80() -> *mut u8 {
    let cache = core::ptr::addr_of_mut!(SINGLETON_0X80);
    lazy_singleton(cache, SINGLETON_0X80_SIZE, || unsafe { ctor!(singleton_0x80) })
}
...
///   other seventeen getters, which cache a NULL ctor result and
...
/// ```
///
/// The seventeenth member of this family: test the `+4` slot of the
...
/// lazy_singleton_0xbc — original: `FUN_08259534` @ **0x08259534**
/// (44 bytes of code + one pool word @ 0x08259560 = **48 bytes** of
...
/// ```text
/// 08259534  push {r4, lr}
/// 08259538  ldr  r4, [pc, #32]      @ = 0x089cc960 (pool @ 0x08259560)
/// 0825953c  ldr  r0, [r4]           @ the cache word itself, no +N slot
/// 08259540  cmp  r0, #0
/// 08259544  bne  0x08259558
/// 08259548  mov  r0, #0xbc
/// 0825954c  bl   0x082aadd4         @ operator new
/// 08259550  bl   0x08258a70         @ constructor (Ghidra drops its `this`)
/// 08259554  str  r0, [r4]
/// 08259558  ldr  r0, [r4]           @ reload the slot before returning
/// 0825955c  pop  {r4, pc}
/// 08259560  .word 0x089cc960
/// ```
///
/// The eighteenth member of this family: test the cache word @
/// 0x089cc960, and when it is NULL allocate exactly 0xbc bytes with the
/// ported `operator_new`, run the constructor @ 0x08258a70 over the raw
/// block, store the ctor's return into the cache, RELOAD the slot and
/// return it (so a self-caching ctor wins, and a NULL-returning ctor
/// leaves no failure memory — the next call re-allocates). Ghidra's 44
/// bytes is the code only; the true extent runs to 0x08259564, where
/// the sibling 0x4c-byte getter opens `push {r4, lr}` (the boot-time
/// singleton checker FUN_08258028 calls both).
///
/// What the object is: an event-listener controller — its constructor
/// builds two embedded map heads, two mutexes, two PathObjects and two
/// StringObjects, plants primary vtable **0x089a7604** at +0 and
/// secondary vtable **0x089a76e0** at +0x40, and subscribes `this +
/// 0x40` to the event hub's kind-1 listener list (see the module
/// header). The 22 call sites dispatch it virtually: fourteen in the
/// registry-class-0x8780 method cluster 0x081a3xxx-0x081a5xxx, four in
/// the command handler FUN_081c9228 (ids 0x10001-0x10005), one in the
/// boot checker. **Class NOT named**: the ctor reaches neither a name
/// factory nor the by-id registry, so — like [`lazy_singleton_0x3c`] &
/// co. — the symbol says only the size.
///
/// Deviations: the ctor rides the [`SINGLETON_CTORS`] dispatch table
/// (`singleton_0xbc` slot, documented zeroing default — NOT HOOK-READY,
/// the family contract) and the cache is the crate static
/// [`SINGLETON_0XBC`] rather than the word @ 0x089cc960 (the
/// block_mgr.rs RW-page deviation). Same NOT-HOOK-READY caveat as every
/// sibling — see the module header.
#[inline(never)]
#[cfg_attr(target_os = "none", no_mangle)]
pub unsafe extern "C" fn lazy_singleton_0xbc() -> *mut u8 {
    let cache = core::ptr::addr_of_mut!(SINGLETON_0XBC);
...
/// ```text
/// 0825a6e8  push {r4, lr}
/// 0825a6ec  ldr  r4, [pc, #32]      @ = 0x08a09f64 (pool @ 0x0825a714)
/// 0825a6f0  ldr  r0, [r4]
/// 0825a6f4  cmp  r0, #0
/// 0825a6f8  bne  0x0825a70c
/// 0825a6fc  mov  r0, #0x44
/// 0825a700  bl   0x082aadd4         @ operator new
/// 0825a704  bl   0x0825aa58         @ constructor
/// 0825a708  str  r0, [r4]
/// 0825a70c  ldr  r0, [r4]           @ reload the slot before returning
/// 0825a710  pop  {r4, pc}
/// 0825a714  .word 0x08a09f64
/// ```
///
/// Tests the cache word @ 0x08a09f64. On NULL, allocates exactly 0x44 bytes
/// through `operator_new`, constructs with `FUN_0825aa58`, stores the
/// constructor's return, reloads, and returns the cache. The raw image puts
/// the next entry at 0x0825a718 after the pool word; Ghidra omits both that
/// entry and this getter's pool word.
///
/// The constructor installs the shared interface vtable 0x089a75c8, builds
/// a map head at +0x04, a mutex at +0x20, and subscribes its +0x40 interface
/// through `event_hub_instance_get` / `event_hub_subscribe`. The fourteen
/// call sites dispatch the returned object through vtable slots +0x20..+0x40,
/// but recover no class name or registry id. The symbol therefore identifies
/// the verified lazy singleton by allocation extent rather than inventing an
/// identity.
///
/// Deviation: the unported constructor uses the [`SINGLETON_CTORS`]
/// `singleton_0x44` slot and its zeroing default; the cache is the crate
/// static [`SINGLETON_0X44`] rather than the runtime-initialized word
/// @ 0x08a09f64. It is not hook-ready until `FUN_0825aa58` is ported.
#[inline(never)]
#[cfg_attr(target_os = "none", no_mangle)]
pub unsafe extern "C" fn lazy_singleton_0x44() -> *mut u8 {
    let cache = core::ptr::addr_of_mut!(SINGLETON_0X44);
    lazy_singleton(cache, SINGLETON_0X44_SIZE, || unsafe { ctor!(singleton_0x44) })
}
...
/// singleton_class_8e80 — original: `FUN_082040ec` @ **0x082040ec**
/// (**44 code bytes** plus its pool word @ 0x08204118, **48 bytes true
/// extent**; **10 direct `bl` call sites, all unconditional — 0 predicated,
/// 0 plain `b`**, verified by decoding every ARM B/BL word in `osos.dec`).
///
/// Tests the cache word @ 0x089cc388. If it is NULL, allocates exactly 0x48
/// bytes through `operator_new`, constructs the registry-class-0x8e80 object
/// through `FUN_0820416c`, stores that return, reloads the cache, and returns
/// it. The constructor publishes the object under class id 0x8e80, so the
/// symbol uses that verified identity rather than inventing a class name.
///
/// Deliberate deviations: the unported constructor uses the
/// [`SINGLETON_CTORS`] `class_8e80` slot with the family's zeroing default,
/// and the cache is crate static [`CLASS_8E80_INSTANCE`] rather than the
/// runtime-initialized firmware word. The getter is not hook-ready until
/// `FUN_0820416c` is ported.
#[inline(never)]
#[cfg_attr(target_os = "none", no_mangle)]
pub unsafe extern "C" fn singleton_class_8e80() -> *mut u8 {
...
mod tests {
    ...
    fn mock(ctor_result: *mut u8) -> MutexGuard<'static, ()> {
        ...
        unsafe {
            ...
            SINGLETON_CTORS = SingletonCtors {
                ...
                singleton_0x28: recording_ctor,
                singleton_0x80: recording_ctor,
    ...
    unsafe fn clear_caches() {
        ...
        SINGLETON_0X28 = ptr::null_mut();
        SINGLETON_0X4C = ptr::null_mut();
        SINGLETON_0XD0 = ptr::null_mut();
        SINGLETON_0X80 = ptr::null_mut();
        ...
        CLASS_8E80_INSTANCE = ptr::null_mut();
    ...
    #[test]
    fn the_0x28_singleton_allocates_constructs_and_reloads_its_cache() {
        let guard = mock(constructed());
        unsafe {
            assert_eq!(lazy_singleton_0x28(), constructed());
            assert_eq!(lazy_singleton_0x28(), constructed());
            assert_eq!(
                *ptr::addr_of!(ALLOC_SIZES),
                std::vec![SINGLETON_0X28_SIZE],
                "the raw `mov r0, #0x28` allocation happens once"
            );
            assert_eq!(*ptr::addr_of!(CTOR_BLOCKS), std::vec![arena()], "the allocation feeds the constructor");
            assert_eq!(
                ptr::read_volatile(ptr::addr_of!(SINGLETON_0X28)),
                constructed(),
                "the constructor result is stored then reloaded"
            );
        }
        restore(guard);
    }
    ...
    #[test]
    fn a_null_returning_0x28_ctor_caches_null_and_reallocates() {
        let guard = mock(ptr::null_mut());
        unsafe {
            assert!(lazy_singleton_0x28().is_null());
            assert!(lazy_singleton_0x28().is_null());
            assert_eq!(
                *ptr::addr_of!(ALLOC_SIZES),
                std::vec![SINGLETON_0X28_SIZE, SINGLETON_0X28_SIZE],
                "NULL is cached, so each subsequent call repeats allocation and construction"
            );
        }
        restore(guard);
    }
    ...
    #[test]
    fn the_0xd0_singleton_allocates_constructs_and_caches() {
        let guard = mock(constructed());
        unsafe {
            assert_eq!(lazy_singleton_0xd0(), constructed());
            assert_eq!(lazy_singleton_0xd0(), constructed());
            assert_eq!(
                *ptr::addr_of!(ALLOC_SIZES),
                std::vec![SINGLETON_0XD0_SIZE],
                "the raw `mov r0, #0xd0` allocation occurs only on the cache miss"
            );
            assert_eq!(
                *ptr::addr_of!(CTOR_BLOCKS),
                std::vec![arena()],
            ...
            assert_eq!(
                ptr::read_volatile(ptr::addr_of!(SINGLETON_0XD0)),
                constructed(),
                "the constructor result is stored and reloaded"
            );
        }
        restore(guard);
    }
    ...
    #[test]
    fn a_pre_seeded_0xd0_cache_skips_all_cold_path_calls() {
        let guard = mock(constructed());
        unsafe {
            SINGLETON_0XD0 = arena().add(64);
            assert_eq!(lazy_singleton_0xd0(), arena().add(64));
            assert!((*ptr::addr_of!(ALLOC_SIZES)).is_empty());
            assert!((*ptr::addr_of!(CTOR_BLOCKS)).is_empty());
        }
        restore(guard);
    ...
    #[test]
    fn a_null_returning_0xd0_ctor_retries_on_every_call() {
        let guard = mock(ptr::null_mut());
        unsafe {
            assert!(lazy_singleton_0xd0().is_null());
            assert!(ptr::read_volatile(ptr::addr_of!(SINGLETON_0XD0)).is_null());
            assert!(lazy_singleton_0xd0().is_null());
            assert_eq!(
                *ptr::addr_of!(ALLOC_SIZES),
                std::vec![SINGLETON_0XD0_SIZE, SINGLETON_0XD0_SIZE],
                "a NULL constructor result remains a cache miss"
            );
        }
        restore(guard);
    }
    ...
    #[test]
    fn the_0xd0_constructor_return_overwrites_its_cache_side_effect() {
        unsafe extern "C" fn cache_poking_ctor(this: *mut u8) -> *mut u8 {
            SINGLETON_0XD0 = this.add(32);
            this.add(48)
        }

        let guard = mock(constructed());
        unsafe {
            SINGLETON_CTORS.singleton_0xd0 = cache_poking_ctor;
            assert_eq!(lazy_singleton_0xd0(), arena().add(48));
            assert_eq!(ptr::read_volatile(ptr::addr_of!(SINGLETON_0XD0)), arena().add(48));
        }
        restore(guard);
    }

    #[test]
    fn the_0xd0_zeroing_stub_clears_exactly_its_object() {
        let guard = SINGLETON_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        unsafe {
            let block = ptr::addr_of_mut!(ARENA) as *mut u8;
            for offset in 0..SINGLETON_0XD0_SIZE + 4 {
                block.add(offset).write(0xa5);
            }
            assert_eq!(zeroing_singleton_0xd0_ctor(block), block);
            assert!((0..SINGLETON_0XD0_SIZE).all(|offset| block.add(offset).read() == 0));
            assert_eq!(block.add(SINGLETON_0XD0_SIZE).read(), 0xa5, "not one byte past the object");
        }
        restore(guard);
    }
    ...
    #[test]
    fn the_0xbc_singleton_is_allocated_at_exactly_0xbc_and_constructed_once() {
        let guard = mock(constructed());
        unsafe {
            assert_eq!(lazy_singleton_0xbc(), constructed());
            assert_eq!(
                *ptr::addr_of!(ALLOC_SIZES),
                std::vec![0xbc],
                "the `mov r0, #0xbc` immediate"
            );
            assert_eq!(*ptr::addr_of!(CTOR_BLOCKS), std::vec![arena()], "constructed on the raw block");
            assert_eq!(
                ptr::read_volatile(ptr::addr_of!(SINGLETON_0XBC)),
                constructed(),
                "the ctor result is cached in the word @ 0x089cc960"
            );
        }
        restore(guard);
    }
    ...
    #[test]
    fn the_0xbc_singleton_second_call_returns_the_cache() {
        // The `cmp r0, #0; bne` fast path: 21 of the 22 call sites run
        // after the boot checker's first construction and never touch
        // the allocator.
        let guard = mock(constructed());
        unsafe {
            assert_eq!(lazy_singleton_0xbc(), constructed());
    ...
    #[test]
    fn a_pre_seeded_0xbc_cache_never_allocates() {
        let guard = mock(constructed());
        unsafe {
            SINGLETON_0XBC = arena().add(48);
            assert_eq!(lazy_singleton_0xbc(), arena().add(48));
            assert!((*ptr::addr_of!(ALLOC_SIZES)).is_empty());
            assert!((*ptr::addr_of!(CTOR_BLOCKS)).is_empty());
        }
        restore(guard);
    ...
    #[test]
    fn a_null_returning_0xbc_ctor_caches_null_and_retries_on_every_call() {
        let guard = mock(ptr::null_mut());
        unsafe {
            assert!(lazy_singleton_0xbc().is_null());
            assert!(ptr::read_volatile(ptr::addr_of!(SINGLETON_0XBC)).is_null());
            assert!(lazy_singleton_0xbc().is_null());
            assert_eq!(
                *ptr::addr_of!(ALLOC_SIZES),
                std::vec![SINGLETON_0XBC_SIZE, SINGLETON_0XBC_SIZE],
                "no failure memory: it re-allocates every call"
            );
        }
        restore(guard);
    ...
    #[test]
    fn the_0xbc_store_lands_after_its_ctor_runs() {
        // Order is alloc -> ctor -> store: a ctor that pokes the cache
        // slot itself is overwritten by the store of the ctor's return
        // (the original's `str r0, [r4]` follows the `bl 0x08258a70`),
        // and the reload then returns that stored value.
        unsafe extern "C" fn cache_poking_ctor(this: *mut u8) -> *mut u8 {
            (*ptr::addr_of_mut!(CTOR_BLOCKS)).push(this);
            SINGLETON_0XBC = this.add(32);
            this.add(48)
        }
        let guard = mock(ptr::null_mut());
        unsafe {
            SINGLETON_CTORS.singleton_0xbc = cache_poking_ctor;
            assert_eq!(lazy_singleton_0xbc(), arena().add(48), "the ctor-return store wins");
            assert_eq!(ptr::read_volatile(ptr::addr_of!(SINGLETON_0XBC)), arena().add(48));
            assert_eq!(*ptr::addr_of!(CTOR_BLOCKS), std::vec![arena()], "the ctor still ran, once");
        }
        restore(guard);
    }
    ...
    #[test]
    fn the_0xbc_zeroing_stub_clears_exactly_0xbc_bytes() {
        let guard = SINGLETON_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        unsafe {
            let block = ptr::addr_of_mut!(ARENA) as *mut u8;
            for offset in 0..SINGLETON_0XBC_SIZE + 4 {
                block.add(offset).write(0xa5);
            }
            assert_eq!(zeroing_singleton_0xbc_ctor(block), block);
            assert!((0..SINGLETON_0XBC_SIZE).all(|offset| block.add(offset).read() == 0));
            assert_eq!(
                block.add(SINGLETON_0XBC_SIZE).read(),
                0xa5,
                "not one byte past the object"
            );
            assert!(zeroing_singleton_0xbc_ctor(ptr::null_mut()).is_null(), "NULL-safe");
        }
        restore(guard);
    }

    #[test]
    fn the_0xbc_cache_is_independent_of_its_globals_page_neighbours() {
        // The cache word 0x089cc960 sits in the same 0x089cc9xx globals
        // page as the settings store (0x089cc948) and the 0x40
        // singleton (0x089cc94c); the crate statics keep them apart.
        let guard = mock(constructed());
        unsafe {
            lazy_singleton_0xbc();
            assert!(ptr::read_volatile(ptr::addr_of!(SINGLETON_0X40)).is_null(), "untouched");
    ...
    #[test]
    fn the_0x4c_singleton_allocates_constructs_and_caches() {
        let guard = mock(constructed());
        unsafe {
            assert_eq!(lazy_singleton_0x4c(), constructed());
            assert_eq!(lazy_singleton_0x4c(), constructed());
            assert_eq!(*ptr::addr_of!(ALLOC_SIZES), std::vec![SINGLETON_0X4C_SIZE]);
            assert_eq!(*ptr::addr_of!(CTOR_BLOCKS), std::vec![arena()]);
            assert_eq!(
                ptr::read_volatile(ptr::addr_of!(SINGLETON_0X4C)),
                constructed(),
                "the ctor result is cached in the word @ 0x089cc950"
            );
        }
        restore(guard);
    }
    ...
    #[test]
    fn a_pre_seeded_0x4c_cache_never_allocates() {
        let guard = mock(constructed());
        unsafe {
            SINGLETON_0X4C = arena().add(32);
            assert_eq!(lazy_singleton_0x4c(), arena().add(32));
            assert!((*ptr::addr_of!(ALLOC_SIZES)).is_empty());
            assert!((*ptr::addr_of!(CTOR_BLOCKS)).is_empty());
        }
        restore(guard);
    ...
    #[test]
    fn a_null_returning_0x4c_ctor_is_retried() {
        let guard = mock(ptr::null_mut());
        unsafe {
            assert!(lazy_singleton_0x4c().is_null());
            assert!(lazy_singleton_0x4c().is_null());
            assert_eq!(
                *ptr::addr_of!(ALLOC_SIZES),
                std::vec![SINGLETON_0X4C_SIZE, SINGLETON_0X4C_SIZE],
                "the NULL cache is not failure memory"
            );
        }
        restore(guard);
    }
    ...
    #[test]
    fn the_0x4c_store_lands_after_its_ctor_runs() {
        unsafe extern "C" fn cache_poking_ctor(this: *mut u8) -> *mut u8 {
            (*ptr::addr_of_mut!(CTOR_BLOCKS)).push(this);
            SINGLETON_0X4C = this.add(16);
            this.add(48)
        }
        let guard = mock(ptr::null_mut());
        unsafe {
            SINGLETON_CTORS.singleton_0x4c = cache_poking_ctor;
            assert_eq!(lazy_singleton_0x4c(), arena().add(48));
            assert_eq!(ptr::read_volatile(ptr::addr_of!(SINGLETON_0X4C)), arena().add(48));
            assert_eq!(*ptr::addr_of!(CTOR_BLOCKS), std::vec![arena()]);
        }
        restore(guard);
    }
    ...
    #[test]
    fn the_0x80_singleton_allocates_constructs_and_caches() {
        let guard = mock(constructed());
        unsafe {
            assert_eq!(lazy_singleton_0x80(), constructed());
            assert_eq!(lazy_singleton_0x80(), constructed());
            assert_eq!(*ptr::addr_of!(ALLOC_SIZES), std::vec![SINGLETON_0X80_SIZE]);
            assert_eq!(*ptr::addr_of!(CTOR_BLOCKS), std::vec![arena()]);
            assert_eq!(
                ptr::read_volatile(ptr::addr_of!(SINGLETON_0X80)),
                constructed(),
                "the ctor result is cached in the word @ 0x089d00b8"
            );
        }
        restore(guard);
    }
    ...
    #[test]
    fn a_pre_seeded_0x80_cache_never_allocates() {
        let guard = mock(constructed());
        unsafe {
            SINGLETON_0X80 = arena().add(32);
            assert_eq!(lazy_singleton_0x80(), arena().add(32));
            assert!((*ptr::addr_of!(ALLOC_SIZES)).is_empty());
            assert!((*ptr::addr_of!(CTOR_BLOCKS)).is_empty());
        }
        restore(guard);
    ...
    #[test]
    fn a_null_returning_0x80_ctor_is_retried() {
        let guard = mock(ptr::null_mut());
        unsafe {
            assert!(lazy_singleton_0x80().is_null());
            assert!(lazy_singleton_0x80().is_null());
            assert_eq!(
                *ptr::addr_of!(ALLOC_SIZES),
                std::vec![SINGLETON_0X80_SIZE, SINGLETON_0X80_SIZE],
                "the NULL cache is not failure memory"
            );
        }
        restore(guard);
    ...
    #[test]
    fn the_0x80_store_lands_after_its_ctor_runs() {
        unsafe extern "C" fn cache_poking_ctor(this: *mut u8) -> *mut u8 {
            (*ptr::addr_of_mut!(CTOR_BLOCKS)).push(this);
            SINGLETON_0X80 = this.add(16);
            this.add(48)
        }
        let guard = mock(ptr::null_mut());
        unsafe {
            SINGLETON_CTORS.singleton_0x80 = cache_poking_ctor;
            assert_eq!(lazy_singleton_0x80(), arena().add(48), "the ctor-return store wins");
            assert_eq!(ptr::read_volatile(ptr::addr_of!(SINGLETON_0X80)), arena().add(48));
            assert_eq!(*ptr::addr_of!(CTOR_BLOCKS), std::vec![arena()]);
        }
        restore(guard);
    }
    ...
    #[test]
    fn the_0x44_singleton_allocates_constructs_and_caches() {
        let guard = mock(constructed());
        unsafe {
            assert_eq!(lazy_singleton_0x44(), constructed());
            assert_eq!(*ptr::addr_of!(ALLOC_SIZES), std::vec![SINGLETON_0X44_SIZE]);
            assert_eq!(*ptr::addr_of!(CTOR_BLOCKS), std::vec![arena()]);
            assert_eq!(ptr::read_volatile(ptr::addr_of!(SINGLETON_0X44)), constructed());
        }
        restore(guard);
    }

    #[test]
    fn the_0x44_singleton_reloads_its_cached_constructor_result() {
        let guard = mock(constructed());
        unsafe {
            assert_eq!(lazy_singleton_0x44(), constructed());
            assert_eq!(lazy_singleton_0x44(), constructed());
            assert_eq!((*ptr::addr_of!(ALLOC_SIZES)).len(), 1);
    ...
    #[test]
    fn a_null_returning_0x44_constructor_reallocates() {
        let guard = mock(ptr::null_mut());
        unsafe {
            assert!(lazy_singleton_0x44().is_null());
            assert!(lazy_singleton_0x44().is_null());
            assert_eq!((*ptr::addr_of!(ALLOC_SIZES)).len(), 2);
            assert_eq!((*ptr::addr_of!(CTOR_BLOCKS)).len(), 2);
        }
    ...
    #[test]
    fn the_0x44_zeroing_default_respects_object_extent() {
        let guard = SINGLETON_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        unsafe {
            let block = ptr::addr_of_mut!(ARENA) as *mut u8;
            for offset in 0..SINGLETON_0X44_SIZE + 4 {
                block.add(offset).write(0xa5);
            }
            assert_eq!(zeroing_singleton_0x44_ctor(block), block);
            assert!((0..SINGLETON_0X44_SIZE).all(|offset| block.add(offset).read() == 0));
            assert_eq!(block.add(SINGLETON_0X44_SIZE).read(), 0xa5);
            assert!(zeroing_singleton_0x44_ctor(ptr::null_mut()).is_null());
        }
        restore(guard);
    }
    #[test]
    fn class_8e80_allocates_constructs_and_reloads_the_cache() {
        let guard = mock(constructed());
        unsafe {
            assert_eq!(singleton_class_8e80(), constructed());
            assert_eq!(singleton_class_8e80(), constructed());
            assert_eq!(*ptr::addr_of!(ALLOC_SIZES), std::vec![CLASS_8E80_SIZE]);
            assert_eq!(*ptr::addr_of!(CTOR_BLOCKS), std::vec![arena()]);
            assert_eq!(
                ptr::read_volatile(ptr::addr_of!(CLASS_8E80_INSTANCE)),
                constructed(),
                "the constructor return is stored and the cache reload is returned"
            );
        }
        restore(guard);
    }
    ...
    #[test]
    fn class_8e80_preseeded_cache_skips_allocation_and_construction() {
        let guard = mock(constructed());
        unsafe {
            CLASS_8E80_INSTANCE = arena().add(32);
            assert_eq!(singleton_class_8e80(), arena().add(32));
            assert!((*ptr::addr_of!(ALLOC_SIZES)).is_empty());
            assert!((*ptr::addr_of!(CTOR_BLOCKS)).is_empty());
        }
        restore(guard);
    ...
    #[test]
    fn null_class_8e80_constructor_result_retries_from_a_null_cache() {
        let guard = mock(ptr::null_mut());
        unsafe {
            assert!(singleton_class_8e80().is_null());
            assert!(singleton_class_8e80().is_null());
            assert_eq!(
                *ptr::addr_of!(ALLOC_SIZES),
                std::vec![CLASS_8E80_SIZE, CLASS_8E80_SIZE],
                "a NULL ctor result remains a NULL cache and re-enters the cold path"
            );
            assert_eq!((*ptr::addr_of!(CTOR_BLOCKS)).len(), 2);
        }
        restore(guard);
