//! First-byte accessor for the `"prID"` resource `0x60f1`: `FUN_081eda10` @
//! 0x081eda10.
//!
//! # Raw extent and call sites
//!
//! Ghidra's 40-byte size is the ten-instruction code extent
//! (`0x081eda10..0x081eda38`). Two literal words follow — resource id
//! `0x60f1` and kind `0x70724944` (`"prID"` read big-endian) — so the true
//! extent is 48 bytes through `0x081eda40`, where `FUN_081eda40` begins.
//! Raw instruction decoding finds four direct, unconditional `bl` call sites;
//! there are no predicated `bl` callers.
//!
//! # Algorithm
//!
//! Load class-0x8900's class-0x6000 store at +0x378, look up resource
//! `(kind "prID", id 0x60f1)` through `resource_chain_find`, and return its
...
/// Resource kind literal @ 0x081eda3c: `"prID"` read big-endian.
const RESOURCE_KIND_PRID: ResourceKind = ResourceKind(0x7072_4944);
/// Resource id literal @ 0x081eda38.
const RESOURCE_ID_PRID: u32 = 0x60f1;

/// class_8900_prid_60f1_first_byte — original: `FUN_081eda10` @ 0x081eda10
/// (**40 bytes of code plus an 8-byte literal pool; 4 unconditional `bl`
/// call sites, binary-scanned**).
///
/// Looks up `(kind "prID", id 0x60f1)` in the class-0x8900 object's
...
pub unsafe extern "C" fn class_8900_prid_60f1_first_byte(this: *const Class8900) -> u32 {
...
mod tests {
    ...
    #[test]
    fn returns_first_byte_of_prid_60f1_resource() {
        ...
        for value in values.iter_mut() {
            unsafe {
                ANSWER = value;
                assert_eq!(class_8900_prid_60f1_first_byte(&owner), *value as u32);
    ...
    fn returns_zero_when_no_provider_answers() {
        ...
        unsafe {
            ANSWER = ptr::null_mut();
            assert_eq!(class_8900_prid_60f1_first_byte(&owner), 0);
            assert_eq!(OBSERVED_KIND, RESOURCE_KIND_PRID);
            assert_eq!(OBSERVED_ID, RESOURCE_ID_PRID);
        }
    }
