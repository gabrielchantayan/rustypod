//! First-byte accessor for the `"prID"` resource `0x60f3`: `FUN_081edad4` @
//! 0x081edad4.
//!
//! # Raw extent and call sites
//!
//! Ghidra's 40-byte size is the ten-instruction code extent
//! (`0x081edad4..0x081edafc`). Two literal words follow — resource id
//! `0x60f3` and kind `0x70724944` (`"prID"` read big-endian) — so the true
//! extent is 48 bytes through `0x081edb04`, where `FUN_081edb04` begins.
//! Raw instruction decoding finds five direct, unconditional `bl` call sites;
//! there are no predicated `bl` callers.
//!
//! # Algorithm
//!
//! Load class-0x8900's class-0x6000 store at +0x378, look up resource
//! `(kind "prID", id 0x60f3)` through `resource_chain_find`, and return its
//! first byte as `u32`. A missing entry returns zero; the returned resource is
...
/// Resource kind literal @ 0x081edb00: `"prID"` read big-endian.
const RESOURCE_KIND_PRID: ResourceKind = ResourceKind(0x7072_4944);
/// Resource id literal @ 0x081edafc.
const RESOURCE_ID_PRID: u32 = 0x60f3;

/// class_8900_prid_60f3_first_byte — original: `FUN_081edad4` @ 0x081edad4
/// (**40 bytes of code plus an 8-byte literal pool; 5 unconditional `bl`
/// call sites, binary-scanned**).
///
/// Looks up `(kind "prID", id 0x60f3)` in the class-0x8900 object's
/// class-0x6000 store. Returns the first byte of a found resource as `u32`, or
/// zero when the chain does not answer. As in the ARM code, `this` itself is
/// not NULL-guarded; it must point to a valid [`Class8900`].
#[inline(never)]
#[cfg_attr(target_os = "none", no_mangle)]
pub unsafe extern "C" fn class_8900_prid_60f3_first_byte(this: *const Class8900) -> u32 {
...
mod tests {
    ...
    #[test]
    fn returns_first_byte_of_prid_60f3_resource() {
        ...
        for value in values.iter_mut() {
            unsafe {
                ANSWER = value;
                assert_eq!(class_8900_prid_60f3_first_byte(&owner), *value as u32);
    ...
    fn returns_zero_when_no_provider_answers() {
        ...
        unsafe {
            ANSWER = ptr::null_mut();
            assert_eq!(class_8900_prid_60f3_first_byte(&owner), 0);
            assert_eq!(OBSERVED_KIND, RESOURCE_KIND_PRID);
            assert_eq!(OBSERVED_ID, RESOURCE_ID_PRID);
        }
    }
