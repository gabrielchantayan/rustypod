//! RetailOS port of `FUN_082a273c` at load address `0x082a273c` (88 bytes).
//!
//! Raw ARM establishes the true extent `0x082a273c..0x082a2794`; the next
//! `push {r4, r5, r6, lr}` at `0x082a2794` begins a distinct function. There
//! are four incoming plain `bl` calls and no predicated forms. The function
//! first calls vtable slot `+8`; when it permits the query, it uses the
//! object's cached resolved-object word at `+8`, or calls `0x082a2cac` with
//! the object and its `+4` successor link, then returns bit 6 from the
//! resolved object's `+0x1c` flags.
//!
//! Deliberate deviation: the two unported call targets remain target-address
...
#[inline(never)]
pub unsafe extern "C" fn object_has_resolved_flag_0x40(object: *mut u32) -> u32 {
...
mod tests {
    ...
    #[test]
    fn gates_resolution_and_extracts_only_flag_bit_6() {
        let _guard = SEAM_LOCK.lock();
        let Some(slab) = crate::testing::try_map_u32_slab(
            crate::testing::hints::OBJECT_HAS_RESOLVED_FLAG_0X40,
            0x1000,
        ) else {
        ...
        unsafe {
            ...
            cached.add(7).write(0xc0);

            PREDICATE_RESULT = 0;
            PREDICATE_CALLS = 0;
            RESOLVE_CALLS = 0;
            assert_eq!(object_has_resolved_flag_0x40(object), 0);
            assert_eq!(PREDICATE_CALLS, 1);
            assert_eq!(RESOLVE_CALLS, 0);

            PREDICATE_RESULT = 1;
            PREDICATE_CALLS = 0;
            assert_eq!(object_has_resolved_flag_0x40(object), 1);
            assert_eq!(PREDICATE_CALLS, 1);
            assert_eq!(RESOLVE_CALLS, 0);

            object.add(2).write(0);
            RESOLVED = successor;
            successor.add(7).write(0x800);
            RESOLVE_CALLS = 0;
            assert_eq!(object_has_resolved_flag_0x40(object), 0);
