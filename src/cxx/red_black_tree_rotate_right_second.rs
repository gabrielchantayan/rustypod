//! Generic C++ red-black-tree right rotation — original: `FUN_083cb898` at
//! load address `0x083cb898`.
//!
//! Raw `osos.dec` establishes the exact 84-byte extent: 21 ARM words from
//! `ldr r2,[r1,#8]` at `0x083cb898` through `bx lr` at `0x083cb8e8`; the
//! separately linked next function begins at `0x083cb8ec`. Decoding every
//! aligned ARM B/BL-immediate word in `osos.dec` finds five inbound direct
//! calls: four unconditional `bl` at `0x083cbbe4`, `0x083cbc5c`, `0x083cbd48`,
//! and `0x083cc120`, plus a predicated `bleq` at `0x083cc170`. The predicated
...
///
/// Original: `FUN_083cb898` at load address `0x083cb898` (84 bytes; five
...
#[cfg_attr(target_os = "none", link_section = ".text.red_black_tree_rotate_right_second")]
#[inline(never)]
pub unsafe extern "C" fn red_black_tree_rotate_right_second(
...
mod tests {
    ...
    use super::{red_black_tree_rotate_right_second, RedBlackTree, RedBlackTreeNode};
    use core::ptr;
    use std::sync::LazyLock;

    static SLAB: LazyLock<Option<usize>> = LazyLock::new(|| {
        crate::testing::try_map_u32_slab(
            crate::testing::hints::RED_BLACK_TREE_ROTATE_RIGHT_SECOND,
            0x1000,
        )
        .map(|p| p as usize)
    });
    ...
    fn rotates_root_and_both_parent_child_slots() {
        let Some(base) = try_slab() else {
            crate::testing::note_missing_u32_fixture("red_black_tree_rotate_right_second");
            return;
        };
        ...
        unsafe {
            ...
            red_black_tree_rotate_right_second(tree, pivot);
