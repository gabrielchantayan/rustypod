...
///
/// `hash_table_bucket_slot_6e30` — original `FUN_083d6e30` @ `0x083d6e30`.
///
/// Raw osos.dec words `e92d4010 e1a04000 e1a00001 e5941008 ebf18033
/// e5940004 e0800101 e8bd8010` establish the 32-byte extent through
/// `0x083d6e50`; the next real function starts with `push {r4, lr}` there.
/// Aligned ARM decoding finds three inbound plain `bl` sites (`0x083d2750`,
/// `0x083d285c`, `0x083d2994`), no predicated inbound calls, one body plain
...
#[cfg_attr(target_os = "none", no_mangle)]
#[cfg_attr(target_os = "none", link_section = ".text.hash_table_bucket_slot_6e30")]
#[inline(never)]
pub unsafe extern "C" fn hash_table_bucket_slot_6e30(table: *const u8, hash: u32) -> *mut u32 {
    let bucket_base = ptr::read(table.add(4) as *const u32);
    let bucket_count = ptr::read(table.add(8) as *const u32);
    let mut remainder = 0;
    __rt_udivmod(hash, bucket_count, &mut remainder);
