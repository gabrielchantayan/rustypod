//! `optional_vtable_slot4_invoke_737c` — retailOS `FUN_083e737c` at load
//! address `0x083e737c`.
//!
//! ## Verified extent and calls
//!
//! Raw `osos.dec` decodes nine ARM words from `push {r4,lr}` at `0x083e737c`
//! through `pop {r4,pc}` at `0x083e739c`; `push {r4,lr}` at `0x083e73a0`
//! starts the next real function. The true size is **36 bytes**. The full
//! image has two inbound, plain unconditional `bl` sites (`0x081dd1fc` and
//! `0x081e8748`) and no predicated direct `bl` sites. The body has no direct
//! `bl` and one predicated indirect `blxne` through vtable slot `+4`.
//!
//! ## Algorithm
//!
//! Load the object pointer from `slot`. If non-NULL, call its vtable slot
//! `+4` with the object in `r0`, then return `slot` unchanged.
//!
...
//! duplicates the independently hookable sibling at `0x083e7424`.
...
#[cfg_attr(target_os = "none", link_section = ".text.optional_vtable_slot4_invoke_737c")]
#[inline(never)]
pub unsafe extern "C" fn optional_vtable_slot4_invoke_737c(slot: *mut *mut u8) -> *mut *mut u8 {
...
mod tests {
    ...
    fn null_object_returns_slot_without_vtable_access() {
        ...
        let result = unsafe { optional_vtable_slot4_invoke_737c(&mut slot) };
