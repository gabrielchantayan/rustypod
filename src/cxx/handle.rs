...
//! [`refcounted_body_release_dtor_copy`] @ 0x0839ccac,
//! [`refcounted_body_release_slot1_copy`] @ 0x0839d038, and
//! [`refcounted_body_release_dtor_variant`] @ 0x0839d3ac are further separately
...
/// 0x0839d3f8, and 0x0839d430). Decoding every ARM `B`/`BL` word in
/// `osos.dec` finds no predicated calls or direct tail `b` sites; no
/// word-aligned image word equals this address, so it is not virtually
/// dispatched. Raw instructions end with `bx lr` at 0x0839d458; the next
/// separately linked function begins at 0x0839d45c.
///
/// Loads the optional mutex from `body` at target +8 and, when non-NULL,
/// tail-branches to [`mutex_unlock`] @ 0x0807f6a0. It has no NULL guard for
/// `body`; callers must supply a readable [`RefcountedBody`].
///
/// No deliberate behavioral deviations. Structural review shows LLVM retains
/// the tail branch after a harmless frame setup; the body+8 load, NULL guard,
/// and mutex-unlock behavior are unchanged.
///
/// # Safety
///
/// `body` must be readable. When its mutex is non-NULL, it must satisfy
...
#[cfg_attr(target_os = "none", no_mangle)]
#[inline(never)]
pub unsafe extern "C" fn refcounted_body_mutex_unlock(body: *mut RefcountedBody) {
    let mutex = (*body).mutex;
    if !mutex.is_null() {
        mutex_unlock(mutex);
    }
...
/// 0x0839d30c, and 0x0839d344). Decoding every ARM `B`/`BL` word in
/// `osos.dec` finds no predicated calls or direct tail `b` sites; no
/// word-aligned image word equals this address, so it is not virtually
/// dispatched. Raw instructions end with `bx lr` at 0x0839d36c; the next
/// separately linked function begins at 0x0839d370.
///
/// Loads the optional mutex from `body` at target +8 and, when non-NULL,
/// tail-branches to [`mutex_unlock`] @ 0x0807f6a0. It has no NULL guard for
/// `body`; callers must supply a readable [`RefcountedBody`].
...
/// No deliberate behavioral deviations. A distinct target-only section keeps
/// this separately linked retail helper from folding into its byte-identical
/// sibling at 0x0839d44c.
///
/// # Safety
///
/// `body` must be readable. When its mutex is non-NULL, it must satisfy
/// [`mutex_unlock`]'s requirements.
#[cfg_attr(target_os = "none", link_section = ".text.refcounted_body_mutex_unlock_owned")]
#[cfg_attr(target_os = "none", no_mangle)]
#[inline(never)]
pub unsafe extern "C" fn refcounted_body_mutex_unlock_owned(body: *mut RefcountedBody) {
    let mutex = (*body).mutex;
    if !mutex.is_null() {
        mutex_unlock(mutex);
    }
...
/// refcounted_body_mutex_unlock_owned_variant — original: `FUN_0839cfec` @
/// load address 0x0839cfec (16 bytes; 4 direct `bl` call sites, all
/// unconditional: 0x0815f4d4, 0x0815f50c, 0x0839cf98, and 0x0839cfd0).
/// Decoding every ARM `B`/`BL` word in `osos.dec` finds no predicated calls
/// or direct tail `b` sites; no word-aligned image word equals this address,
/// so it is not virtually dispatched. Raw instructions:
///
/// ```text
/// 0839cfec:  ldr r0,[r0,#0x8]
/// 0839cff0:  cmp r0,#0x0
/// 0839cff4:  bne 0x0807f6a0
/// 0839cff8:  bx lr
/// ```
///
/// ending with `bx lr` at 0x0839cff8; the next separately linked function
/// begins at 0x0839cffc. This is the mutex-unlock half of the helper pair
/// (lock @ 0x0839cfdc, unlock @ 0x0839cfec) linked immediately after
/// [`refcounted_body_release_owned_variant`] @ 0x0839cf4c, which calls it
/// twice; the remaining two sites come from the 0x0815f4xx family.
///
/// Loads the optional mutex from `body` at target +8 and, when non-NULL,
/// tail-branches to [`mutex_unlock`] @ 0x0807f6a0. It has no NULL guard for
/// `body`; callers must supply a readable [`RefcountedBody`].
...
/// No deliberate behavioral deviations. A distinct target-only section keeps
/// this separately linked retail helper from folding into its byte-identical
/// siblings at 0x0839d44c and 0x0839d360.
///
/// # Safety
///
/// `body` must be readable. When its mutex is non-NULL, it must satisfy
/// [`mutex_unlock`]'s requirements.
#[cfg_attr(target_os = "none", link_section = ".text.refcounted_body_mutex_unlock_owned_variant")]
#[cfg_attr(target_os = "none", no_mangle)]
#[inline(never)]
pub unsafe extern "C" fn refcounted_body_mutex_unlock_owned_variant(body: *mut RefcountedBody) {
    let mutex = (*body).mutex;
    if !mutex.is_null() {
        mutex_unlock(mutex);
    }
...
/// refcounted_body_mutex_unlock_array_variant — original: `FUN_0839cf00` @
/// load address 0x0839cf00 (16 bytes; 4 direct `bl` call sites, all
/// unconditional: 0x08096264, 0x080962d8, 0x0839cea8, and 0x0839cee0).
/// Decoding every ARM `B`/`BL` word in `osos.dec` finds no predicated calls
/// or direct tail `b` sites; no word-aligned image word equals this address,
/// so it is not virtually dispatched. Raw instructions:
///
/// ```text
/// 0839cf00:  ldr r0,[r0,#0x8]
/// 0839cf04:  cmp r0,#0x0
/// 0839cf08:  bne 0x0807f6a0
/// 0839cf0c:  bx lr
/// ```
///
/// ending with `bx lr` at 0x0839cf0c; the next separately linked function
/// (the attach helper 0x0839cf10) begins at 0x0839cf10. This is the
/// mutex-unlock half of the helper pair (lock @ 0x0839cef0, unlock @
/// 0x0839cf00) linked immediately after the release function @ 0x0839ce48,
/// which calls it twice; the remaining two sites come from the 0x080962xx
/// family. That release disposes the implementation through
/// `cpp_finalise_dtor_guard` (0x082ab3ec) as a 4-element array of 12-byte
/// elements before tag-2 operator delete, distinguishing it from the
/// owned-variant family.
///
/// Loads the optional mutex from `body` at target +8 and, when non-NULL,
/// tail-branches to [`mutex_unlock`] @ 0x0807f6a0. It has no NULL guard for
/// `body`; callers must supply a readable [`RefcountedBody`].
...
/// No deliberate behavioral deviations. A distinct target-only section keeps
/// this separately linked retail helper from folding into its byte-identical
/// siblings at 0x0839d44c, 0x0839d360, and 0x0839cfec.
///
/// # Safety
///
/// `body` must be readable. When its mutex is non-NULL, it must satisfy
/// [`mutex_unlock`]'s requirements.
#[cfg_attr(target_os = "none", link_section = ".text.refcounted_body_mutex_unlock_array_variant")]
#[cfg_attr(target_os = "none", no_mangle)]
#[inline(never)]
pub unsafe extern "C" fn refcounted_body_mutex_unlock_array_variant(body: *mut RefcountedBody) {
    let mutex = (*body).mutex;
    if !mutex.is_null() {
        mutex_unlock(mutex);
    }
...
/// refcounted_body_mutex_unlock_dtor_copy — original: `FUN_0839d188` @
/// load address 0x0839d188 (16 bytes; 4 direct `bl` call sites, all
/// unconditional: 0x0816ce40, 0x0816ce78, 0x0839d134, and 0x0839d16c).
/// Decoding every ARM `B`/`BL` word in `osos.dec` finds no predicated
/// calls or direct tail `b` sites; no word-aligned image word equals this
/// address, so it is not virtually dispatched. Raw instructions end with
/// `bx lr` at 0x0839d194; the next separately linked function
/// ([`refcounted_body_attach_slot1`]) begins at 0x0839d198.
///
/// Loads the optional mutex from `body` at target +8 and, when non-NULL,
/// tail-branches to [`mutex_unlock`] @ 0x0807f6a0. It has no NULL guard for
/// `body`; callers must supply a readable [`RefcountedBody`]. It is the
/// separately linked unlock half paired with the lock helper @ 0x0839d178
/// inside the slot-1-virtual-destructor release template @ 0x0839d0e8 (the
/// two call sites at 0x0839d134/0x0839d16c) and its 0x0816cdec sibling
/// release (0x0816ce40/0x0816ce78).
///
/// No deliberate behavioral deviations. A distinct target-only section keeps
/// this separately linked retail helper from folding into its byte-identical
/// siblings at 0x0839d360, 0x0839d44c, and 0x0839d274.
///
/// # Safety
///
/// `body` must be readable. When its mutex is non-NULL, it must satisfy
/// [`mutex_unlock`]'s requirements.
#[cfg_attr(target_os = "none", link_section = ".text.refcounted_body_mutex_unlock_dtor_copy")]
#[cfg_attr(target_os = "none", no_mangle)]
#[inline(never)]
pub unsafe extern "C" fn refcounted_body_mutex_unlock_dtor_copy(body: *mut RefcountedBody) {
    let mutex = (*body).mutex;
    if !mutex.is_null() {
        mutex_unlock(mutex);
    }
...
/// refcounted_body_mutex_unlock_slot1_copy — original: `FUN_0839d0d8` @
/// load address 0x0839d0d8 (16 bytes; 4 direct `bl` call sites, all
/// unconditional: 0x0816cda4, 0x0816cddc, 0x0839d084, and 0x0839d0bc).
/// Decoding every ARM `B`/`BL` word in `osos.dec` finds no predicated
/// calls or direct tail `b` sites; no word-aligned image word equals this
/// address, so it is not virtually dispatched. Raw words are `ldr
/// r0,[r0,#8]; cmp r0,#0; bne mutex_unlock @ 0x0807f6a0; bx lr`; the
/// next separately linked function (an unported release-template copy)
/// begins at 0x0839d0e8, confirming the exact 16-byte extent.
///
/// Loads the optional mutex from `body` at target +8 and, when non-NULL,
/// tail-branches to [`mutex_unlock`] @ 0x0807f6a0. It has no NULL guard for
/// `body`; callers must supply a readable [`RefcountedBody`]. It is the
/// separately linked unlock half paired with the lock helper @ 0x0839d0c8
/// inside the release template [`refcounted_body_release_slot1_copy`] @
/// 0x0839d038 (the two call sites at 0x0839d084/0x0839d0bc) and its
/// 0x0816cdxx sibling release (0x0816cda4/0x0816cddc).
///
/// No deliberate behavioral deviations. A distinct target-only section keeps
/// this separately linked retail helper from folding into its byte-identical
/// siblings at 0x0839d188, 0x0839d274, 0x0839d360, and 0x0839d44c.
///
/// # Safety
///
/// `body` must be readable. When its mutex is non-NULL, it must satisfy
/// [`mutex_unlock`]'s requirements.
#[cfg_attr(target_os = "none", link_section = ".text.refcounted_body_mutex_unlock_slot1_copy")]
#[cfg_attr(target_os = "none", no_mangle)]
#[inline(never)]
pub unsafe extern "C" fn refcounted_body_mutex_unlock_slot1_copy(body: *mut RefcountedBody) {
    let mutex = (*body).mutex;
    if !mutex.is_null() {
        mutex_unlock(mutex);
    }
...
/// 0x0839cb84..0x0839cbc0; [`refcounted_body_release_dtor`] starts at the
/// next real function boundary. Its two internal calls are predicated `blne`
/// to mutex_lock @ 0x0807f5c4 and tail `bne` to mutex_unlock @ 0x0807f6a0.
///
...
/// [`refcounted_body_acquire`] @ 0x0839cd5c modulo its mutex-call branch
/// displacements, so it directly composes the same ported mutex helpers. A
/// dedicated target text section preserves this hookable entry.
///
/// # Safety
///
...
#[cfg_attr(target_os = "none", no_mangle)]
#[cfg_attr(target_os = "none", link_section = ".text.refcounted_body_acquire_dtor")]
#[inline(never)]
pub unsafe extern "C" fn refcounted_body_acquire_dtor(
    dst: *mut *mut RefcountedBody,
    body: *mut RefcountedBody,
) {
    dst.write(body);
...
/// trailing `str r6,[r4]` / `pop {r4,r5,r6,pc}`; the separately linked
/// mutex-lock helper begins at 0x0839cd3c and the mutex-unlock helper at
/// 0x0839cd4c). Decoding every ARM B/BL word in osos.dec finds 9 direct
/// `bl` callers, all unconditional (0x08280908, 0x08280aa4, 0x08280c9c,
/// 0x08280d24, 0x08280d50, 0x08280d68, 0x08280e38, 0x08280f68, and
/// 0x0839ed20): no predicated forms, no tail `b` sites, and no image
/// data-word references, so it is never virtually dispatched.
///
/// A separately linked copy of [`refcounted_body_release_dtor`] @ 0x0839cbc0:
/// it has the same 36 ARM words except for its three direct-call
/// displacements. A NULL body leaves its slot untouched. Otherwise it
/// locks the optional mutex, wrapping-decrements the signed refcount, and
/// NULLs the slot. A non-final reference only unlocks. A final reference
/// NULL-guardedly dispatches vtable word 1 (+4) of the implementation,
/// never frees that implementation block, unlocks, mutex-deletes and
/// tag-2-deletes the mutex, clears body+8, then tag-2-deletes the body.
///
/// Deliberate codegen deviation: LLVM may inline the already ported mutex
/// and heap helpers instead of preserving the stock local helper calls;
/// the guard/decrement/slot-1-dispatch/teardown sequence is retained. A
/// dedicated target section prevents folding this hookable copy into its
/// byte-identical siblings.
///
/// # Safety
///
/// `slot` must be a valid aligned pointer slot. A non-NULL body, its mutex,
/// implementation, and that implementation's vtable (with a live virtual
/// destructor at word index 1) must be valid. Like retailOS, this function
/// does not NULL-check `slot`, and its mutex helpers guard only the mutex
/// word.
#[cfg_attr(target_os = "none", no_mangle)]
#[cfg_attr(target_os = "none", link_section = ".text.refcounted_body_release_dtor_copy")]
#[inline(never)]
pub unsafe extern "C" fn refcounted_body_release_dtor_copy(
    slot: *mut *mut RefcountedBody,
) {
    let body = slot.read();
    if body.is_null() {
...
/// refcounted_ptr_release_dtor_copy — original: `FUN_0816ccb0` @
/// 0x0816ccb0 (148 bytes — Ghidra's reported 140-byte extent omits the
/// final `mov r0,r4` / `pop {r4,r5,r6,pc}`; the separately linked next
/// function begins at 0x0816cd44). Decoding every ARM `B`/`BL` word in
/// `osos.dec` finds exactly seven inbound direct calls, all unconditional
/// `bl`: 0x081425e8, 0x08142604, 0x0826fa64, 0x0826fc0c, 0x082c7eb8,
/// 0x082c7fa0, and 0x083d4590. No predicated calls, direct tail `b` sites,
/// or word-aligned image references exist, so it is not virtually dispatched.
///
/// A separately linked copy of [`refcounted_ptr_release_dtor`] @ 0x0816cf24:
...
/// clears body+8, and tag-2-frees the body.
///
...
/// target section preserves this separately hookable copy.
///
/// # Safety
///
/// `slot` must be a valid aligned pointer slot. A non-NULL body, its mutex,
...
#[cfg_attr(target_os = "none", no_mangle)]
#[cfg_attr(target_os = "none", link_section = ".text.refcounted_ptr_release_dtor_copy")]
#[inline(never)]
pub unsafe extern "C" fn refcounted_ptr_release_dtor_copy(
    slot: *mut *mut RefcountedBody,
) -> *mut *mut RefcountedBody {
    let body = slot.read();
    if body.is_null() {
...
mod tests {
    ...
    #[test]
    fn acquire_dtor_null_body_overwrites_slot() {
        unsafe {
            let mut slot = 0xdead_beefusize as *mut RefcountedBody;

            refcounted_body_acquire_dtor(&mut slot, core::ptr::null_mut());
    ...
    #[test]
    fn acquire_dtor_wraps_refcount_without_mutex() {
        unsafe {
            let mut body = RefcountedBody {
                opaque0: 0x1111_2222,
                refcount: i32::MAX,
            ...
            refcounted_body_acquire_dtor(&mut slot, &mut body);
    ...
    #[test]
    fn acquire_dtor_handles_empty_mutex_cell() {
        unsafe {
            let mut mutex = Mutex {
                sem_cell: core::ptr::null_mut(),
                unused: 0,
    ...
    mod release {
        ...
        #[test]
        fn owned_variant_body_mutex_unlock_signals_only_a_present_mutex() {
            let _bench = bench();
            let mut semaphore = 0x3b;
            let mut mutex = Mutex {
                sem_cell: &mut semaphore,
            ...
            unsafe { refcounted_body_mutex_unlock_owned_variant(&mut body) };
        ...
        #[test]
        fn array_variant_body_mutex_unlock_signals_only_a_present_mutex() {
            let _bench = bench();
        ...
        #[test]
        fn owned_body_mutex_unlock_signals_only_a_present_mutex() {
            let _bench = bench();
            let mut semaphore = 0x3a;
            let mut mutex = Mutex {
                sem_cell: &mut semaphore,
        ...
        #[test]
        fn dtor_copy_body_mutex_unlock_signals_only_a_present_mutex() {
            let _bench = bench();
            let mut semaphore = 0x3c;
            let mut mutex = Mutex {
                sem_cell: &mut semaphore,
            ...
            unsafe { refcounted_body_mutex_unlock_dtor_copy(&mut body) };
            assert_eq!(events(), std::vec![Event::Signal(0x3c)]);
        ...
        #[test]
        fn slot1_copy_body_mutex_unlock_signals_only_a_present_mutex() {
            let _bench = bench();
            let mut semaphore = 0x5d;
            let mut mutex = Mutex {
                sem_cell: &mut semaphore,
                unused: 0,
            };
            ...
            unsafe { refcounted_body_mutex_unlock_slot1_copy(&mut body) };
            assert_eq!(events(), std::vec![Event::Signal(0x5d)]);
        ...
        /// The 0x0839ccac copy preserves the NULL-body early-out: it does
        /// not write the slot or touch the release machinery.
        #[test]
        fn dtor_copy_null_body_leaves_slot_untouched() {
            let _bench = bench();
            let mut slot: *mut RefcountedBody = core::ptr::null_mut();

            unsafe { refcounted_body_release_dtor_copy(&mut slot) };

            assert!(slot.is_null());
            assert!(events().is_empty());
        }

        /// A non-final release takes the shared path: decrement, optional
        /// lock/unlock, then the trailing slot NULL store.
        #[test]
        fn dtor_copy_shared_reference_decrements_unlocks_and_nulls_slot() {
            let _bench = bench();
            ...
            assert_eq!(body.refcount, 1);
            assert!(slot.is_null());
            assert_eq!(events(), std::vec![Event::Wait(0x61), Event::Signal(0x61)]);
        }

        /// The final drop dispatches vtable word 1, does not free that
        /// implementation allocation, and frees the body after its mutex.
        #[test]
        fn dtor_copy_final_reference_dispatches_and_tears_down_in_order() {
            let _bench = bench();
            let mut semaphore = 0x62;
            let mut mutex = Mutex {
                sem_cell: &mut semaphore,
                unused: 0,
            };
        ...
        /// ARM `subs` is wrapping: zero becomes -1 and cannot run final
        /// teardown through this copy.
        #[test]
        fn dtor_copy_zero_refcount_wraps_without_cleanup() {
            let _bench = bench();
            let mut body = RefcountedBody {
                opaque0: 0xdead_0000,
                refcount: 0,
        ...
        /// The 0x0816ccb0 copy preserves the slot address on a NULL-body
        /// early exit, then wraps a zero refcount to -1 without teardown.
        #[test]
        fn ptr_release_dtor_copy_null_and_wrapping_release_return_slot() {
            let _bench = bench();
            let mut empty_slot: *mut RefcountedBody = core::ptr::null_mut();
            let empty_slot_ptr = &mut empty_slot as *mut *mut RefcountedBody;

            assert_eq!(
                unsafe { refcounted_ptr_release_dtor_copy(empty_slot_ptr) },
                empty_slot_ptr
            );
            ...
            assert_eq!(
                unsafe { refcounted_ptr_release_dtor_copy(slot_ptr) },
                slot_ptr
            );
            assert_eq!(body.refcount, -1);
            assert!(slot.is_null());
        ...
        /// The 0x0816ccb0 final release dispatches vtable word 1 and destroys
        /// the mutex and body while retaining the implementation block.
        #[test]
        fn ptr_release_dtor_copy_final_reference_returns_slot_and_cleans_up() {
            let _bench = bench();
            let mut semaphore = 0x63;
            let mut mutex = Mutex {
                sem_cell: &mut semaphore,
                unused: 0,
            };
            ...
            let returned = unsafe { refcounted_ptr_release_dtor_copy(slot_ptr) };
