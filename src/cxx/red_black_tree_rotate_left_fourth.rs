//! Generic C++ red-black-tree left rotation — original: `FUN_083cd738` at
//! load address `0x083cd738`.
//!
//! Raw `osos.dec` establishes the exact 84-byte extent: 21 ARM words from
//! `ldr r2,[r1,#0xc]` at `0x083cd738` through `bx lr` at `0x083cd788`; the
//! separately linked right-rotation sibling begins at `0x083cd78c`. Decoding
//! every aligned ARM B/BL-immediate word in `osos.dec` finds five inbound direct
//! calls: four unconditional `bl` at `0x083cda74`, `0x083cdb14`, `0x083cdc00`,
//! and `0x083ce194`, plus a predicated `bleq` at `0x083ce0fc`. The predicated
...
///
/// Original: `FUN_083cd738` at load address `0x083cd738` (84 bytes; five
...
#[cfg_attr(target_os = "none", link_section = ".text.red_black_tree_rotate_left_fourth")]
#[inline(never)]
pub unsafe extern "C" fn red_black_tree_rotate_left_fourth(
...
mod tests {
    ...
    use super::red_black_tree_rotate_left_fourth;
    ...
    static SLAB: LazyLock<Option<usize>> = LazyLock::new(|| {
        crate::testing::try_map_u32_slab(
            crate::testing::hints::RED_BLACK_TREE_ROTATE_LEFT_FOURTH,
            0x1000,
        )
        .map(|p| p as usize)
    });
    ...
    fn rotates_root_and_both_parent_child_slots() {
        let Some(base) = try_slab() else {
            crate::testing::note_missing_u32_fixture("red_black_tree_rotate_left_fourth");
            return;
        };
        ...
        unsafe {
            ...
            red_black_tree_rotate_left_fourth(tree, pivot);
