//! Generic C++ red-black-tree right rotation — original: `FUN_083c4e68` at
//! load address `0x083c4e68`.
//!
//! Raw `osos.dec` establishes the exact 84-byte extent: 21 ARM words from
//! `ldr r2,[r1,#8]` at `0x083c4e68` through `bx lr` at `0x083c4eb8`; the
//! separately linked next function begins at `0x083c4ebc`. Decoding every
//! aligned ARM B/BL-immediate word in `osos.dec` finds five inbound direct
//! calls: four unconditional `bl` at `0x083c5258`, `0x083c52d0`, `0x083c53c0`,
//! and `0x083c5828`, plus a predicated `bleq` at `0x083c5878`. The predicated
//! site reaches this no-guard rotation only after its caller has selected the
//! left child as the pivot; all callers therefore supply a non-null pivot with
//! a non-null left child.
//!
...
///
/// Original: `FUN_083bb60c` at load address `0x083bb60c` (84 bytes; five
...
#[cfg_attr(target_os = "none", link_section = ".text.red_black_tree_rotate_right_nineteenth")]
#[inline(never)]
pub unsafe extern "C" fn red_black_tree_rotate_right_nineteenth(
...
mod tests {
    ...
    use super::{red_black_tree_rotate_right_nineteenth, RedBlackTree, RedBlackTreeNode};
    use core::ptr;
    use std::sync::LazyLock;

    static SLAB: LazyLock<Option<usize>> = LazyLock::new(|| {
        crate::testing::try_map_u32_slab(
            crate::testing::hints::RED_BLACK_TREE_ROTATE_RIGHT_EIGHTEENTH,
            0x1000,
        )
        .map(|p| p as usize)
    });
    ...
    fn rotates_root_and_both_parent_child_slots() {
        let Some(base) = try_slab() else {
            crate::testing::note_missing_u32_fixture("red_black_tree_rotate_right_nineteenth");
            return;
        };
        ...
        unsafe {
            ...
            red_black_tree_rotate_right_nineteenth(tree, pivot);
