//! `tree_lookup_result_write` — original: `FUN_083dc0a4` @ `0x083dc0a4`
//! (40 bytes).
//!
//! Raw `osos.dec` establishes the exact ten-word extent from `push
//! {r1,r2,r3,r4,r5,lr}` through `pop {r1,r2,r3,r4,r5,pc}`; `0x083dc0cc`
//! begins the next function. It makes one unconditional plain `bl`, to the
//! retail tree lookup at `0x083cf2b8`, and no predicated `bl` calls. The
//! lookup writes a target-width node pointer and one-byte match flag into a
//! stack result; this wrapper copies those five bytes to `result`.
//!
...
#[cfg(target_os = "none")]
const RETAIL_TREE_LOOKUP_ADDRESS: usize = 0x083c_f2b8;

#[cfg(not(target_os = "none"))]
unsafe extern "C" fn unavailable_tree_lookup(_: *mut TreeLookupResult, _: *mut u8, _: *const u8) {
    panic!("tree_lookup_result_write requires a retail tree lookup fixture")
}
...
#[inline(never)]
pub unsafe extern "C" fn tree_lookup_result_write(
    ...
    let mut lookup_result = TreeLookupResult { node: 0, matched: 0 };
    unsafe { lookup(&mut lookup_result, tree, key) };
    unsafe {
        result.cast::<u32>().write(lookup_result.node);
        result.cast::<u8>().add(4).write(lookup_result.matched);
    }
}
...
mod tests {
    ...
    fn copies_the_retail_lookup_result() {
        ...
        unsafe {
            ...
            tree_lookup_result_write(&mut result, tree, key);
