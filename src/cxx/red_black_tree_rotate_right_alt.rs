//! Generic C++ red-black-tree right rotation — original: `FUN_083ce2dc` at
//! load address `0x083ce2dc`.
//!
//! Raw `osos.dec` establishes the exact 84-byte extent: 21 ARM words from
//! `ldr r2,[r1,#8]` at `0x083ce2dc` through `bx lr` at `0x083ce32c`; the next
//! separately linked function begins at `0x083ce330`. Decoding every aligned
//! ARM B/BL-immediate word in `osos.dec` finds five inbound direct `bl` calls:
//! four unconditional calls at `0x083ce628`, `0x083ce6a0`, `0x083ce78c`, and
//! `0x083ceb64`, plus a predicated `bleq` at `0x083cebb4`. The predicated site
...
///
/// Original: `FUN_083ce2dc` at load address `0x083ce2dc` (84 bytes; five
...
#[cfg_attr(target_os = "none", link_section = ".text.red_black_tree_rotate_right_alt")]
#[inline(never)]
pub unsafe extern "C" fn red_black_tree_rotate_right_alt(
...
mod tests {
    ...
    use super::red_black_tree_rotate_right_alt;
    ...
    static SLAB: LazyLock<Option<usize>> = LazyLock::new(|| {
        crate::testing::try_map_u32_slab(
            crate::testing::hints::RED_BLACK_TREE_ROTATE_RIGHT_ALT,
            0x1000,
        )
        .map(|p| p as usize)
    });
    ...
    fn rotates_root_and_both_parent_child_slots() {
        let Some(base) = try_slab() else {
            crate::testing::note_missing_u32_fixture("red_black_tree_rotate_right_alt");
            return;
        };
        ...
        unsafe {
            ...
            red_black_tree_rotate_right_alt(tree, pivot);
