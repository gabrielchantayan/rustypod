//! Opaque allocation release — retailOS `FUN_0803a544` at load address
//! `0x0803a544` (8 bytes, `0x0803a544..0x0803a54c`; its descriptor literal
//! follows outside the function). Raw `osos.dec` establishes the next real
//! function at `0x0803a550`.
//!
//! ```text
//! 0803a544  ldr r1, [pc]        @ 0x089062ec
//! 0803a548  b   0x0803b3a4
//! 0803a54c  .word 0x089062ec
//! ```
//!
//! ## Call sites and algorithm
//!
//! Raw ARM decoding finds four inbound `bl` calls: one plain unconditional
//! call and three `blne` calls. The predicated callers establish their own
//! NULL gates. This wrapper deliberately has no NULL guard: it loads the fixed
//! opaque descriptor into r1 and tail-branches to `0x0803b3a4`, which creates
//! a temporary `(allocation, descriptor)` frame and calls the unported
//! type-erased release engine at `0x080c85bc` with r2 zero.
//!
//! The descriptor at `0x089062ec` has no recoverable concrete type. The port
...
pub const OPAQUE_ALLOCATION_DESCRIPTOR_089062EC: usize = 0x0890_62ec;

#[cfg(target_arch = "arm")]
extern "C" {
    /// release_opaque_allocation_089062ec — original: `FUN_0803a544` @ `0x0803a544`
    /// (8 bytes; one plain `bl` call and three `blne` calls).
    pub fn release_opaque_allocation_089062ec(allocation: *mut u8);
}
...
#[inline(never)]
pub unsafe extern "C" fn release_opaque_allocation_089062ec(allocation: *mut u8) {
    typed_allocation_release_helper(
        allocation,
        OPAQUE_ALLOCATION_DESCRIPTOR_089062EC as *const u8,
    );
}
...
core::arch::global_asm!(
    r#"
    .syntax unified
    .section .text.release_opaque_allocation_089062ec, "ax", %progbits
    .p2align 2
    .globl release_opaque_allocation_089062ec
    .type release_opaque_allocation_089062ec, %function
release_opaque_allocation_089062ec:
    ldr     r1, [pc]
    b       0x0803b3a4
    .word   0x089062ec
    .size release_opaque_allocation_089062ec, . - release_opaque_allocation_089062ec
"#
);
...
mod tests {
    ...
    fn nonnull_allocation_and_fixed_descriptor_reach_release_helper() {
        ...
        unsafe { release_opaque_allocation_089062ec(allocation) };
