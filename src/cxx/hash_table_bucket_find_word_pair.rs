//! `hash_table_bucket_find_word_pair` — original: `FUN_083d6d38` @
//! **0x083d6d38** (**72 bytes**, `0x083d6d38..0x083d6d80`; the next separately
//! linked function starts at `0x083d6d80`).
//!
//! Raw ARM decoding finds two direct, unconditional inbound `bl` sites
//! (`0x083d2204`, `0x083d2870`) and no predicated inbound calls. The body has
...
#[inline(never)]
pub unsafe extern "C" fn hash_table_bucket_find_word_pair(
    _table: *const u8,
    bucket: *const u32,
    key: *const u32,
) -> *mut u8 {
...
mod tests {
    ...
    static SLAB: LazyLock<Option<usize>> = LazyLock::new(|| {
        try_map_u32_slab(hints::HASH_TABLE_BUCKET_FIND_WORD_PAIR, FIXTURE_LEN)
    ...
    fn returns_null_for_an_empty_bucket() {
        ...
        if SLAB.is_none() {
            assert!(note_missing_u32_fixture("cxx/hash_table_bucket_find_word_pair"));
            return;
        }
        unsafe {
            ...
            assert!(hash_table_bucket_find_word_pair(base(), bucket, key).is_null());
        }
    }
    ...
    fn skips_nonmatching_nodes_and_returns_the_first_matching_link() {
        ...
        unsafe {
            ...
            assert_eq!(hash_table_bucket_find_word_pair(base(), bucket.cast(), key), second_link);
        }
    }
}
