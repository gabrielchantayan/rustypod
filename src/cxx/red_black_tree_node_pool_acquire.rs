//! Red-black-tree node-pool acquire — original: `FUN_083cc1d0` at load
//! address `0x083cc1d0`.
//!
//! Raw `osos.dec` establishes the exact 184-byte extent: 46 ARM words from
//! `push {r4-r8,lr}` at `0x083cc1d0` through `pop {r4-r8,pc}` at
//! `0x083cc284`; the separately linked left-rotation sibling begins at
//! `0x083cc288`. Decoding every aligned ARM B/BL-immediate word in
//! `osos.dec` finds exactly five inbound direct calls, all unconditional `bl`
//! at `0x08101a80`, `0x08177b64`, `0x08213a64`, `0x083cca58`, and
//! `0x083dc048`; there are no predicated direct calls.
//!
...
//! words are cleared, preserving its 4-byte client payload at +0x10.
//!
...
/// A 20-byte red-black-tree node with a caller-owned payload word.
#[repr(C)]
#[derive(Clone, Copy, Default)]
pub struct RedBlackTreePooledNode {
    pub color: u8,
    _padding: [u8; 3],
    pub parent: u32,
    pub left: u32,
...
const _: [u8; 0x04] = [0; core::mem::offset_of!(RedBlackTreePooledNode, parent)];
const _: [u8; 0x08] = [0; core::mem::offset_of!(RedBlackTreePooledNode, left)];
const _: [u8; 0x0c] = [0; core::mem::offset_of!(RedBlackTreePooledNode, right)];
const _: [u8; 0x10] = [0; core::mem::offset_of!(RedBlackTreePooledNode, value)];
const _: [u8; NODE_SIZE] = [0; core::mem::size_of::<RedBlackTreePooledNode>()];

/// Heap header prepended to each pool chunk.
#[repr(C)]
#[derive(Clone, Copy, Default)]
struct RedBlackTreeNodePoolChunk {
    previous: u32,
    capacity: u32,
    nodes: u32,
}

const _: [u8; 0x04] = [0; core::mem::offset_of!(RedBlackTreeNodePoolChunk, capacity)];
const _: [u8; 0x08] = [0; core::mem::offset_of!(RedBlackTreeNodePoolChunk, nodes)];
const _: [u8; 0x0c] = [0; core::mem::size_of::<RedBlackTreeNodePoolChunk>()];

/// The target-width node-pool state consumed by
/// [`red_black_tree_node_pool_acquire`].
#[repr(C)]
#[derive(Clone, Copy, Default)]
pub struct RedBlackTreeNodePool {
    pub chunks: u32,
    pub free: u32,
    pub next: u32,
    pub end: u32,
...
const _: [u8; 0x04] = [0; core::mem::offset_of!(RedBlackTreeNodePool, free)];
const _: [u8; 0x08] = [0; core::mem::offset_of!(RedBlackTreeNodePool, next)];
const _: [u8; 0x0c] = [0; core::mem::offset_of!(RedBlackTreeNodePool, end)];
const _: [u8; 0x10] = [0; core::mem::size_of::<RedBlackTreeNodePool>()];
...
#[inline(always)]
unsafe fn node_from_word(word: u32) -> *mut RedBlackTreePooledNode {
    word as usize as *mut RedBlackTreePooledNode
}

#[inline(always)]
unsafe fn chunk_from_word(word: u32) -> *mut RedBlackTreeNodePoolChunk {
    word as usize as *mut RedBlackTreeNodePoolChunk
}

/// Acquires and initializes one node from `pool`.
///
/// Original: `FUN_083cc1d0` at load address `0x083cc1d0` (184 bytes; five
/// unconditional inbound `bl` sites).
///
/// # Safety
///
/// `pool` must point to a writable [`RedBlackTreeNodePool`]. Existing pool
/// words must describe valid aligned chunks and nodes. The checked allocator
/// is assumed to return writable storage, matching the retail routine's
/// immediate dereference of both allocation results.
#[cfg_attr(target_os = "none", no_mangle)]
#[cfg_attr(target_os = "none", link_section = ".text.red_black_tree_node_pool_acquire")]
#[inline(never)]
pub unsafe extern "C" fn red_black_tree_node_pool_acquire(
    pool: *mut RedBlackTreeNodePool,
) -> *mut RedBlackTreePooledNode {
    let node = if (*pool).free != 0 {
        ...
        if next as u32 == (*pool).end {
            ...
            let chunk = operator_new_checked(core::mem::size_of::<RedBlackTreeNodePoolChunk>())
                as *mut RedBlackTreeNodePoolChunk;
...
mod tests {
    ...
    fn acquires_from_new_free_and_grown_chunks() {
        ...
        let Some(slab) = try_map_u32_slab(hints::RED_BLACK_TREE_NODE_POOL_ACQUIRE, 0x7000) else {
            note_missing_u32_fixture("cxx/red_black_tree_node_pool_acquire");
            return;
        };
        ...
        unsafe {
            ...
            let mut pool = RedBlackTreeNodePool::default();
            let first = red_black_tree_node_pool_acquire(&mut pool);
            ...
            let first_chunk = slab as *mut RedBlackTreeNodePoolChunk;
            ...
            assert_eq!((*first).value, 0xa5a5_a5a5);

            let second_free = slab.add(0x1040) as *mut RedBlackTreePooledNode;
            (*first).color = 1;
            (*first).parent = 2;
            (*first).left = 3;
            ...
            assert_eq!(red_black_tree_node_pool_acquire(&mut pool), first);
            ...
            pool.free = 0;
            pool.next = pool.end;
            let grown = red_black_tree_node_pool_acquire(&mut pool);
            ...
            let second_chunk = slab.add(0x2000) as *mut RedBlackTreeNodePoolChunk;
            ...
            assert_eq!((*grown).value, 0xa5a5_a5a5);

            pool.next = pool.end;
            let geometric = red_black_tree_node_pool_acquire(&mut pool);
            ...
            let third_chunk = slab.add(0x4000) as *mut RedBlackTreeNodePoolChunk;
