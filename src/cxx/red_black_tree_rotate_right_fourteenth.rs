//! Generic C++ red-black-tree right rotation — original: `FUN_083bca8c` at
//! load address `0x083bca8c`.
//!
//! Raw `osos.dec` establishes the exact 84-byte extent: 21 ARM words from
//! `ldr r2,[r1,#8]` at `0x083bca8c` through `bx lr` at `0x083bcadc`; the next
//! separately linked function begins at `0x083bcae0`. Decoding every aligned
//! ARM B/BL-immediate word in `osos.dec` finds five inbound direct calls: four
//! unconditional `bl` at `0x083bcdd8`, `0x083bce50`, `0x083bcf3c`, and
//! `0x083bd314`, plus a predicated `bleq` at `0x083bd364`; no direct tail `b`
//! reaches this function. The predicated site reaches this no-NULL-guard
//! rotation only after its caller selects the left child as the pivot.
//!
...
///
/// Original: `FUN_083bca8c` at load address `0x083bca8c` (84 bytes; five
...
#[cfg_attr(target_os = "none", link_section = ".text.red_black_tree_rotate_right_fourteenth")]
#[inline(never)]
pub unsafe extern "C" fn red_black_tree_rotate_right_fourteenth(
...
mod tests {
    ...
    use super::{red_black_tree_rotate_right_fourteenth, RedBlackTree, RedBlackTreeNode};
    use core::ptr;
    use std::sync::LazyLock;

    static SLAB: LazyLock<Option<usize>> = LazyLock::new(|| {
        crate::testing::try_map_u32_slab(
            crate::testing::hints::RED_BLACK_TREE_ROTATE_RIGHT_FOURTEENTH,
            0x1000,
        )
        .map(|p| p as usize)
    });
    ...
    fn rotates_root_and_both_parent_child_slots() {
        let Some(base) = try_slab() else {
            crate::testing::note_missing_u32_fixture("red_black_tree_rotate_right_fourteenth");
            return;
        };
        ...
        unsafe {
            ...
            red_black_tree_rotate_right_fourteenth(tree, pivot);
