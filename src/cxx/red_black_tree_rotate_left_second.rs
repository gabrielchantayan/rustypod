//! Generic C++ red-black-tree left rotation — original: `FUN_083cb844` at
//! load address `0x083cb844`.
//!
//! Raw `osos.dec` establishes the exact 84-byte extent: 21 ARM words from
//! `ldr r2,[r1,#12]` at `0x083cb844` through `bx lr` at `0x083cb894`; the
//! separately linked next function begins at `0x083cb898`. Decoding every
//! aligned ARM B/BL-immediate word in `osos.dec` finds five inbound direct
//! calls: four unconditional `bl` at `0x083cbb80`, `0x083cbc20`, `0x083cbd0c`,
//! and `0x083cc194`, plus a predicated `bleq` at `0x083cc0fc`. The predicated
//! site reaches this no-guard rotation only after its caller has selected the
//! right child as the pivot; all callers therefore supply a non-null pivot with
//! a non-null right child.
//!
...
///
/// Original: `FUN_083cb844` at load address `0x083cb844` (84 bytes; five
...
#[cfg_attr(target_os = "none", link_section = ".text.red_black_tree_rotate_left_second")]
#[inline(never)]
pub unsafe extern "C" fn red_black_tree_rotate_left_second(
...
mod tests {
    ...
    use super::{red_black_tree_rotate_left_second, RedBlackTree, RedBlackTreeNode};
    use core::ptr;
    use std::sync::LazyLock;

    static SLAB: LazyLock<Option<usize>> = LazyLock::new(|| {
        crate::testing::try_map_u32_slab(
            crate::testing::hints::RED_BLACK_TREE_ROTATE_LEFT_SECOND,
            0x1000,
        )
        .map(|p| p as usize)
    });
    ...
    fn rotates_root_and_both_parent_child_slots() {
        let Some(base) = try_slab() else {
            crate::testing::note_missing_u32_fixture("red_black_tree_rotate_left_second");
            return;
        };
        ...
        unsafe {
            ...
            red_black_tree_rotate_left_second(tree, pivot);
