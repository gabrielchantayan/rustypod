//! Generic C++ red-black-tree right rotation — original: `FUN_083cc2dc` at
//! load address `0x083cc2dc`.
//!
//! Raw `osos.dec` establishes the exact 84-byte extent: 21 ARM words from
//! `ldr r2,[r1,#8]` at `0x083cc2dc` through `bx lr` at `0x083cc32c`; the next
//! separately linked function begins at `0x083cc330`. Decoding every aligned
//! ARM B/BL-immediate word in `osos.dec` finds five inbound direct `bl` calls:
//! four unconditional calls at `0x083cc628`, `0x083cc6a0`, `0x083cc78c`, and
//! `0x083ccb5c`, plus predicated `bleq` at `0x083ccbac`. The predicated site
//! reaches this no-guard rotation only after its caller selects a left child,
//! so every caller supplies a non-null pivot with a non-null left child.
//!
...
//! target-width `u32` words so target layout and host fixtures are identical.
...
///
/// Original: `FUN_083cc2dc` at load address `0x083cc2dc` (84 bytes; five
...
#[cfg_attr(target_os = "none", link_section = ".text.red_black_tree_rotate_right_sixth")]
#[inline(never)]
pub unsafe extern "C" fn red_black_tree_rotate_right_sixth(
...
mod tests {
    ...
    use super::red_black_tree_rotate_right_sixth;
    use crate::cxx::red_black_tree_increment::RedBlackTreeNode;
    use crate::cxx::red_black_tree_rotate_right::RedBlackTree;
    use core::ptr;
    use std::sync::LazyLock;

    static SLAB: LazyLock<Option<usize>> = LazyLock::new(|| {
        crate::testing::try_map_u32_slab(
            crate::testing::hints::RED_BLACK_TREE_ROTATE_RIGHT_SIXTH,
            0x1000,
        )
        .map(|p| p as usize)
    });
    ...
    fn rotates_root_and_both_parent_child_slots() {
        let Some(base) = try_slab() else {
            crate::testing::note_missing_u32_fixture("red_black_tree_rotate_right_sixth");
            return;
        };
        ...
        unsafe {
            ...
            red_black_tree_rotate_right_sixth(tree, pivot);
