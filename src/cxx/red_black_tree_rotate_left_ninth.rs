//! Generic C++ red-black-tree left rotation — original: `FUN_083ca388` at
//! load address `0x083ca388`.
//!
//! Raw `osos.dec` establishes the exact 84-byte extent: 21 ARM words from
//! `ldr r2,[r1,#0xc]` at `0x083ca388` through `bx lr` at `0x083ca3d8`; the
//! separately linked right-rotation sibling begins at `0x083ca3dc`. Decoding
//! every aligned ARM B/BL-immediate word in `osos.dec` finds five inbound direct
//! calls: four unconditional `bl` at `0x083ca6c4`, `0x083ca764`, `0x083ca850`,
//! and `0x083cacd8`, plus a predicated `bleq` at `0x083cac40`. There are no
...
///
/// Original: `FUN_083ca388` at load address `0x083ca388` (84 bytes; five
...
#[cfg_attr(target_os = "none", link_section = ".text.red_black_tree_rotate_left_ninth")]
#[inline(never)]
pub unsafe extern "C" fn red_black_tree_rotate_left_ninth(
...
mod tests {
    ...
    use super::{red_black_tree_rotate_left_ninth, RedBlackTree, RedBlackTreeNode};
    use core::ptr;
    use std::sync::LazyLock;

    static SLAB: LazyLock<Option<usize>> = LazyLock::new(|| {
        crate::testing::try_map_u32_slab(
            crate::testing::hints::RED_BLACK_TREE_ROTATE_LEFT_NINTH,
            0x1000,
        )
        .map(|p| p as usize)
    });
    ...
    fn rotates_root_and_both_parent_child_slots() {
        let Some(base) = try_slab() else {
            crate::testing::note_missing_u32_fixture("red_black_tree_rotate_left_ninth");
            return;
        };
        ...
        unsafe {
            ...
            red_black_tree_rotate_left_ninth(tree, pivot);
