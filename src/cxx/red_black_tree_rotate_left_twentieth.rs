//! Generic C++ red-black-tree left rotation — original: `FUN_083c64dc` at
//! load address `0x083c64dc`.
//!
//! Raw `osos.dec` establishes the exact 84-byte extent: 21 ARM words from
//! `ldr r2,[r1,#0xc]` at `0x083c64dc` through `bx lr` at `0x083c652c`; the
//! separately linked right-rotation sibling begins at `0x083c6530`. Decoding
//! every aligned ARM B/BL-immediate word in `osos.dec` finds five inbound
//! direct calls: four unconditional `bl` at `0x083c6818`, `0x083c68b8`,
//! `0x083c69a8`, and `0x083c6e34`, plus a predicated `bleq` at `0x083c6d9c`.
...
///
/// Original: `FUN_083c64dc` at load address `0x083c64dc` (84 bytes; five
...
#[cfg_attr(target_os = "none", link_section = ".text.red_black_tree_rotate_left_twentieth")]
#[inline(never)]
pub unsafe extern "C" fn red_black_tree_rotate_left_twentieth(
...
mod tests {
    ...
    use super::{red_black_tree_rotate_left_twentieth, RedBlackTree, RedBlackTreeNode};
    use core::ptr;
    use std::sync::LazyLock;

    static SLAB: LazyLock<Option<usize>> = LazyLock::new(|| {
        crate::testing::try_map_u32_slab(
            crate::testing::hints::RED_BLACK_TREE_ROTATE_LEFT_TWENTIETH,
            0x1000,
        )
        .map(|p| p as usize)
    });
    ...
    fn rotates_root_and_both_parent_child_slots() {
        let Some(base) = try_slab() else {
            crate::testing::note_missing_u32_fixture("red_black_tree_rotate_left_twentieth");
            return;
        };
        ...
        unsafe {
            ...
            red_black_tree_rotate_left_twentieth(tree, pivot);
