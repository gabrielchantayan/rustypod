//! C++ red-black-tree right rotation — original: `FUN_083c6f7c` at load
//! address `0x083c6f7c`.
//!
//! Raw `osos.dec` establishes the exact 84-byte extent: 21 ARM words from
//! `ldr r2,[r1,#8]` at `0x083c6f7c` through `bx lr` at `0x083c6fcc`; the next
//! separately linked function begins at `0x083c6fd0`. Decoding every aligned
//! ARM B/BL-immediate word in `osos.dec` finds five inbound direct calls: four
//! unconditional `bl` at `0x083c72c8`, `0x083c7340`, `0x083c742c`, and
//! `0x083c7804`, plus a predicated `bleq` at `0x083c7854`. The predicated site
//! first selects the pivot's left child, so every caller supplies a non-null
//! pivot with a non-null left child. There are no direct tail-B callers or
//! aligned raw-word references to this address.
//!
...
///
/// Original: `FUN_083c6f7c` at load address `0x083c6f7c` (84 bytes; five
...
#[cfg_attr(target_os = "none", link_section = ".text.red_black_tree_rotate_right_seventeenth")]
#[inline(never)]
pub unsafe extern "C" fn red_black_tree_rotate_right_seventeenth(
...
mod tests {
    ...
    use super::{red_black_tree_rotate_right_seventeenth, RedBlackTree, RedBlackTreeNode};
    use core::ptr;
    use std::sync::LazyLock;

    static SLAB: LazyLock<Option<usize>> = LazyLock::new(|| {
        crate::testing::try_map_u32_slab(
            crate::testing::hints::RED_BLACK_TREE_ROTATE_RIGHT_SEVENTEENTH,
            0x1000,
        )
        .map(|p| p as usize)
    });
    ...
    fn rotates_root_and_both_parent_child_slots() {
        let Some(base) = try_slab() else {
            crate::testing::note_missing_u32_fixture("red_black_tree_rotate_right_seventeenth");
            return;
        };
        ...
        unsafe {
            ...
            red_black_tree_rotate_right_seventeenth(tree, pivot);
