...
use crate::cxx::vector_copy_construct_elem20::vector_copy_construct_elem20;
...
/// deque_iter_assign_alias_9e9c — original: `FUN_083d9e9c` @ 0x083d9e9c
/// (36 bytes, `0x083d9e9c..0x083d9ec0`; Ghidra reports 36).
///
/// Raw `osos.dec` words establish four forward aligned u32 loads from `src`
/// and stores to `dst`, followed by `bx lr` at 0x083d9ebc. The separately
/// linked `deque_seg_capacity` at 0x083d9ec0 fixes the true extent. Decoding
/// every ARM B/BL word finds two unconditional plain `bl` call sites
/// (0x081a846c and 0x081a8484), no predicated `bl` forms, and no body calls.
///
...
#[cfg_attr(target_os = "none", no_mangle)]
#[cfg_attr(target_os = "none", link_section = ".text.deque_iter_assign_alias_9e9c")]
#[inline(never)]
pub unsafe extern "C" fn deque_iter_assign_alias_9e9c(
    dst: *mut u32,
    src: *const u32,
) -> *mut u32 {
    for word in 0..4 {
...
///
/// Raw `osos.dec` words are four forward aligned `ldr`/`str` pairs followed
/// by `bx lr`; the independent `mov r0,#0x20; bx lr` function at
/// 0x083da240 fixes the true boundary. Whole-image ARM decoding finds two
/// inbound plain `bl` calls (0x081559fc and 0x08155a14), no inbound
/// predicated `bl` calls, and no body calls.
///
/// This byte-identical deque-iterator assignment copies `cur`, `seg_base`,
/// `seg_end`, and `seg_slot` from `src` to `dst`, preserving `dst` in r0 as
/// the return value. Deliberate deviation: this distinct export and text
/// section prevent folding with the identical iterator-copy instantiations.
///
/// # Safety
///
/// Both pointers must be valid, 4-byte aligned and 16 bytes wide; the
/// original is a plain forward word copy and does not handle overlap.
#[cfg_attr(target_os = "none", no_mangle)]
#[cfg_attr(target_os = "none", link_section = ".text.deque_iter_assign_alias_a21c")]
#[inline(never)]
pub unsafe extern "C" fn deque_iter_assign_alias_a21c(
    dst: *mut u32,
    src: *const u32,
) -> *mut u32 {
    for word in 0..4 {
...
/// deque_iter_assign_alias_9f38 — original: `FUN_083d9f38` @ 0x083d9f38
/// (36 bytes, `0x083d9f38..0x083d9f5b`; Ghidra reports 36).
///
/// Raw `osos.dec` is four forward aligned `ldr`/`str` word pairs followed by
/// `bx lr`; the independent `mov r0,#0x20; bx lr` at 0x083d9f5c fixes the
/// true boundary. Whole-image A32 decoding finds two inbound unconditional
/// plain `bl` calls (0x083dffec and 0x083e0004), no predicated `bl` calls,
/// and no body calls. The sole caller uses the words as the deque iterator
/// (`cur`, `seg_base`, `seg_end`, `seg_slot`), and r0 preserves `dst` despite
/// Ghidra declaring `void`.
///
/// Deliberate deviation: this distinct export and text section prevent LLVM
/// from folding this independently hookable copy into an identical template
/// instantiation.
///
/// # Safety
/// Both pointers must be valid, 4-byte aligned and 16 bytes wide; the
/// original is a plain forward word copy and does not handle overlap.
#[cfg_attr(target_os = "none", no_mangle)]
#[cfg_attr(target_os = "none", link_section = ".text.deque_iter_assign_alias_9f38")]
#[inline(never)]
pub unsafe extern "C" fn deque_iter_assign_alias_9f38(
    dst: *mut u32,
    src: *const u32,
) -> *mut u32 {
    for word in 0..4 {
...
/// container_element_at_alias_5fa4 — original: `FUN_083d5fa4` @ 0x083d5fa4
/// (24 bytes; two plain `bl` call sites, no predicated calls).
///
/// Raw ARM establishes the complete six-instruction body through `pop
/// {r4,pc}` at 0x083d5fb8; the next independently linked sibling starts at
/// 0x083d5fbc. It is a byte-identical [`container_element_at`] instantiation:
/// dispatch through vtable word 16 (`+0x40`) with `this` and the live `r1`
/// index, then load and return the element word from the returned slot.
///
/// Ghidra omits the live index parameter, but raw `blx r2` preserves `r1`.
/// This dedicated text section prevents LLVM identical-function folding from
/// removing the independently hookable target. Deliberate deviations: host
/// vtable entries use native-width pointers while preserving target word slot
/// 16; otherwise none.
///
/// # Safety
///
...
/// 1 slots.
#[cfg_attr(target_os = "none", no_mangle)]
#[cfg_attr(target_os = "none", link_section = ".text.container_element_at_alias_5fa4")]
#[inline(never)]
pub unsafe extern "C" fn container_element_at_alias_5fa4(this: *mut u8, index: usize) -> *mut u8 {
    let vtable = (this as *const *const ElementSlotFn).read();
    let element_slot = vtable.add(ELEMENT_SLOT_VTABLE_INDEX).read();
    element_slot(this, index).read()
}
...
/// vector_copy_range_elem12_alias_8acc — original: `FUN_083e8acc` @
/// 0x083e8acc (48 bytes; raw extent 0x083e8acc..0x083e8afc, bounded by the
/// next independently linked `push {r4,r5,r6,lr}` entry).
///
/// Copies the half-open `[first,last)` range of aligned 12-byte vector records
/// into `output`, advancing both cursors by 12 bytes and returning the
/// resulting output cursor. The three source words are loaded before any
/// output word is stored; a NULL initial output skips the first record only.
/// Whole-image A32 decoding finds two inbound plain `bl` sites (0x083e138c,
/// 0x083e13d8), no predicated inbound `bl` sites, and no body calls.
///
/// # Deliberate deviations
///
...
/// forward copies without an overlap guard.
#[cfg_attr(target_os = "none", no_mangle)]
#[cfg_attr(target_os = "none", link_section = ".text.vector_copy_range_elem12_alias_8acc")]
#[inline(never)]
pub unsafe extern "C" fn vector_copy_range_elem12_alias_8acc(
    mut first: *const u8,
    last: *const u8,
    mut output: *mut u8,
) -> *mut u8 {
...
/// # Deviations
///
/// None. The element helper is ported as
...
pub unsafe extern "C" fn vector_copy_construct_range_elem12(
    ...
    while first != last {
        copy_record_12_if_destination(vector.cast(), output, first);
        first = first.wrapping_add(0x0c);
        output = output.wrapping_add(0x0c);
    }
    output
}
/// vector_fill_construct_n_elem20 — original: `FUN_083e9550` @
/// 0x083e9550 (60 bytes; true extent 0x083e9550..0x083e958c, bounded by the
/// `push {r4-r8,lr}` of [`vector_fill_construct_n_elem12`] at 0x083e958c;
/// reference `ipod-decomp/decomp/c/038/083e9550_FUN_083e9550.c`).
///
/// `std::vector<T>` uninitialized-fill for `count` 0x14-byte elements. Each
/// iteration copy-constructs `source` into the current output slot, then
/// decrements the count and advances only that output cursor by 0x14. The
/// source and vector arguments are invariant across calls.
///
/// **Call count**, verified from raw A32 words: two inbound direct plain `bl`
/// sites (0x083e16e0 and 0x083e17a0) and no predicated inbound calls; the body
/// has one plain `bl` (`ebffb9e9` at 0x083e9574 to 0x083d7d20) and no
/// predicated calls.
///
/// # Deviations
///
/// There are no deliberate deviations: this now directly calls the ported
/// [`vector_copy_construct_elem20`] helper.
///
/// # Safety
///
/// `output` must be writable for `count` consecutive 0x14-byte elements
/// whenever it is non-NULL. `source` must identify a readable element whenever
/// `count` is nonzero and `output` is non-NULL.
#[cfg_attr(target_os = "none", no_mangle)]
#[inline(never)]
pub unsafe extern "C" fn vector_fill_construct_n_elem20(
    mut output: *mut u8,
    mut count: u32,
    source: *const u8,
    vector: *mut VectorStorage,
    ...
    while count != 0 {
        vector_copy_construct_elem20(vector.cast(), output, source);
        count -= 1;
...
/// vector_fill_construct_n_elem12 — original: `FUN_083e958c` @
/// 0x083e958c (60 bytes; true extent 0x083e958c..0x083e95c8, bounded by the
/// `b 0x083e95e0` entry of [`vector_fill_copy_word`] at 0x083e95c8;
/// reference `ipod-decomp/decomp/c/038/083e958c_FUN_083e958c.c`).
///
/// `std::vector<T>` uninitialized-fill for `count` 0x0c-byte elements. Each
/// iteration copy-constructs `source` into the current output slot, then
/// decrements the count and advances only that output cursor by 0x0c. The
/// source and vector arguments are invariant across calls.
///
/// **Call count**, verified from raw A32 words: two inbound direct plain `bl`
/// sites and no predicated inbound calls; the body has one plain `bl`
/// (`ebffba19` at 0x083e95b0 to 0x083d7e1c) and no predicated calls.
///
/// # Deviations
///
/// None. Each element construction calls the ported
/// [`copy_record_12_if_destination`] directly.
///
/// # Safety
///
/// `output` must be writable for `count` consecutive 0x0c-byte elements
/// whenever the installed helper writes. `source` must identify a readable
/// element whenever `count` is nonzero.
#[cfg_attr(target_os = "none", no_mangle)]
#[inline(never)]
pub unsafe extern "C" fn vector_fill_construct_n_elem12(
    mut output: *mut u8,
    mut count: u32,
    source: *const u8,
    vector: *mut VectorStorage,
    ...
    while count != 0 {
        copy_record_12_if_destination(vector.cast(), output, source);
        count -= 1;
        output = output.wrapping_add(0x0c);
    }
}
...
/// vector_fill_construct_n_elem28 — original: `FUN_083e9628` @ 0x083e9628
/// (60 bytes; extent 0x083e9628..0x083e9664, bounded by the next function's
/// `push {r4-r7,lr}` at 0x083e9664; reference
/// `ipod-decomp/decomp/c/038/083e9628_FUN_083e9628.c`).
///
/// `std::vector<T>` uninitialized-fill for `count` 0x1c-byte elements. Each
/// iteration copy-constructs `source` into the current output slot, then
/// decrements the count and advances only that output cursor by 0x1c. The
/// source and vector arguments are invariant across calls.
///
/// **Call count**, verified from the raw A32 words: two inbound direct plain
/// `bl` sites and no predicated inbound calls; the body has one plain `bl`
/// (`ebffba25` at 0x083e9648 to 0x083d7ee8) and no predicated calls.
///
/// # Deviations
///
/// The unported element helper dispatches through
/// [`VECTOR_COPY_CONSTRUCT_ELEM28_OPS`], as in
/// [`vector_copy_construct_range_elem28`]: target builds call its firmware
/// address and the host default is inert.
///
/// # Safety
///
/// `output` must be writable for `count` consecutive 0x1c-byte elements
/// whenever the installed helper writes. `source` must identify a readable
/// element whenever `count` is nonzero.
#[cfg_attr(target_os = "none", no_mangle)]
#[inline(never)]
pub unsafe extern "C" fn vector_fill_construct_n_elem28(
    mut output: *mut u8,
    mut count: u32,
    source: *const u8,
    vector: *mut VectorStorage,
    ...
    while count != 0 {
        (vector_copy_construct_elem28_ops().copy_construct)(vector, output, source);
        count -= 1;
...
/// vector_copy_construct_elem16 — original: `FUN_083d7ec0` @ 0x083d7ec0
/// (40 bytes; extent 0x083d7ec0..0x083d7ee8, bounded by the next independent
/// `push {r4,lr}` at 0x083d7ee8; reference
/// `ipod-decomp/decomp/c/037/083d7ec0_FUN_083d7ec0.c`).
///
/// Copy-constructs the two adjacent [`StringObject`] members of a 0x10-byte
/// vector element. A NULL `output` returns NULL without reading `source`;
/// otherwise it constructs the first member, then tail-calls the constructor
/// for the second and returns that second subobject. `vector` is ignored.
///
/// **Call count**, verified from raw A32 words: two inbound direct plain `bl`
/// sites (0x083e909c and 0x083e960c), no predicated inbound calls. The body
/// has one plain `bl` to [`string_object_copy_construct`] @ 0x082773e0, no
/// predicated `bl`, and an unconditional tail branch to that same constructor.
/// No semantic deviations. LLVM emits two normal calls instead of the stock
/// tail branch for the second constructor, but preserves the second call's
/// result as the return value.
///
/// # Safety
///
/// When `output` is non-NULL, `output` and `source` must identify writable
/// and readable [`StringObjectPair`] values respectively.
#[cfg_attr(target_os = "none", no_mangle)]
#[inline(never)]
pub unsafe extern "C" fn vector_copy_construct_elem16(
    _vector: *mut VectorStorage,
    output: *mut u8,
    source: *const u8,
) -> *mut StringObject {
    ...
    string_object_copy_construct(output.cast(), source.cast());
    string_object_copy_construct(
        output.cast::<StringObject>().add(1),
        source.cast::<StringObject>().add(1),
    )
}
...
/// predicated calls.
///
/// The element helper is ported directly as [`vector_copy_construct_elem16`].
///
/// # Safety
///
...
pub unsafe extern "C" fn vector_fill_construct_n_elem16(
    ...
    while count != 0 {
        vector_copy_construct_elem16(vector, output, source);
        count -= 1;
...
pub unsafe extern "C" fn vector_copy_construct_range_elem16(
    ...
    while first != last {
        vector_copy_construct_elem16(vector, output, first);
...
pub unsafe extern "C" fn vector_copy_construct_range_elem20(
    ...
    while first != last {
        vector_copy_construct_elem20(vector.cast(), output, first);
...
mod tests {
    ...
    #[test]
    fn iter_assign_alias_9e9c_copies_exactly_four_words_forward() {
        unsafe {
            let src = [0xdead_beefu32, 0, 0xffff_ffff, 0x1234_5678];
            let mut dst = [0xaaaa_aaaa; 5];
            assert_eq!(
                deque_iter_assign_alias_9e9c(dst.as_mut_ptr(), src.as_ptr()),
                dst.as_mut_ptr(),
            );
            assert_eq!(&dst[..4], &src);
            ...
            deque_iter_assign_alias_9e9c(overlapping.as_mut_ptr().add(1), overlapping.as_ptr());
    ...
    #[test]
    fn iter_assign_alias_9f38_preserves_destination_and_forward_overlap() {
        unsafe {
            let src = [0xdead_beefu32, 0, 0xffff_ffff, 0x1234_5678];
            let mut dst = [0xaaaa_aaaa; 5];
            assert_eq!(
                deque_iter_assign_alias_9f38(dst.as_mut_ptr(), src.as_ptr()),
                dst.as_mut_ptr(),
            );
            assert_eq!(&dst[..4], &src);
            assert_eq!(dst[4], 0xaaaa_aaaa, "does not write past 16 bytes");
            ...
            let mut overlapping = [1u32, 2, 3, 4, 5];
            deque_iter_assign_alias_9f38(overlapping.as_mut_ptr().add(1), overlapping.as_ptr());
            assert_eq!(overlapping, [1, 1, 1, 1, 1]);
        }
    }

    #[test]
    fn iter_assign_alias_a21c_preserves_destination_and_forward_overlap() {
        unsafe {
            let src = [0xdead_beefu32, 0, 0xffff_ffff, 0x1234_5678];
            let mut dst = [0xaaaa_aaaa; 5];
            assert_eq!(
                deque_iter_assign_alias_a21c(dst.as_mut_ptr(), src.as_ptr()),
                dst.as_mut_ptr(),
            );
            assert_eq!(&dst[..4], &src);
            assert_eq!(dst[4], 0xaaaa_aaaa, "does not write past 16 bytes");
            ...
            let mut overlapping = [1u32, 2, 3, 4, 5];
            deque_iter_assign_alias_a21c(overlapping.as_mut_ptr().add(1), overlapping.as_ptr());
            assert_eq!(overlapping, [1, 1, 1, 1, 1]);
        }
    }
    ...
    #[test]
    fn element_at_alias_5fa4_dispatches_through_slot_0x40_and_derefs() {
        unsafe {
            let mut vtable = [fake_element_slot as ElementSlotFn; ELEMENT_SLOT_VTABLE_INDEX + 1];
            let mut a: u8 = 1;
            let mut b: u8 = 2;
            ...
            let this = core::ptr::addr_of_mut!(container) as *mut u8;
            assert_eq!(container_element_at_alias_5fa4(this, 0), &mut a as *mut u8);
            assert_eq!(LAST_INDEX, 0, "the index is passed through in r1");
            assert_eq!(container_element_at_alias_5fa4(this, 1), &mut b as *mut u8);
            assert!(container_element_at_alias_5fa4(this, 2).is_null(), "NULL element, not NULL slot");
        }
    }
    ...
    #[test]
    fn vector_copy_range_elem12_alias_8acc_copies_and_skips_null_output_source() {
        let source = [
            0x0102_0304, 0x1112_1314, 0x2122_2324,
            0x3132_3334, 0x4142_4344, 0x5152_5354,
        ];
        ...
        let returned = unsafe {
            vector_copy_range_elem12_alias_8acc(
                source.as_ptr().cast::<u8>(),
    ...
    fn fill_construct_n_elem28_reuses_source_and_strides_output() {
        ...
        let mut destination = [0xa5u8; 3 * 0x1c];
        let mut vector = VectorStorage {
            begin: core::ptr::null_mut(),
            end: core::ptr::null_mut(),
        ...
        unsafe {
            vector_fill_construct_n_elem28(
                output,
                3,
                source.as_ptr(),
    ...
    #[test]
    fn fill_construct_n_elem28_zero_count_does_not_dispatch() {
        let _guard = copy_construct_elem28_guard();
        unsafe { install_recording_copy_construct_elem28() };
    ...
    #[test]
    fn fill_construct_n_elem28_null_output_still_dispatches() {
        let _guard = copy_construct_elem28_guard();
        unsafe { install_recording_copy_construct_elem28() };
    ...
    #[test]
    fn vector_copy_construct_elem16_constructs_adjacent_members_and_returns_second() {
        let source = StringObjectPair {
            first: StringObject { vtable: core::ptr::null(), payload: core::ptr::null_mut() },
            second: StringObject { vtable: core::ptr::null(), payload: core::ptr::null_mut() },
        };
        let mut destination = StringObjectPair {
            first: StringObject { vtable: core::ptr::null(), payload: 1usize as *mut u8 },
            second: StringObject { vtable: core::ptr::null(), payload: 2usize as *mut u8 },
        };

        let returned = unsafe {
            vector_copy_construct_elem16(
                core::ptr::null_mut(),
                core::ptr::addr_of_mut!(destination).cast(),
                core::ptr::addr_of!(source).cast(),
            )
        };

        assert_eq!(returned, core::ptr::addr_of_mut!(destination.second));
        assert!(destination.first.vtable == &STRING_OBJECT_VTABLE);
        assert!(destination.second.vtable == &STRING_OBJECT_VTABLE);
        assert!(destination.first.payload.is_null());
        assert!(destination.second.payload.is_null());
    }

    #[test]
    fn vector_copy_construct_elem16_null_output_does_not_read_source() {
        assert!(unsafe {
            vector_copy_construct_elem16(
                core::ptr::null_mut(),
                core::ptr::null_mut(),
