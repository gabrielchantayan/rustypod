//! Generic C++ red-black-tree right rotation — original: `FUN_083bc048` at
//! load address `0x083bc048`.
//!
//! Raw `osos.dec` establishes the exact 84-byte extent: 21 ARM words from
//! `ldr r2,[r1,#8]` at `0x083bc048` through `bx lr` at `0x083bc098`; the next
//! separately linked function begins at `0x083bc09c`. Decoding every aligned
//! ARM B/BL-immediate word in `osos.dec` finds five inbound direct calls: four
//! unconditional `bl` at `0x083bc394`, `0x083bc40c`, `0x083bc4f8`, and
//! `0x083bc8d0`, plus a predicated `bleq` at `0x083bc920`; no direct tail `b`
...
///
/// Original: `FUN_083bc048` at load address `0x083bc048` (84 bytes; five
...
#[cfg_attr(target_os = "none", link_section = ".text.red_black_tree_rotate_right_fifteenth")]
#[inline(never)]
pub unsafe extern "C" fn red_black_tree_rotate_right_fifteenth(
...
mod tests {
    ...
    use super::{red_black_tree_rotate_right_fifteenth, RedBlackTree, RedBlackTreeNode};
    use core::ptr;
    use std::sync::LazyLock;

    static SLAB: LazyLock<Option<usize>> = LazyLock::new(|| {
        crate::testing::try_map_u32_slab(
            crate::testing::hints::RED_BLACK_TREE_ROTATE_RIGHT_FIFTEENTH,
            0x1000,
        )
        .map(|p| p as usize)
    });
    ...
    fn rotates_root_and_both_parent_child_slots() {
        let Some(base) = try_slab() else {
            crate::testing::note_missing_u32_fixture("red_black_tree_rotate_right_fifteenth");
            return;
        };
        ...
        unsafe {
            ...
            red_black_tree_rotate_right_fifteenth(tree, pivot);
