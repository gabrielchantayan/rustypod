//! Generic C++ red-black-tree right rotation, third copy — original:
//! `FUN_083cae4c` at load address `0x083cae4c`.
//!
//! Raw `osos.dec` establishes the exact 84-byte extent: 21 ARM words from
//! `ldr r2,[r1,#8]` at `0x083cae4c` through `bx lr` at `0x083cae9c`; the next
//! separately linked function begins at `0x083caea0`. Decoding every aligned
//! ARM B/BL-immediate word in `osos.dec` finds five inbound direct calls: four
//! unconditional `bl` at `0x083cb198`, `0x083cb210`, `0x083cb300`, and
//! `0x083cb6dc`, plus a predicated `bleq` at `0x083cb72c`. The predicated site
...
///
/// Original: `FUN_083cae4c` at load address `0x083cae4c` (84 bytes; five
...
#[cfg_attr(target_os = "none", link_section = ".text.red_black_tree_rotate_right_third")]
#[inline(never)]
pub unsafe extern "C" fn red_black_tree_rotate_right_third(
...
mod tests {
    ...
    use super::red_black_tree_rotate_right_third;
    ...
    static SLAB: LazyLock<Option<usize>> = LazyLock::new(|| {
        crate::testing::try_map_u32_slab(
            crate::testing::hints::RED_BLACK_TREE_ROTATE_RIGHT_THIRD,
            0x1000,
        )
        .map(|p| p as usize)
    });
    ...
    fn rotates_root_and_both_parent_child_slots() {
        let Some(base) = try_slab() else {
            crate::testing::note_missing_u32_fixture("red_black_tree_rotate_right_third");
            return;
        };
        ...
        unsafe {
            ...
            red_black_tree_rotate_right_third(tree, pivot);
