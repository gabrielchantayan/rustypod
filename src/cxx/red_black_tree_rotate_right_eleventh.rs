//! Generic C++ red-black-tree right rotation — original: `FUN_083c8f54` at
//! load address `0x083c8f54`.
//!
//! Raw `osos.dec` establishes the exact 84-byte extent: 21 ARM words from
//! `ldr r2,[r1,#8]` at `0x083c8f54` through `bx lr` at `0x083c8fa4`; the next
//! separately linked function begins at `0x083c8fa8`. Decoding every aligned
//! ARM B/BL-immediate word in `osos.dec` finds five inbound direct calls: four
//! unconditional `bl` at `0x083c92a0`, `0x083c9318`, `0x083c9404`, and
//! `0x083c97dc`, plus a predicated `bleq` at `0x083c982c`. The predicated site
...
///
/// Original: `FUN_083c8f54` at load address `0x083c8f54` (84 bytes; five
...
#[cfg_attr(target_os = "none", link_section = ".text.red_black_tree_rotate_right_eleventh")]
#[inline(never)]
pub unsafe extern "C" fn red_black_tree_rotate_right_eleventh(
...
mod tests {
    ...
    use super::{red_black_tree_rotate_right_eleventh, RedBlackTree, RedBlackTreeNode};
    use core::ptr;
    use std::sync::LazyLock;

    static SLAB: LazyLock<Option<usize>> = LazyLock::new(|| {
        crate::testing::try_map_u32_slab(
            crate::testing::hints::RED_BLACK_TREE_ROTATE_RIGHT_ELEVENTH,
            0x1000,
        )
        .map(|p| p as usize)
    });
    ...
    fn rotates_root_and_both_parent_child_slots() {
        let Some(base) = try_slab() else {
            crate::testing::note_missing_u32_fixture("red_black_tree_rotate_right_eleventh");
            return;
        };
        ...
        unsafe {
            ...
            red_black_tree_rotate_right_eleventh(tree, pivot);
