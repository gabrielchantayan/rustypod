//! vector_record24_copy — original: `FUN_083e9c80` @ 0x083e9c80 (60 bytes).
//!
//! Raw `osos.dec` words establish the exact 15-instruction extent
//! 0x083e9c80..0x083e9cbb: the final `pop {r4,r5,r6,pc}` is at 0x083e9cb8,
//! and 0x083e9cbc starts a separate `push {r4,r5,r6,lr}` function. Full-image
//! A32 branch decoding finds two inbound unconditional plain `bl` sites
//! (0x083e2ec0 and 0x083e2f10), zero inbound predicated `bl` sites, and one
...
//! byte-identical `fixed_record_range_copy` port.
...
#[cfg_attr(target_os = "none", link_section = ".text.vector_record24_copy")]
#[inline(never)]
pub unsafe extern "C" fn vector_record24_copy(
    mut source: *const u8,
    source_end: *const u8,
...
mod tests {
    ...
    use super::vector_record24_copy;
    ...
    fn empty_range_returns_destination_without_accessing_source() {
        ...
        let result = unsafe { vector_record24_copy(source, source, destination.as_mut_ptr()) };
        assert_eq!(result, unsafe { destination.as_mut_ptr().add(24 - 24) });
    ...
    fn copies_multiple_complete_records_and_returns_end() {
        ...
        let result = unsafe {
            vector_record24_copy(
                source.as_ptr().cast(),
                unsafe { source.as_ptr().cast::<u8>().add(72) },
    ...
    fn preserves_per_record_forward_grouped_overlap() {
        ...
        unsafe {
            vector_record24_copy(base, base.add(48), base.add(8));
        }
