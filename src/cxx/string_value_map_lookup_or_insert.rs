//! String-value map lookup-or-insert — `FUN_083db510` @ `0x083db510`.
//!
//! Raw `osos.dec` establishes the exact 76-byte extent: 19 ARM words from
//! `push {r4,r5,lr}` at `0x083db510` through `pop {r4,r5,pc}` at `0x083db558`;
//! the next independently entered function begins at `0x083db55c`. Decoding
//! every word finds three unconditional plain `bl` instructions and no
//! predicated `bl`: veneers at `0x083db524` and `0x083db548` resolve to
//! `cxx_string_copy_ctor` @ `0x083d8c30` and `cxx_string_release` @ `0x083d8b04`,
//! while `0x083db538` resolves to the still-unported tree operation @ `0x083c3cb8`.
//!
...
type StringValueMapTreeOperation = unsafe extern "C" fn(
    result: *mut StringKeyInsertResult,
    map: *mut u8,
    pair: *const StringKeyPair,
);

const STRING_VALUE_MAP_TREE_OPERATION_ADDRESS: usize = 0x083c_3cb8;

#[cfg(target_os = "none")]
#[inline(never)]
unsafe fn string_value_map_tree_operation(
    result: *mut StringKeyInsertResult,
    map: *mut u8,
    pair: *const StringKeyPair,
) {
    let operation: StringValueMapTreeOperation = unsafe {
        core::mem::transmute(STRING_VALUE_MAP_TREE_OPERATION_ADDRESS)
    };
    unsafe { operation(result, map, pair) }
}

#[cfg(not(target_os = "none"))]
static mut STRING_VALUE_MAP_TREE_OPERATION: StringValueMapTreeOperation = missing_string_value_map_tree_operation;

#[cfg(not(target_os = "none"))]
unsafe extern "C" fn missing_string_value_map_tree_operation(
    result: *mut StringKeyInsertResult,
    _map: *mut u8,
...
unsafe fn string_value_map_tree_operation(
    ...
    let operation = unsafe {
        core::ptr::addr_of!(STRING_VALUE_MAP_TREE_OPERATION).read_volatile()
    };
    unsafe { operation(result, map, pair) }
}

/// string_value_map_lookup_or_insert — original: `FUN_083db510` @
/// `0x083db510` (76 bytes; three unconditional plain `bl` instructions, no
/// predicated `bl`; the next independently entered function is `0x083db55c`).
///
/// # Safety
///
/// `key` must point to a live COW `basic_string` word. `map` and its tree
/// operation must satisfy the 0x083c3cb8 result contract: write a node pointer
/// at `result + 0` and an inserted flag byte at `result + 4`.
#[cfg_attr(target_os = "none", no_mangle)]
#[inline(never)]
pub unsafe extern "C" fn string_value_map_lookup_or_insert(
    map: *mut u8,
    key: *const *mut u8,
) -> *mut u8 {
    ...
    unsafe { string_value_map_tree_operation(&mut result, map, &pair) };
...
mod tests {
    ...
    struct TreeOperationGuard(StringValueMapTreeOperation);

    impl TreeOperationGuard {
        unsafe fn install(operation: StringValueMapTreeOperation) -> Self {
            let previous = unsafe {
                core::ptr::addr_of!(STRING_VALUE_MAP_TREE_OPERATION).read_volatile()
            };
            unsafe {
                core::ptr::addr_of_mut!(STRING_VALUE_MAP_TREE_OPERATION).write_volatile(operation);
            }
            Self(previous)
        }
    }

    impl Drop for TreeOperationGuard {
        fn drop(&mut self) {
            unsafe {
                core::ptr::addr_of_mut!(STRING_VALUE_MAP_TREE_OPERATION).write_volatile(self.0);
            }
        }
    }
    ...
    fn null_result_node_returns_original_wrapping_offset() {
        ...
        unsafe {
            let _guard = TreeOperationGuard::install(missing_string_value_map_tree_operation);
            let key = empty_rep_data();
            let value = string_value_map_lookup_or_insert(core::ptr::null_mut(), &key);
    ...
    fn passes_zero_value_pair_and_returns_node_value_offset() {
        ...
        unsafe extern "C" fn record_tree_operation(
            result: *mut StringKeyInsertResult,
            map: *mut u8,
            pair: *const StringKeyPair,
        ) {
        ...
        unsafe {
            let _guard = TreeOperationGuard::install(record_tree_operation);
            let mut map = [0u8; 1];
            let key = empty_rep_data();
            let value = string_value_map_lookup_or_insert(map.as_mut_ptr(), &key);
