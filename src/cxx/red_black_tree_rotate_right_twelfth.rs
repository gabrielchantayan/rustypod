//! Generic C++ red-black-tree right rotation — original: `FUN_083c8510` at
//! load address `0x083c8510`.
//!
//! Raw `osos.dec` establishes the exact 84-byte extent: 21 ARM words from
//! `ldr r2,[r1,#8]` at `0x083c8510` through `bx lr` at `0x083c8560`; the next
//! separately linked function begins at `0x083c8564`. Decoding every aligned
//! ARM B/BL-immediate word in `osos.dec` finds five inbound direct calls: four
//! unconditional `bl` at `0x083c885c`, `0x083c88d4`, `0x083c89c0`, and
//! `0x083c8d98`, plus a predicated `bleq` at `0x083c8de8`. The predicated site
...
///
/// Original: `FUN_083c8510` at load address `0x083c8510` (84 bytes; five
...
#[cfg_attr(target_os = "none", link_section = ".text.red_black_tree_rotate_right_twelfth")]
#[inline(never)]
pub unsafe extern "C" fn red_black_tree_rotate_right_twelfth(
...
mod tests {
    ...
    use super::{red_black_tree_rotate_right_twelfth, RedBlackTree, RedBlackTreeNode};
    use core::ptr;
    use std::sync::LazyLock;

    static SLAB: LazyLock<Option<usize>> = LazyLock::new(|| {
        crate::testing::try_map_u32_slab(
            crate::testing::hints::RED_BLACK_TREE_ROTATE_RIGHT_TWELFTH,
            0x1000,
        )
        .map(|p| p as usize)
    });
    ...
    fn rotates_root_and_both_parent_child_slots() {
        let Some(base) = try_slab() else {
            crate::testing::note_missing_u32_fixture("red_black_tree_rotate_right_twelfth");
            return;
        };
        ...
        unsafe {
            ...
            red_black_tree_rotate_right_twelfth(tree, pivot);
