//! `rgba8_cursor_write_rgb555a1` — original: `FUN_0825eca8` @ `0x0825eca8`
//! (28 bytes; **5 unconditional `bl` call sites**, no predicated `bl` forms
//! or direct tail branches, binary-scanned by decoding every ARM B/BL word in
//! `osos.dec`).
//!
//! The complete seven-word body starts with `push {r4,lr}` at `0x0825eca8` and
//! ends with `pop {r4,pc}` at `0x0825ecc4`; the distinct packed-pixel reader
//! begins at `0x0825ecc8`, so there is no literal pool. It loads the current
...
//! `rgba8_to_rgb555a1`, and stores the returned low halfword.
//!
...
use crate::ui::rgb555a1_pack::rgba8_to_rgb555a1;

/// Packs one RGBA8 record through `destination_cursor` as RGB555A1.
///
/// # Safety
///
...
#[inline(never)]
pub unsafe extern "C" fn rgba8_cursor_write_rgb555a1(
    _unused: u32,
    ...
    destination.write_volatile(rgba8_to_rgb555a1(components) as u16);
}

#[cfg(test)]
mod tests {
    use super::rgba8_cursor_write_rgb555a1;

    fn reference_rgb555a1(components: &[u8; 4]) -> u16 {
        (((u16::from(components[0]) & 0xf8) << 8)
            | ((u16::from(components[1]) & 0xf8) << 3)
            | ((u16::from(components[2]) & 0xf8) >> 2)
            | (u16::from(components[3]) >> 7))
    }
    ...
    fn advances_cursor_and_writes_only_its_current_halfword() {
        ...
        unsafe { rgba8_cursor_write_rgb555a1(0xffff_ffff, &mut cursor, components.as_ptr()) };

        assert_eq!(pixels, [0xa5a5, reference_rgb555a1(&components), 0xc3c3]);
        assert_eq!(cursor, unsafe { pixels.as_mut_ptr().add(2) });
    }

    #[test]
    fn preserves_low_component_bits_behavior_at_an_offset() {
        let record = [0xa5, 0x0f, 0xf7, 0x9a, 0x7f, 0x5a];
        let mut pixels = [0; 2];
        let mut cursor = pixels.as_mut_ptr();
        let components = unsafe { record.as_ptr().add(1) };

        unsafe { rgba8_cursor_write_rgb555a1(0, &mut cursor, components) };

        assert_eq!(pixels[0], 0x0fa6);
