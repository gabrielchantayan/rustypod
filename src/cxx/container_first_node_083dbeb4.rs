...
//!
//! `cxx_container_first_node_083dbeb4` — retailOS `FUN_083dbeb4` @
//! **0x083dbeb4**, **20 bytes** (`0x083dbeb4..0x083dbec7`; the following
//! `push {r3,lr}` at `0x083dbec8` starts a separately linked function). Raw
...
//! (0x0819ed38 and 0x0819f054), and no predicated inbound `bl` sites.
//!
//! # Algorithm
//!
//! Load the nested container pointer at `header + 0x10`, then return its word
//! at `+0x08`. The two callers use the returned word as the root or current
//! node of an intrusive tree traversal.
//!
...
//! value without that dead stack traffic.
...
pub unsafe extern "C" fn cxx_container_first_node_083dbeb4(header: *const u8) -> *mut u8 {
...
mod tests {
    ...
    fn returns_each_headers_nested_containers_first_node_word() {
        let Some(slab) = try_map_u32_slab(hints::CXX_CONTAINER_FIRST_NODE_083DBEB4, 0x1000) else {
            return;
        };
        unsafe {
            ...
            assert_eq!(cxx_container_first_node_083dbeb4(first_header), first_node);
            assert_eq!(cxx_container_first_node_083dbeb4(second_header), second_node);
        }
    }
}
