//! C++ red-black-tree right rotation — original: `FUN_083c79c0` at load
//! address `0x083c79c0`.
//!
//! Raw `osos.dec` establishes the exact 84-byte extent: 21 ARM words from
//! `ldr r2,[r1,#8]` at `0x083c79c0` through `bx lr` at `0x083c7a10`; the next
//! separately linked function begins at `0x083c7a14`. Decoding every aligned
//! ARM B/BL-immediate word in `osos.dec` finds five inbound direct calls: four
//! unconditional `bl` at `0x083c7d0c`, `0x083c7d84`, `0x083c7e70`, and
//! `0x083c8354`, plus a predicated `bleq` at `0x083c83a4`. The predicated site
//! reaches this no-guard rotation only after its caller selects the left child
//! as the pivot; all callers therefore supply a non-null pivot with a non-null
//! left child.
//!
...
///
/// Original: `FUN_083c79c0` at load address `0x083c79c0` (84 bytes; five
...
#[cfg_attr(target_os = "none", link_section = ".text.red_black_tree_rotate_right_sixteenth")]
#[inline(never)]
pub unsafe extern "C" fn red_black_tree_rotate_right_sixteenth(
...
mod tests {
    ...
    use super::{red_black_tree_rotate_right_sixteenth, RedBlackTree, RedBlackTreeNode};
    use core::ptr;
    use std::sync::LazyLock;

    static SLAB: LazyLock<Option<usize>> = LazyLock::new(|| {
        crate::testing::try_map_u32_slab(
            crate::testing::hints::RED_BLACK_TREE_ROTATE_RIGHT_SIXTEENTH,
            0x1000,
        )
        .map(|p| p as usize)
    });
    ...
    fn rotates_root_and_both_parent_child_slots() {
        let Some(base) = try_slab() else {
            crate::testing::note_missing_u32_fixture("red_black_tree_rotate_right_sixteenth");
            return;
        };
        ...
        unsafe {
            ...
            red_black_tree_rotate_right_sixteenth(tree, pivot);
