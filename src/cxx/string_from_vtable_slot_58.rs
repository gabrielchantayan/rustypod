...
//!
//! - `string_construct_from_vtable_slot_58` — original: `FUN_082a3e74` @
//!   `0x082a3e74` (**36 bytes**, exact: nine ARM instructions through
//!   `0x082a3e94`; the separately linked sibling begins at `0x082a3e98`).
//!   **6 direct `bl` call sites**, all unconditional plain `bl`, with zero
...
//! `source` through its vtable's opaque `+0x58` slot as `slot(source, out)`.
...
/// Opaque `+0x58` string-writing virtual method.
pub type StringSlot58Write = unsafe extern "C" fn(*mut StringSlot58Source, *mut StringObject);

/// Vtable prefix consumed by [`string_construct_from_vtable_slot_58`].
///
/// The function only reads its word at `+0x58`; the preceding entries remain
/// intentionally unnamed.
#[repr(C)]
pub struct StringSlot58SourceVtable {
    /// `+0x00..+0x54`: opaque virtual entries.
    pub opaque_00_54: [usize; 22],
    /// `+0x58`: writes a representation of `source` into `out`.
    pub write_string: StringSlot58Write,
}

/// Vtable-headed object accepted by the wrapper.
#[repr(C)]
pub struct StringSlot58Source {
    /// `+0x00`: pointer to [`StringSlot58SourceVtable`].
    pub vtable: *const StringSlot58SourceVtable,
}

#[cfg(target_pointer_width = "32")]
const _: [u8; 0x58] = [0; core::mem::offset_of!(StringSlot58SourceVtable, write_string)];

/// string_construct_from_vtable_slot_58 — original: `FUN_082a3e74` @
/// `0x082a3e74` (36 bytes, exact extent `0x082a3e74..0x082a3e94`; 6 direct
/// unconditional `bl` callers, binary-verified).
///
/// Constructs `out`, then invokes `source`'s opaque vtable slot `+0x58` as
...
/// to an object with a readable vtable and a callable `+0x58` slot. The slot
/// owns any further validity requirements for `source` and `out`.
#[cfg_attr(target_os = "none", no_mangle)]
#[cfg_attr(target_os = "none", link_section = ".text.string_construct_from_vtable_slot_58")]
#[inline(never)]
pub unsafe extern "C" fn string_construct_from_vtable_slot_58(
    out: *mut StringObject,
    source: *mut StringSlot58Source,
) {
...
mod tests {
    ...
    static mut OBSERVED_SOURCE: *mut StringSlot58Source = ptr::null_mut();
    ...
    static SOURCE_VTABLE: StringSlot58SourceVtable = StringSlot58SourceVtable {
        opaque_00_54: [0xdead_beef; 22],
    ...
    fn constructs_before_dispatching_the_exact_slot() {
        ...
        let mut source = StringSlot58Source { vtable: &SOURCE_VTABLE };

        unsafe {
            SLOT_CALLS = 0;
            OBSERVED_SOURCE = ptr::null_mut();
            string_construct_from_vtable_slot_58(&mut out, &mut source);
            assert_eq!(SLOT_CALLS, 1, "the +0x58 slot runs exactly once");
