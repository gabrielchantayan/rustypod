//! Generic C++ red-black-tree right rotation — original: `FUN_083ccd40` at
//! load address `0x083ccd40`.
//!
//! Raw `osos.dec` establishes the exact 84-byte extent: 21 ARM words from
//! `ldr r2,[r1,#8]` at `0x083ccd40` through `bx lr` at `0x083ccd90`; the next
//! separately linked function begins at `0x083ccd94`. Decoding every aligned
//! ARM B/BL-immediate word in `osos.dec` finds five inbound direct calls: four
//! unconditional `bl` at `0x083cd08c`, `0x083cd104`, `0x083cd1f4`, and
//! `0x083cd5d0`, plus a predicated `bleq` at `0x083cd620`. The predicated site
...
///
/// Original: `FUN_083ccd40` at load address `0x083ccd40` (84 bytes; five
...
#[cfg_attr(target_os = "none", link_section = ".text.red_black_tree_rotate_right_fifth")]
#[inline(never)]
pub unsafe extern "C" fn red_black_tree_rotate_right_fifth(
...
mod tests {
    ...
    use super::red_black_tree_rotate_right_fifth;
    ...
    static SLAB: LazyLock<Option<usize>> = LazyLock::new(|| {
        crate::testing::try_map_u32_slab(
            crate::testing::hints::RED_BLACK_TREE_ROTATE_RIGHT_FIFTH,
            0x1000,
        )
        .map(|p| p as usize)
    });
    ...
    fn rotates_root_and_both_parent_child_slots() {
        let Some(base) = try_slab() else {
            crate::testing::note_missing_u32_fixture("red_black_tree_rotate_right_fifth");
            return;
        };
        ...
        unsafe {
            ...
            red_black_tree_rotate_right_fifth(tree, pivot);
