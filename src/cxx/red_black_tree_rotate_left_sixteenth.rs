//! Generic C++ red-black-tree left rotation — original: `FUN_083c84bc` at
//! load address `0x083c84bc`.
//!
//! Raw `osos.dec` establishes the exact 84-byte extent: 21 ARM words from
//! `ldr r2,[r1,#0xc]` at `0x083c84bc` through `bx lr` at `0x083c850c`; the
//! separately linked right-rotation sibling begins at `0x083c8510`. Decoding
//! every aligned ARM B/BL-immediate word in `osos.dec` finds five inbound
//! direct calls: four unconditional `bl` at `0x083c87f8`, `0x083c8898`,
//! `0x083c8984`, and `0x083c8e0c`, plus a predicated `bleq` at `0x083c8d74`.
...
///
/// Original: `FUN_083c84bc` at load address `0x083c84bc` (84 bytes; five
...
#[cfg_attr(target_os = "none", link_section = ".text.red_black_tree_rotate_left_sixteenth")]
#[inline(never)]
pub unsafe extern "C" fn red_black_tree_rotate_left_sixteenth(
...
mod tests {
    ...
    use super::{red_black_tree_rotate_left_sixteenth, RedBlackTree, RedBlackTreeNode};
    use core::ptr;
    use std::sync::LazyLock;

    static SLAB: LazyLock<Option<usize>> = LazyLock::new(|| {
        crate::testing::try_map_u32_slab(
            crate::testing::hints::RED_BLACK_TREE_ROTATE_LEFT_SIXTEENTH,
            0x1000,
        )
        .map(|p| p as usize)
    });
    ...
    fn rotates_root_and_both_parent_child_slots() {
        let Some(base) = try_slab() else {
            crate::testing::note_missing_u32_fixture("red_black_tree_rotate_left_sixteenth");
            return;
        };
        ...
        unsafe {
            ...
            red_black_tree_rotate_left_sixteenth(tree, pivot);
