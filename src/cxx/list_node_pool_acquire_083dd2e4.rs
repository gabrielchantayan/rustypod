//! List node-pool acquire — original: `FUN_083dd2e4` at load address
//! `0x083dd2e4`.
//!
//! Raw `osos.dec` establishes the true 184-byte extent: 46 A32 words from
//! `push {r4-r8,lr}` at `0x083dd2e4` through `pop {r4-r8,pc}` at
//! `0x083dd398`; `0x083dd39c` begins the next separately linked function.
//! The body contains two unconditional `bl` calls, both to
//! `operator_new_checked` @ `0x08266c70`, and no predicated `bl` calls. It
//! has two inbound plain `bl` calls, at `0x08292a18` and `0x082acc44`.
//!
...
#[cfg_attr(target_os = "none", link_section = ".text.list_node_pool_acquire_083dd2e4")]
#[inline(never)]
pub unsafe extern "C" fn list_node_pool_acquire_083dd2e4(
...
mod tests {
    ...
    fn acquires_free_single_initial_and_grown_nodes() {
        ...
        let Some(slab) = try_map_u32_slab(hints::LIST_NODE_POOL_ACQUIRE_083DD2E4, 0x8000) else {
            note_missing_u32_fixture("cxx/list_node_pool_acquire_083dd2e4");
            return;
        };
        unsafe {
            ...
            let sentinel = list_node_pool_acquire_083dd2e4(&mut list, 1);
            ...
            assert_eq!(list_node_pool_acquire_083dd2e4(&mut list, 0), sentinel);
            assert_eq!((list.free, (*sentinel).previous, CURSOR), (pointer_word(next_free.cast()), 7, 2));

            let mut list = ListNodePoolList::default();
            assert_eq!(list_node_pool_acquire_083dd2e4(&mut list, 0).cast::<u8>(), slab.add(0x3000));
            assert_eq!(SIZES[3], 32 * NODE_SIZE);
            list.next = list.end;
            assert_eq!(list_node_pool_acquire_083dd2e4(&mut list, 0).cast::<u8>(), slab.add(0x5000));
            assert_eq!(SIZES[5], 64 * NODE_SIZE);
            list.next = list.end;
            assert_eq!(list_node_pool_acquire_083dd2e4(&mut list, 0).cast::<u8>(), slab.add(0x7000));
