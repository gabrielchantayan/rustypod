//! Generic C++ red-black-tree left rotation — original: `FUN_083cccec` at
//! load address `0x083cccec`.
//!
//! Raw `osos.dec` establishes the exact 84-byte extent: 21 ARM words from
//! `ldr r2,[r1,#0xc]` at `0x083cccec` through `bx lr` at `0x083ccd3c`; the
//! separately linked sibling `FUN_083ccd40` begins at `0x083ccd40`. Decoding
//! every aligned ARM B/BL-immediate word in `osos.dec` finds five inbound
//! direct calls: four unconditional `bl` at `0x083cd028`, `0x083cd0c8`,
//! `0x083cd1b8`, and `0x083cd644`, plus predicated `bleq` at `0x083cd5ac`.
//! The predicated caller reaches this no-guard rotation only after selecting
//! the right child as its pivot; callers therefore supply a non-null pivot
//! with a non-null right child.
//!
...
///
/// Original: `FUN_083cccec` at load address `0x083cccec` (84 bytes; five
...
#[cfg_attr(target_os = "none", link_section = ".text.red_black_tree_rotate_left_fifth")]
#[inline(never)]
pub unsafe extern "C" fn red_black_tree_rotate_left_fifth(
...
mod tests {
    ...
    use super::red_black_tree_rotate_left_fifth;
    ...
    static SLAB: LazyLock<Option<usize>> = LazyLock::new(|| {
        crate::testing::try_map_u32_slab(
            crate::testing::hints::RED_BLACK_TREE_ROTATE_LEFT_FIFTH,
            0x1000,
        )
        .map(|p| p as usize)
    });
    ...
    fn rotates_root_and_both_parent_child_slots() {
        let Some(base) = try_slab() else {
            crate::testing::note_missing_u32_fixture("red_black_tree_rotate_left_fifth");
            return;
        };
        ...
        unsafe {
            ...
            red_black_tree_rotate_left_fifth(tree, pivot);
