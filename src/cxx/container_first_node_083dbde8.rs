...
//!
//! `cxx_container_first_node_083dbde8` — retailOS `FUN_083dbde8` @
//! **0x083dbde8**, **20 bytes** (`0x083dbde8..0x083dbdfb`; the following
//! `push {r1, r2, r3, lr}` at `0x083dbdfc` starts a separately linked
//! function). Raw A32 decoding finds zero plain or predicated body `bl`
//! instructions. Full-image A32 decoding finds two inbound unconditional plain
//! `bl` sites (0x081e1f14, 0x081e2044), and no predicated inbound `bl` sites.
//!
//! # Algorithm
//!
...
#[cfg_attr(target_os = "none", unsafe(link_section = ".text.cxx_container_first_node_083dbde8"))]
pub unsafe extern "C" fn cxx_container_first_node_083dbde8(header: *const u8) -> *mut u8 {
...
mod tests {
    ...
    fn returns_each_headers_nested_containers_first_node_word() {
        let Some(slab) = try_map_u32_slab(hints::CXX_CONTAINER_FIRST_NODE_083DBDE8, 0x1000) else {
            return;
        };
        unsafe {
            ...
            assert_eq!(cxx_container_first_node_083dbde8(first_header), first_node);
            assert_eq!(cxx_container_first_node_083dbde8(second_header), second_node);
        }
    }
}
