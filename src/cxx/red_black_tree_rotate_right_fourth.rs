//! Generic C++ red-black-tree right rotation, fourth copy — original:
//! `FUN_083ca3dc` at load address `0x083ca3dc`.
//!
//! Raw `osos.dec` establishes the exact 84-byte extent: 21 ARM words from
//! `ldr r2,[r1,#8]` at `0x083ca3dc` through `bx lr` at `0x083ca42c`; the next
//! separately linked function begins at `0x083ca430`. Decoding every aligned
//! ARM B/BL-immediate word in `osos.dec` finds five inbound direct calls: four
//! unconditional `bl` at `0x083ca728`, `0x083ca7a0`, `0x083ca88c`, and
//! `0x083cac64`, plus a predicated `bleq` at `0x083cacb4`. The predicated site
...
///
/// Original: `FUN_083ca3dc` at load address `0x083ca3dc` (84 bytes; five
...
#[cfg_attr(target_os = "none", link_section = ".text.red_black_tree_rotate_right_fourth")]
#[inline(never)]
pub unsafe extern "C" fn red_black_tree_rotate_right_fourth(
...
mod tests {
    ...
    use super::red_black_tree_rotate_right_fourth;
    ...
    static SLAB: LazyLock<Option<usize>> = LazyLock::new(|| {
        crate::testing::try_map_u32_slab(
            crate::testing::hints::RED_BLACK_TREE_ROTATE_RIGHT_FOURTH,
            0x1000,
        )
        .map(|p| p as usize)
    });
    ...
    fn rotates_root_and_both_parent_child_slots() {
        let Some(base) = try_slab() else {
            crate::testing::note_missing_u32_fixture("red_black_tree_rotate_right_fourth");
            return;
        };
        ...
        unsafe {
            ...
            red_black_tree_rotate_right_fourth(tree, pivot);
