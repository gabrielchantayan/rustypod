//! Generic C++ red-black-tree right rotation — original: `FUN_083c9998` at
//! load address `0x083c9998`.
//!
//! Raw `osos.dec` establishes the exact 84-byte extent: 21 ARM words from
//! `ldr r2,[r1,#8]` at `0x083c9998` through `bx lr` at `0x083c99e8`; the next
//! separately linked function begins at `0x083c99ec`. Decoding every aligned
//! ARM B/BL-immediate word in `osos.dec` finds five inbound direct calls: four
//! unconditional `bl` at `0x083c9ce4`, `0x083c9d5c`, `0x083c9e48`, and
//! `0x083ca220`, plus a predicated `bleq` at `0x083ca270`. The predicated site
...
///
/// Original: `FUN_083c9998` at load address `0x083c9998` (84 bytes; five
...
#[cfg_attr(target_os = "none", link_section = ".text.red_black_tree_rotate_right_ninth")]
#[inline(never)]
pub unsafe extern "C" fn red_black_tree_rotate_right_ninth(
...
mod tests {
    ...
    use super::{red_black_tree_rotate_right_ninth, RedBlackTree, RedBlackTreeNode};
    use core::ptr;
    use std::sync::LazyLock;

    static SLAB: LazyLock<Option<usize>> = LazyLock::new(|| {
        crate::testing::try_map_u32_slab(
            crate::testing::hints::RED_BLACK_TREE_ROTATE_RIGHT_NINTH,
            0x1000,
        )
        .map(|p| p as usize)
    });
    ...
    fn rotates_root_and_both_parent_child_slots() {
        let Some(base) = try_slab() else {
            crate::testing::note_missing_u32_fixture("red_black_tree_rotate_right_ninth");
            return;
        };
        ...
        unsafe {
            ...
            red_black_tree_rotate_right_ninth(tree, pivot);
