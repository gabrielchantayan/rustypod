...
//!
//! `guarded_vtable_slot_04_dispatch` — original: `FUN_083e72b0` @
//! **0x083e72b0** (36 bytes). Raw ARM establishes the extent: nine
//! instructions from `push {r4, lr}` at `0x083e72b0` through
//! `pop {r4, pc}` at `0x083e72d0`; the next function's `push {r4, lr}`
//! (sibling `FUN_083e72d4`, slot `+0x1c`) starts at `0x083e72d4`,
//! confirming the 36-byte extent Ghidra reports.
//!
//! Decoding every immediate ARM `B`/`BL` word in `osos.dec` finds exactly
//! four inbound direct calls, all unconditional `bl`: `0x0808e5bc`,
//! `0x0808e5dc`, `0x080c6c94`, and `0x080c6d34`. There are no predicated
//! call forms or direct tail branches. The body makes no static calls —
//! the "4 bl call sites" attributed to it are its four inbound callers.
//!
//! # Algorithm
//!
//! The argument is a handle: a pointer to an object pointer (recovered
//! callers pass the address of a stack local, e.g. `&local_54`). When the
...
//! word 1 (`+0x04`), and calls it as `method(object)` — the dispatch's
//! `r0` holds the inner object, not the handle. The method's result is
//! discarded; the function always returns the original handle in `r0`.
//! Recovered callers use it in cleanup paths alongside an object release,
//! consistent with a close/dispose method, but the concrete virtual
//! method identity remains unrecovered and is deliberately not inferred.
//!
...
#[inline(never)]
pub unsafe extern "C" fn guarded_vtable_slot_04_dispatch(
...
mod tests {
    ...
    fn null_object_returns_handle_without_dereferencing_vtable() {
        ...
        let result = unsafe { guarded_vtable_slot_04_dispatch(handle_pointer) };
