//! Generic C++ red-black-tree right rotation — original: `FUN_083c16dc` at
//! load address `0x083c16dc`.
//!
//! Raw `osos.dec` establishes the exact 84-byte extent: 21 ARM words from
//! `ldr r2,[r1,#0x8]` at `0x083c16dc` through `bx lr` at `0x083c172c`; the
//! separately linked rebalance helper begins at `0x083c1730`. Decoding every
//! aligned ARM B/BL-immediate word in `osos.dec` finds five inbound direct
//! calls: four unconditional `bl` at `0x083c1acc`, `0x083c1b44`,
//! `0x083c1c34`, and `0x083c2080`, plus a predicated `bleq` at `0x083c20d0`.
//! The predicated call gates this no-guard rotation; every caller supplies a
//! non-null pivot with a non-null left child.
//!
...
///
/// Original: `FUN_083c16dc` at load address `0x083c16dc` (84 bytes; five
...
#[cfg_attr(target_os = "none", link_section = ".text.red_black_tree_rotate_right_twentieth")]
#[inline(never)]
pub unsafe extern "C" fn red_black_tree_rotate_right_twentieth(
...
mod tests {
    ...
    use super::{red_black_tree_rotate_right_twentieth, RedBlackTree, RedBlackTreeNode};
    use core::ptr;
    use std::sync::LazyLock;

    static SLAB: LazyLock<Option<usize>> = LazyLock::new(|| {
        crate::testing::try_map_u32_slab(
            crate::testing::hints::RED_BLACK_TREE_ROTATE_RIGHT_TWENTIETH,
            0x1000,
        )
        .map(|p| p as usize)
    });
    ...
    fn rotates_root_and_both_parent_child_slots() {
        let Some(base) = try_slab() else {
            crate::testing::note_missing_u32_fixture("red_black_tree_rotate_right_twentieth");
            return;
        };
        ...
        unsafe {
            ...
            red_black_tree_rotate_right_twentieth(tree, pivot);
