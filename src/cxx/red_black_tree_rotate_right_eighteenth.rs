//! C++ red-black-tree right rotation — original: `FUN_083bb60c` at load
//! address `0x083bb60c`.
//!
//! Raw `osos.dec` establishes the exact 84-byte extent: 21 ARM words from
//! `ldr r2,[r1,#8]` at `0x083bb60c` through `bx lr` at `0x083bb65c`; the next
//! separately linked function begins at `0x083bb660`. Decoding every aligned
//! ARM B/BL-immediate word in `osos.dec` finds five inbound direct calls: four
//! unconditional `bl` at `0x083bb958`, `0x083bb9d0`, `0x083bbabc`, and
//! `0x083bbe8c`, plus a predicated `bleq` at `0x083bbedc`. The predicated site
...
#[cfg_attr(target_os = "none", link_section = ".text.red_black_tree_rotate_right_eighteenth")]
#[inline(never)]
pub unsafe extern "C" fn red_black_tree_rotate_right_eighteenth(
...
mod tests {
    ...
    use super::{red_black_tree_rotate_right_eighteenth, RedBlackTree, RedBlackTreeNode};
    use core::ptr;
    ...
    fn rotates_root_and_both_parent_child_slots() {
        let Some(base) = try_slab() else {
            crate::testing::note_missing_u32_fixture("red_black_tree_rotate_right_eighteenth");
            return;
        };
        ...
        unsafe {
            ...
            red_black_tree_rotate_right_eighteenth(tree, pivot);
