//! Generic C++ red-black-tree right rotation — original: `FUN_083bdfac` at
//! load address `0x083bdfac`.
//!
//! Raw `osos.dec` establishes the exact 84-byte extent: 21 ARM words from
//! `ldr r2,[r1,#8]` at `0x083bdfac` through `bx lr` at `0x083bdffc`; the next
//! separately linked function begins at `0x083be000`. Decoding every aligned
//! ARM B/BL-immediate word in `osos.dec` finds five inbound direct calls: four
//! unconditional `bl` at `0x083be2f8`, `0x083be370`, `0x083be45c`, and
//! `0x083be938`, plus a predicated `bleq` at `0x083be988`. The predicated site
...
///
/// Original: `FUN_083bdfac` at load address `0x083bdfac` (84 bytes; five
...
#[cfg_attr(target_os = "none", link_section = ".text.red_black_tree_rotate_right_tenth")]
#[inline(never)]
pub unsafe extern "C" fn red_black_tree_rotate_right_tenth(
...
mod tests {
    ...
    use super::{red_black_tree_rotate_right_tenth, RedBlackTree, RedBlackTreeNode};
    use core::ptr;
    use std::sync::LazyLock;

    static SLAB: LazyLock<Option<usize>> = LazyLock::new(|| {
        crate::testing::try_map_u32_slab(
            crate::testing::hints::RED_BLACK_TREE_ROTATE_RIGHT_TENTH,
            0x1000,
        )
        .map(|p| p as usize)
    });
    ...
    fn rotates_root_and_both_parent_child_slots() {
        let Some(base) = try_slab() else {
            crate::testing::note_missing_u32_fixture("red_black_tree_rotate_right_tenth");
            return;
        };
        ...
        unsafe {
            ...
            red_black_tree_rotate_right_tenth(tree, pivot);
