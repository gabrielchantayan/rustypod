//! Tests whether a condition queue's linked-item list is empty — `FUN_0839e42c` @
//! 0x0839e42c.
//!
//! Raw `osos.dec` establishes the true 48-byte extent
//! 0x0839e42c..0x0839e45c: twelve ARM words from `push {r4,r5,r6,lr}`
//! through `pop {r4,r5,r6,pc}`; the separate constructor begins at
//! 0x0839e45c. It contains three direct, unconditional `bl` calls and no
//! predicated `bl` calls.
//!
...
//! same mutex, then return the predicate result. The predicate at 0x0839e3bc
...
pub struct ConditionQueueIsEmptyAlternateOps {
    pub lock: MutexCall,
...
unsafe extern "C" fn missing_mutex_call(_mutex: *mut u8) {
    panic!("condition_queue_is_empty_alternate mutex operation was not installed")
}

#[cfg(not(target_os = "none"))]
unsafe extern "C" fn missing_list_is_empty(_queue: *mut u8) -> u32 {
    panic!("condition_queue_is_empty_alternate list predicate was not installed")
}

#[cfg(not(target_os = "none"))]
pub static mut CONDITION_QUEUE_IS_EMPTY_ALTERNATE_OPS: ConditionQueueIsEmptyAlternateOps = ConditionQueueIsEmptyAlternateOps {
...
unsafe fn lock(mutex: *mut u8) {
    ...
    #[cfg(not(target_os = "none"))]
    unsafe {
        let call = core::ptr::read_volatile(core::ptr::addr_of!(CONDITION_QUEUE_IS_EMPTY_ALTERNATE_OPS.lock));
        call(mutex);
    }
}
...
unsafe fn list_is_empty(queue: *mut u8) -> u32 {
    ...
    #[cfg(not(target_os = "none"))]
    unsafe {
        let call = core::ptr::read_volatile(core::ptr::addr_of!(CONDITION_QUEUE_IS_EMPTY_ALTERNATE_OPS.list_is_empty));
        call(queue)
    }
}
...
unsafe fn unlock(mutex: *mut u8) {
    ...
    #[cfg(not(target_os = "none"))]
    unsafe {
        let call = core::ptr::read_volatile(core::ptr::addr_of!(CONDITION_QUEUE_IS_EMPTY_ALTERNATE_OPS.unlock));
        call(mutex);
    }
}
...
pub unsafe extern "C" fn condition_queue_is_empty_alternate(queue: *mut u8) -> u32 {
...
mod tests {
    ...
    fn install_ops(result: u32) {
        unsafe {
            CALLS = [0; 3];
            CALL_COUNT = 0;
            RESULT = result;
            addr_of_mut!(CONDITION_QUEUE_IS_EMPTY_ALTERNATE_OPS).write(ConditionQueueIsEmptyAlternateOps {
                lock: record_lock,
    ...
    fn locks_queries_unlocks_and_returns_empty_result() {
        ...
        unsafe {
            ...
            install_ops(1);
            assert_eq!(condition_queue_is_empty_alternate(EXPECTED_QUEUE), 1);
    ...
    fn preserves_nonempty_result_after_unlock() {
        ...
        unsafe {
            ...
            EXPECTED_MUTEX = queue.as_mut_ptr().add(MUTEX_WORD_OFFSET).cast();
            install_ops(0);
            assert_eq!(condition_queue_is_empty_alternate(EXPECTED_QUEUE), 0);
            assert_eq!(CALLS, [1, 2, 3]);
        }
    }
}
