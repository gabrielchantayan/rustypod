//! Red-black-tree word-payload node-pool acquire — original: `FUN_083cec14`
//! at load address `0x083cec14`.
//!
//! Raw `osos.dec` establishes the exact 184-byte extent: 46 ARM words from
//! `push {r4-r8,lr}` at `0x083cec14` through `pop {r4-r8,pc}` at
//! `0x083cecc8`; the separately linked tree-rotation function starts at
//! `0x083ceccc`. The body has two unconditional `bl` instructions, both to
//! `operator_new_checked` @ `0x08266c70`, and no predicated calls. Decoding
//! every aligned ARM B/BL-immediate word finds three unconditional inbound
//! `bl` sites (`0x080d1eec`, `0x0818534c`, `0x083cf49c`) and none predicated.
//!
...
#[repr(C)]
#[derive(Clone, Copy)]
pub struct RedBlackTreePayloadWordNode {
    pub color: u8,
    pub _pad: [u8; 3],
    pub parent: u32,
    pub left: u32,
...
const _: [u8; 0x04] = [0; core::mem::offset_of!(RedBlackTreePayloadWordNode, parent)];
const _: [u8; 0x08] = [0; core::mem::offset_of!(RedBlackTreePayloadWordNode, left)];
const _: [u8; 0x0c] = [0; core::mem::offset_of!(RedBlackTreePayloadWordNode, right)];
const _: [u8; 0x10] = [0; core::mem::offset_of!(RedBlackTreePayloadWordNode, payload)];
const _: [u8; NODE_SIZE] = [0; core::mem::size_of::<RedBlackTreePayloadWordNode>()];
...
#[repr(C)]
#[derive(Clone, Copy, Default)]
pub struct RedBlackTreePayloadWordNodePool {
    pub chunks: u32,
    pub free: u32,
    pub next: u32,
    pub end: u32,
}
const _: [u8; 0x10] = [0; core::mem::size_of::<RedBlackTreePayloadWordNodePool>()];

#[inline(always)]
fn pointer_word(pointer: *mut u8) -> u32 { pointer as usize as u32 }
#[inline(always)]
unsafe fn node_from_word(word: u32) -> *mut RedBlackTreePayloadWordNode {
    word as usize as *mut RedBlackTreePayloadWordNode
}
#[inline(always)]
...
#[cfg_attr(target_os = "none", link_section = ".text.red_black_tree_payload_word_node_pool_acquire")]
#[inline(never)]
pub unsafe extern "C" fn red_black_tree_payload_word_node_pool_acquire(
    pool: *mut RedBlackTreePayloadWordNodePool,
) -> *mut RedBlackTreePayloadWordNode {
...
mod tests {
    ...
    fn acquires_fresh_free_and_grown_nodes() {
        ...
        let Some(slab) = try_map_u32_slab(hints::RED_BLACK_TREE_PAYLOAD_WORD_NODE_POOL_ACQUIRE, 0x5000) else { note_missing_u32_fixture("cxx/red_black_tree_payload_word_node_pool_acquire"); return; };
        unsafe {
            ...
            let mut pool = RedBlackTreePayloadWordNodePool::default();
            let first = red_black_tree_payload_word_node_pool_acquire(&mut pool);
            ...
            let second = slab.add(0x1040) as *mut RedBlackTreePayloadWordNode;
            (*first).right = pointer_word(second.cast()); (*first).payload = 7; (*first).color = 1; pool.free = pointer_word(first.cast());
            assert_eq!(red_black_tree_payload_word_node_pool_acquire(&mut pool), first);
            ...
            let grown = red_black_tree_payload_word_node_pool_acquire(&mut pool);
