//! Generic C++ red-black-tree left rotation — original: `FUN_083cd78c` at
//! load address `0x083cd78c`.
//!
//! Raw `osos.dec` establishes the exact 84-byte extent: 21 ARM words from
//! `ldr r2,[r1,#0xc]` at `0x083cd78c` through `bx lr` at `0x083cd7dc`; the
//! separately linked next function begins at `0x083cd7e0`. Decoding every
//! aligned ARM B/BL-immediate word in `osos.dec` finds five inbound direct
//! calls: four unconditional `bl` at `0x083cdad8`, `0x083cdb50`,
//! `0x083cdc3c`, and `0x083ce120`, plus a predicated `bleq` at `0x083ce170`.
//! The predicated site reaches this no-guard rotation only after its caller has
//! selected the right child as the pivot; all callers therefore supply a
...
///
/// Original: `FUN_083cd78c` at load address `0x083cd78c` (84 bytes; five
...
#[cfg_attr(target_os = "none", link_section = ".text.red_black_tree_rotate_left_third")]
#[inline(never)]
pub unsafe extern "C" fn red_black_tree_rotate_left_third(
...
mod tests {
    ...
    use super::{red_black_tree_rotate_left_third, RedBlackTree, RedBlackTreeNode};
    use core::ptr;
    use std::sync::LazyLock;

    static SLAB: LazyLock<Option<usize>> = LazyLock::new(|| {
        crate::testing::try_map_u32_slab(
            crate::testing::hints::RED_BLACK_TREE_ROTATE_LEFT_THIRD,
            0x1000,
        )
        .map(|p| p as usize)
    });
    ...
    fn rotates_root_and_both_parent_child_slots() {
        let Some(base) = try_slab() else {
            crate::testing::note_missing_u32_fixture("red_black_tree_rotate_left_third");
            return;
        };
        ...
        unsafe {
            ...
            red_black_tree_rotate_left_third(tree, pivot);
