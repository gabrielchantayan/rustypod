//! Word-list node-pool acquire — original: `FUN_083dd78c` at load address
//! `0x083dd78c`.
//!
//! Raw `osos.dec` establishes the true 180-byte extent: 45 ARM words from
//! `push {r4-r8,lr}` at `0x083dd78c` through `pop {r4-r8,pc}` at
//! `0x083dd840`; `0x083dd844` begins `list_node_pool_recycle_all`. The body
//! has exactly two unconditional `bl` calls, both to `operator_new_checked` @
//! `0x08266c70`, and no predicated `bl` calls.
//!
//! This allocates uninitialized 12-byte `{next, previous, value}` nodes from a
//! free list or a bump chunk. A `single` request allocates one node; ordinary
//! chunks start at 32 nodes then grow by `max(capacity + 32, capacity +
//! capacity/2 + capacity/8)`. Deliberate deviations: none; pointers remain
//! target-width `u32` words so host pointer width cannot alter the layout.
...
const _: [u8; 0x04] = [0; core::mem::offset_of!(WordListNode, previous)];
const _: [u8; 0x08] = [0; core::mem::offset_of!(WordListNode, value)];
const _: [u8; NODE_SIZE] = [0; core::mem::size_of::<WordListNode>()];

#[repr(C)]
#[derive(Clone, Copy, Default)]
struct WordListNodePoolChunk {
    previous: u32,
    capacity: u32,
    nodes: u32,
}

const _: [u8; 0x04] = [0; core::mem::offset_of!(WordListNodePoolChunk, capacity)];
const _: [u8; 0x08] = [0; core::mem::offset_of!(WordListNodePoolChunk, nodes)];
const _: [u8; 0x0c] = [0; core::mem::size_of::<WordListNodePoolChunk>()];

/// The target-width first 16 bytes of an ADS word-list header.
#[repr(C)]
#[derive(Clone, Copy, Default)]
pub struct WordListNodePool {
    pub chunks: u32,
    pub free: u32,
    pub next: u32,
    pub end: u32,
...
const _: [u8; 0x04] = [0; core::mem::offset_of!(WordListNodePool, free)];
const _: [u8; 0x08] = [0; core::mem::offset_of!(WordListNodePool, next)];
const _: [u8; 0x0c] = [0; core::mem::offset_of!(WordListNodePool, end)];
const _: [u8; 0x10] = [0; core::mem::size_of::<WordListNodePool>()];
...
#[inline(always)]
unsafe fn chunk_from_word(word: u32) -> *mut WordListNodePoolChunk {
    word as usize as *mut WordListNodePoolChunk
}

/// Acquires an uninitialized node from `pool`; `single != 0` forces a
/// one-node chunk, matching the list sentinel constructor.
///
/// # Safety
///
...
#[cfg_attr(target_os = "none", link_section = ".text.word_list_node_pool_acquire")]
#[inline(never)]
pub unsafe extern "C" fn word_list_node_pool_acquire(
    ...
    if (*pool).next == (*pool).end {
        ...
        let chunk = operator_new_checked(core::mem::size_of::<WordListNodePoolChunk>())
            as *mut WordListNodePoolChunk;
...
mod tests {
    ...
    fn acquires_single_free_fresh_and_grown_nodes() {
        ...
        let Some(slab) = try_map_u32_slab(hints::WORD_LIST_NODE_POOL_ACQUIRE, 0x8000) else {
            note_missing_u32_fixture("cxx/word_list_node_pool_acquire");
            return;
        };
        ...
        unsafe {
            ...
            let sentinel = word_list_node_pool_acquire(&mut pool, 1);
            ...
            let chunk1 = slab.cast::<WordListNodePoolChunk>();
            ...
            assert_eq!(word_list_node_pool_acquire(&mut pool, 0), sentinel);
            ...
            let mut pool = WordListNodePool::default();
            let first = word_list_node_pool_acquire(&mut pool, 0);
