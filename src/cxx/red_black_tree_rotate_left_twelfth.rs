//! Generic C++ red-black-tree left rotation — original: `FUN_083bdf58` at
//! load address `0x083bdf58`.
//!
//! Raw `osos.dec` establishes the exact 84-byte extent: 21 ARM words from
//! `ldr r2,[r1,#0xc]` at `0x083bdf58` through `bx lr` at `0x083bdfa8`; the
//! separately linked right-rotation sibling begins at `0x083bdfac`. Decoding
//! every aligned ARM B/BL-immediate word in `osos.dec` finds five inbound direct
//! calls: four unconditional `bl` at `0x083be294`, `0x083be334`, `0x083be420`,
//! and `0x083be9ac`, plus a predicated `bleq` at `0x083be914`. The predicated
...
///
/// Original: `FUN_083bdf58` at load address `0x083bdf58` (84 bytes; five
...
#[cfg_attr(target_os = "none", link_section = ".text.red_black_tree_rotate_left_twelfth")]
#[inline(never)]
pub unsafe extern "C" fn red_black_tree_rotate_left_twelfth(
...
mod tests {
    ...
    use super::{red_black_tree_rotate_left_twelfth, RedBlackTree, RedBlackTreeNode};
    use core::ptr;
    use std::sync::LazyLock;

    static SLAB: LazyLock<Option<usize>> = LazyLock::new(|| {
        crate::testing::try_map_u32_slab(
            crate::testing::hints::RED_BLACK_TREE_ROTATE_LEFT_TWELFTH,
            0x1000,
        )
        .map(|p| p as usize)
    });
    ...
    fn rotates_root_and_both_parent_child_slots() {
        let Some(base) = try_slab() else {
            crate::testing::note_missing_u32_fixture("red_black_tree_rotate_left_twelfth");
            return;
        };
        ...
        unsafe {
            ...
            red_black_tree_rotate_left_twelfth(tree, pivot);
