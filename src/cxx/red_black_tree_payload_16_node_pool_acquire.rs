//! Red-black-tree 16-byte-payload node-pool acquire — original:
//! `FUN_083c4d30` at load address `0x083c4d30`.
//!
//! Raw `osos.dec` establishes the exact 176-byte extent: 44 ARM words from
//! `push {r4-r8,lr}` at `0x083c4d30` through `pop {r4-r8,pc}` at
//! `0x083c4ddc`; the separately linked next function
//! (`cxx_string_vector_record_link`) starts at `0x083c4de0`. The body holds
//! exactly two `bl` instructions, both unconditional calls to
//! `operator_new_checked` @ `0x08266c70`; there are no predicated calls and
//! Decoding every ARM B/BL-immediate word in `osos.dec`
//! finds exactly four inbound direct callers, all unconditional `bl` at
//! `0x08134dfc`, `0x081cdb0c`, `0x083c5648`, and `0x083c5714`; there are no
//! predicated direct calls or plain `b` tail calls.
//!
//! The pool first removes a node from its free list threaded through
//! node+0x0c. Otherwise it bumps through its current 0x20-byte node chunk,
//! allocating a new 12-byte chunk header and backing chunk when exhausted.
//! The first chunk has 32 nodes; later chunks grow by `max(capacity + 32,
//! capacity + capacity/2 + capacity/8)`. The returned node's color byte and
//! three red-black-tree link words are cleared, preserving its 16-byte
//! client payload at +0x10.
//!
...
//! than handled. This is the 0x20-stride twin of
//! `red_black_tree_payload_24_node_pool_acquire` @ `0x083c14c8`; the two
//! keep separate hooks, so the bodies are not merged.
...
/// A 0x20-byte red-black-tree node with a caller-owned 16-byte payload.
#[repr(C)]
#[derive(Clone, Copy)]
pub struct RedBlackTreePayload16Node {
    /// Color byte; the remaining three bytes are padding.
    pub color: u8,
    pub _pad: [u8; 3],
    pub parent: u32,
    pub left: u32,
    /// Free-list link while the node is pooled.
    pub right: u32,
    pub payload: [u8; 16],
}
...
struct RedBlackTreePayload16NodePoolChunk {
    pub previous: u32,
    pub capacity: u32,
    pub nodes: u32,
}

const _: [u8; 0x04] = [0; core::mem::offset_of!(RedBlackTreePayload16NodePoolChunk, capacity)];
const _: [u8; 0x08] = [0; core::mem::offset_of!(RedBlackTreePayload16NodePoolChunk, nodes)];
const _: [u8; 0x0c] = [0; core::mem::size_of::<RedBlackTreePayload16NodePoolChunk>()];

/// The target-width node-pool state consumed by
/// [`red_black_tree_payload_16_node_pool_acquire`].
#[repr(C)]
#[derive(Clone, Copy, Default)]
...
const _: [u8; 0x04] = [0; core::mem::offset_of!(RedBlackTreePayload16NodePool, free)];
const _: [u8; 0x08] = [0; core::mem::offset_of!(RedBlackTreePayload16NodePool, next)];
const _: [u8; 0x0c] = [0; core::mem::offset_of!(RedBlackTreePayload16NodePool, end)];
...
#[inline(always)]
unsafe fn chunk_from_word(word: u32) -> *mut RedBlackTreePayload16NodePoolChunk {
    word as usize as *mut RedBlackTreePayload16NodePoolChunk
}

/// Acquires and initializes one 16-byte-payload node from `pool`.
///
/// Original: `FUN_083c4d30` at load address `0x083c4d30` (176 bytes; two
/// unconditional internal `bl` instructions, four unconditional inbound
/// `bl` sites).
///
/// # Safety
///
/// `pool` must point to a writable [`RedBlackTreePayload16NodePool`]. Existing
...
#[cfg_attr(target_os = "none", link_section = ".text.red_black_tree_payload_16_node_pool_acquire")]
#[inline(never)]
pub unsafe extern "C" fn red_black_tree_payload_16_node_pool_acquire(
    ...
    let node = if (*pool).free != 0 {
        ...
        if next as u32 == (*pool).end {
            ...
            let chunk = operator_new_checked(core::mem::size_of::<RedBlackTreePayload16NodePoolChunk>())
                as *mut RedBlackTreePayload16NodePoolChunk;
...
mod tests {
    ...
    fn acquires_from_new_free_and_grown_chunks() {
        ...
        let Some(slab) = try_map_u32_slab(hints::RED_BLACK_TREE_PAYLOAD_16_NODE_POOL_ACQUIRE, 0x7000) else {
            note_missing_u32_fixture("cxx/red_black_tree_payload_16_node_pool_acquire");
            return;
        };
        ...
        unsafe {
            ...
            let first = red_black_tree_payload_16_node_pool_acquire(&mut pool);
            ...
            let first_chunk = slab as *mut RedBlackTreePayload16NodePoolChunk;
            ...
            assert_eq!((*first).payload, [0xa5; 16]);

            let second_free = slab.add(0x1080) as *mut RedBlackTreePayload16Node;
            (*first).color = 1;
            (*first).parent = 2;
            (*first).left = 3;
            ...
            assert_eq!(red_black_tree_payload_16_node_pool_acquire(&mut pool), first);
            ...
            assert_eq!((*first).left, 0);
            assert_eq!((*first).right, 0);
            assert_eq!((*first).payload, [4; 16]);

            pool.free = 0;
            pool.next = pool.end;
            let grown = red_black_tree_payload_16_node_pool_acquire(&mut pool);
            ...
            let second_chunk = slab.add(0x2000) as *mut RedBlackTreePayload16NodePoolChunk;
            ...
            assert_eq!((*grown).payload, [0xa5; 16]);

            pool.next = pool.end;
            let geometric = red_black_tree_payload_16_node_pool_acquire(&mut pool);
            ...
            let third_chunk = slab.add(0x4000) as *mut RedBlackTreePayload16NodePoolChunk;
