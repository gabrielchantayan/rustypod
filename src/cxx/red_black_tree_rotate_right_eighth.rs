//! Generic C++ red-black-tree right rotation — original: `FUN_083beaf4` at
//! load address `0x083beaf4`.
//!
//! Raw `osos.dec` establishes the exact 84-byte extent: 21 ARM words from
//! `ldr r2,[r1,#0x8]` at `0x083beaf4` through `bx lr` at `0x083beb44`; the
//! separately linked next function begins at `0x083beb48`. Decoding every
//! aligned ARM B/BL-immediate word in `osos.dec` finds five inbound direct
//! calls: four unconditional `bl` at `0x083bee40`, `0x083beeb8`,
//! `0x083befa4`, and `0x083bf374`, plus a predicated `bleq` at `0x083bf3c4`.
...
///
/// Original: `FUN_083beaf4` at load address `0x083beaf4` (84 bytes; five
...
#[cfg_attr(target_os = "none", link_section = ".text.red_black_tree_rotate_right_eighth")]
#[inline(never)]
pub unsafe extern "C" fn red_black_tree_rotate_right_eighth(
...
mod tests {
    ...
    use super::{red_black_tree_rotate_right_eighth, RedBlackTree, RedBlackTreeNode};
    use core::ptr;
    use std::sync::LazyLock;

    static SLAB: LazyLock<Option<usize>> = LazyLock::new(|| {
        crate::testing::try_map_u32_slab(
            crate::testing::hints::RED_BLACK_TREE_ROTATE_RIGHT_EIGHTH,
            0x1000,
        )
        .map(|p| p as usize)
    });
    ...
    fn rotates_root_and_both_parent_child_slots() {
        let Some(base) = try_slab() else {
            crate::testing::note_missing_u32_fixture("red_black_tree_rotate_right_eighth");
            return;
        };
        ...
        unsafe {
            ...
            red_black_tree_rotate_right_eighth(tree, pivot);
