//! copy_byte_range_returning_end — original: `FUN_083e9e38` @ 0x083e9e38 (24 bytes).
//!
//! Raw `osos.dec` words establish the exact six-word ARM body from 0x083e9e38
//! through 0x083e9e4c (`bx lr`); the separately linked next function begins at
//! 0x083e9e50. Whole-image A32 decoding finds three inbound direct plain `bl`
//! calls (0x080fd008, 0x080fd01c, and 0x080fd038) and no predicated `bl` calls.
//! The body contains no outbound plain or predicated `bl` instructions.
//!
...
//!
//! Deviation: volatile accesses prevent LLVM from replacing this separately
//! linked retail leaf with a memcpy intrinsic; the target-only text section
//! prevents identical-code folding with other forward byte-copy ports.
...
#[cfg_attr(target_os = "none", link_section = ".text.copy_byte_range_returning_end_083e9e38")]
#[inline(never)]
pub unsafe extern "C" fn copy_byte_range_returning_end(
    mut begin: *const u8,
    end: *const u8,
    mut dst: *mut u8,
) -> *mut u8 {
...
mod tests {
        ...
        for source_offset in 0..4 {
            for destination_offset in 0..4 {
                for len in 0..=LEN {
                    ...
                    let returned = unsafe { copy_byte_range_returning_end(begin, end, output) };
    ...
    fn empty_range_returns_destination_without_dereferencing() {
        let returned = unsafe {
            copy_byte_range_returning_end(core::ptr::null(), core::ptr::null(), core::ptr::null_mut())
        };
